...
def demonstrate_adaptive_batching():
    ...
    for i in range(10):
        ...
        optimal_size = batch_manager.get_optimal_batch_size(processing_time, tokens_processed)
        ...
        if 'latest_metrics' in status and status['latest_metrics']:
            print(f"     Memory pressure: {status['latest_metrics']['memory_pressure_ratio']:.2f}")
            print(f"     Performance score: {status['latest_metrics']['performance_score']:.2f}")
        else:
    ...
    print(f"   Average processing time: {final_status['average_processing_time_ms']:.2f}ms")
    print(f"   Average throughput: {final_status['average_throughput_tokens_per_sec']:.2f} tokens/sec")
...
def demonstrate_plugin_integration():
    ...
    print(f"   Has adaptive batching methods: {hasattr(plugin, 'setup_adaptive_batching')}\n")
    ...
    for i in range(3):
        ...
        print(f"   Scenario {i+1}: Processed {tokens_processed} tokens in {processing_time}ms -> Recommended batch size: {optimal_size}")
...
if __name__ == "__main__":
    ...
    print("\n=== Summary ===")
    print("The adaptive micro-batching system successfully monitors performance and memory")
    print("metrics to dynamically adjust batch sizes for optimal efficiency. The system")
    print("can detect performance degradation and memory pressure, responding by reducing")
