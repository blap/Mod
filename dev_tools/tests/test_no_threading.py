...
def test_basic_functionality():
    ...
    memory_manager.memory_predictor.record_memory_usage(
        current_time + 1, 2000000
    )  # 2MB
...
def test_multiple_tensors():
    ...
    for i in range(3):
        ...
        result = tensor_paging_manager.page_tensor(
            tensor, tensor_id, MemoryPriority.MEDIUM
        )
    ...
    for tensor, tensor_id in tensors_data:
        ...
        if retrieved is not None:
            ...
            print(
                f"  {tensor_id}: accessed={retrieved is not None}, equal={tensors_equal}"
            )
