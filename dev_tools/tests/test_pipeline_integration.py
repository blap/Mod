...
def test_base_plugin_interface_pipeline_methods():
    ...
    for method_name in base_methods:
        assert hasattr(
            ModelPluginInterface, method_name
        ), f"ModelPluginInterface missing {method_name}"
        assert hasattr(
            TextModelPluginInterface, method_name
        ), f"TextModelPluginInterface missing {method_name}"
...
def test_pipeline_config_updates():
    ...
    assert (
        "enable_pipeline" in glm_config["parameters"]
    ), "GLM-4.7 config missing enable_pipeline"
    assert (
        "pipeline_checkpoint_dir" in glm_config["parameters"]
    ), "GLM-4.7 config missing pipeline_checkpoint_dir"
    assert (
        "pipeline_stages" in glm_config["parameters"]
    ), "GLM-4.7 config missing pipeline_stages"
    print("  ✓ GLM-4.7 config has pipeline settings")

    # Check Qwen3-4B-Instruct config
    qwen_config_path = (
        Path(__file__).parent / "plugin_configs" / "qwen3_4b_instruct_2507_config.json"
    )
    with open(qwen_config_path, "r") as f:
        qwen_config = json.load(f)

    assert (
        "enable_pipeline" in qwen_config["parameters"]
    ), "Qwen3-4B-Instruct config missing enable_pipeline"
    assert (
        "pipeline_checkpoint_dir" in qwen_config["parameters"]
    ), "Qwen3-4B-Instruct config missing pipeline_checkpoint_dir"
    assert (
        "pipeline_stages" in qwen_config["parameters"]
    ), "Qwen3-4B-Instruct config missing pipeline_stages"
    print("  ✓ Qwen3-4B-Instruct config has pipeline settings")

    # Check other configs too
    qwen_coder_config_path = (
        Path(__file__).parent
        / "plugin_configs"
        / "qwen3_coder_30b_a3b_instruct_config.json"
    )
    with open(qwen_coder_config_path, "r") as f:
        qwen_coder_config = json.load(f)

    assert (
        "enable_pipeline" in qwen_coder_config["parameters"]
    ), "Qwen3-Coder config missing enable_pipeline"
    print("  ✓ Qwen3-Coder config has pipeline settings")

    qwen_vl_config_path = (
        Path(__file__).parent / "plugin_configs" / "qwen3_vl_2b_instruct_config.json"
    )
    with open(qwen_vl_config_path, "r") as f:
        qwen_vl_config = json.load(f)

    assert (
        "enable_pipeline" in qwen_vl_config["parameters"]
    ), "Qwen3-VL config missing enable_pipeline"
...
def main():
    ...
    print(
        "ALL INTEGRATION TESTS PASSED! Disk-based inference pipeline system is fully integrated."
    )
