...
def test_pipeline_implementation():
    ...
    try:
        from inference_pio.common.disk_pipeline import (
            DiskBasedPipeline,
            PipelineManager,
            PipelineStage,
        )
        ...
        print(
            "   PASS: DiskBasedPipeline, PipelineStage, PipelineManager imported successfully"
        )
        ...
        for config_file in config_files:
            try:
                ...
                has_pipeline_settings = (
                    "enable_pipeline" in config.get("parameters", {})
                    and "pipeline_checkpoint_dir" in config.get("parameters", {})
                    and "pipeline_stages" in config.get("parameters", {})
                )

                status = "PASS" if has_pipeline_settings else "FAIL"
                print(
                    f"   {status}: {config_file}: {'Has pipeline settings' if has_pipeline_settings else 'Missing pipeline settings'}"
                )
