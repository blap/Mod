...
def test_model_creation():
    ...
    try:
        ...
        print(f"  ✓ Model created with {len(model.language_model.layers)} language layers")
        print(f"  ✓ Model has {len(model.vision_tower.layers)} vision layers")
...
def test_model_forward_pass():
    ...
    try:
        ...
        model = Qwen3VLForConditionalGeneration(config)
        model.eval()
        print("  ✓ Model initialized in evaluation mode")
        ...
        print(f"  ✓ Text-only forward pass completed, output shape: {text_output.shape}")
        
        # Forward pass with image only
        with torch.no_grad():
            vision_output = model(pixel_values=pixel_values)
        print(f"  ✓ Vision-only forward pass completed, output shape: {vision_output.shape}")
        
        # Forward pass with multimodal input
        with torch.no_grad():
            multimodal_output = model(input_ids=input_ids, pixel_values=pixel_values)
        print(f"  ✓ Multimodal forward pass completed, output shape: {multimodal_output.shape}")
        ...
        print("  ✓ All outputs contain finite values")
...
def test_model_generation():
    ...
    try:
        ...
        model = Qwen3VLForConditionalGeneration(config)
        model.eval()
        print("  ✓ Model initialized for generation")
        ...
        print(f"  ✓ Text-only generation completed, output shape: {generated_text.shape}")
        ...
        print(f"  ✓ Multimodal generation completed, output shape: {generated_multimodal.shape}")
        ...
        print("  ✓ Generated tokens are valid")
...
def benchmark_model_performance():
    ...
    try:
        ...
        model = Qwen3VLForConditionalGeneration(config)
        model.eval()
        print("  ✓ Model initialized for benchmarking")
        ...
        print(f"  ✓ Forward pass benchmark completed: {avg_time:.4f}s per run")
        ...
        print(f"  ✓ Generation benchmark completed: {gen_time:.4f}s per run")
...
def test_memory_efficiency():
    ...
    try:
        ...
        print("  ✓ Baseline and optimized models created")
...
def run_comprehensive_test_suite():
    ...
    if passed_tests == total_tests:
        print("\n🎉 ALL TESTS AND BENCHMARKS PASSED! Qwen3-VL model components are working correctly.")
        return True
    else:
        print(f"\n❌ {total_tests - passed_tests} COMPONENT(S) FAILED!")
        return False
