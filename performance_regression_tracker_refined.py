"""
Refined Performance Regression Tracking System
...
from src.inference_pio.common.test_utilities import (
    calculate_statistics, format_duration, format_bytes, sanitize_filename
)
...
class PerformanceRegressionTracker:
    ...
    def __init__(self, storage_dir: str = "performance_history",
    ...
    def log_alert(self, alert: RegressionAlert) -> None:
        ...
        with open(alerts_log_file, 'a') as f:
            f.write(f"[{datetime.fromtimestamp(alert.timestamp)}] {alert.severity.value.upper()}: {alert.message}\n")
    ...
    def generate_report(self, output_dir: str = "performance_reports") -> str:
        ...
        with open(report_file, 'w') as f:
            ...
            # Summary section
            f.write("## Summary\n\n")
            f.write(f"- Total metrics tracked: {sum(len(metrics) for metrics in self.metrics_history.values())}\n")
            f.write(f"- Unique metrics: {len(self.metrics_history)}\n")
            f.write(f"- Regression alerts: {len(self.alerts)}\n")
            f.write(f"- Critical alerts: {len([a for a in self.alerts if a.severity == RegressionSeverity.CRITICAL])}\n\n")
            ...
            for metric_key, metrics in self.metrics_history.items():
                if metrics:  # Only show metrics that have data
                    ...
                    if stats:
                        f.write(f"- Historical mean: {stats['mean']:.2f} {latest.unit}\n")
                        f.write(f"- Historical stdev: {stats['stdev']:.2f} {latest.unit}\n")
    ...
    def export_csv(self, output_dir: str = "performance_exports") -> str:
        ...
        with open(csv_file, 'w', newline='') as f:
            ...
            # Write header
            writer.writerow([
                'Model Name', 'Metric Name', 'Value', 'Unit', 'Category',
                'Timestamp', 'Date Time'
            ])
