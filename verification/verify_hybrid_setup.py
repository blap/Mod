...
def verify_hybrid_setup():
    ...
    logger.info(f"Created Mock Profile: {user_profile.cpu_brand}, GPU: {user_profile.gpu_name}")
    ...
    distribution = plugin.get_layer_distribution(total_layers, model_size_gb, available_vram)
    ...
    if gpu_layers != expected_gpu:
        logger.warning(f"GPU layers mismatch. Expected {expected_gpu}, got {gpu_layers}")
        # Allow small deviation due to float precision, but strict check is better

    if gpu_layers + cpu_layers != total_layers:
        logger.error(f"Total layers mismatch! {gpu_layers} + {cpu_layers} != {total_layers}")
        return False
