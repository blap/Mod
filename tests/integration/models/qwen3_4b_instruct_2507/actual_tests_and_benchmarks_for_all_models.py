...
try:
    ...
    print("✓ Successfully imported GLM-4-7 plugin")
except Exception as e:
    print(f"✗ Failed to import GLM-4-7 plugin: {e}")
...
try:
    ...
    print("✓ Successfully imported Qwen3-4B-Instruct-2507 plugin")
except Exception as e:
    print(f"✗ Failed to import Qwen3-4B-Instruct-2507 plugin: {e}")
...
try:
    ...
    print("✓ Successfully imported Qwen3-Coder-30B plugin")
except Exception as e:
    print(f"✗ Failed to import Qwen3-Coder-30B plugin: {e}")
...
try:
    ...
    print("✓ Successfully imported Qwen3-VL-2B plugin")
except Exception as e:
    print(f"✗ Failed to import Qwen3-VL-2B plugin: {e}")
...
def run_basic_test(model_name, plugin_creator_func):
    ...
    try:
        ...
        if plugin is None:
            print(f"   [FAIL] {model_name} plugin creation returned None")
            return False
        ...
        if not success:
            print(f"   [FAIL] {model_name} plugin initialization failed")
            return False
        ...
        if model is None:
            print(f"   [FAIL] {model_name} model loading returned None")
            return False
        ...
        if result is None:
            print(f"   [FAIL] {model_name} inference returned None")
            return False
        ...
        if generated is None:
            print(f"   [FAIL] {model_name} text generation returned None")
            return False
        
        print(f"   [PASS] {model_name} text generation completed")
        print(f"   Generated: {generated[:50]}..." if len(generated) > 50 else f"   Generated: {generated}")
        
        # Get model info
        info = plugin.get_model_info()
        if info is None:
            print(f"   [FAIL] {model_name} model info retrieval failed")
            return False
        
        print(f"   [PASS] {model_name} model info retrieved")
        print(f"   Model name in info: {info.get('name', 'N/A')}")
        
        print(f"\n[SUCCESS] ALL BASIC TESTS PASSED FOR {model_name}")
        return True
        
    except Exception as e:
        print(f"   [FAIL] {model_name} test failed with exception: {str(e)}")
        import traceback
        traceback.print_exc()
        return False
...
def run_basic_benchmark(model_name, plugin_creator_func):
    ...
    try:
        ...
        print(f"   Generation: {avg_chars_per_sec:.2f} chars/sec across {len(prompts)} prompts")
        print(f"   [PASS] {model_name} generation speed benchmark completed")
        
        print(f"\n[SUCCESS] ALL BENCHMARKS COMPLETED FOR {model_name}")
        return True
        
    except Exception as e:
        print(f"   [FAIL] {model_name} benchmark failed with exception: {str(e)}")
        import traceback
        traceback.print_exc()
        return False
