...
class MockModel(RealGLM47Model):
    """
    A real model for testing purposes.
    """
    def __init__(self):
    ...
    def forward(self, x):
        # Convert input to the expected format for the real model
        if x.dim() == 2 and x.size(1) == 10:
            # If input is (batch, 10), treat as token IDs
            input_ids = x.argmax(dim=1).unsqueeze(0)  # Convert to token IDs
            return super().forward(input_ids=input_ids)
        else:
            # For other inputs, use the parent forward method
            return super().forward(input_ids=x.long())
