"""
Comprehensive Test Reporting System for CI/CD Integration
...
class TestReportGenerator:
    ...
    def _collect_system_info(self) -> SystemInfo:
        ...
        info = SystemInfo(
            ...
            os_version=os.uname().version if hasattr(os, 'uname') else "Unknown",
            cpu_info=cpuinfo.get_cpu_info()['brand_raw'] if hasattr(cpuinfo.get_cpu_info(), 'brand_raw') else "Unknown",
        ...
        if torch.cuda.is_available():
            ...
            info.gpu_usage = torch.cuda.utilization() if hasattr(torch.cuda, 'utilization') else None
            info.gpu_memory = torch.cuda.get_device_properties(0).total_memory / (1024**3) if info.gpu_count > 0 else None

        return info

    def calculate_metrics(self, test_results: List[TestResult]) -> TestSuiteMetrics:
        """
        ...
        metrics = TestSuiteMetrics(
            ...
            start_time=min(result.start_time for result in test_results if result.start_time) if test_results else 0,
            end_time=max(result.end_time for result in test_results if result.end_time) if test_results else 0,
    ...
    def generate_json_report(self, test_results: List[TestResult], metrics: TestSuiteMetrics) -> str:
        """
    ...
    def generate_xml_report(self, test_results: List[TestResult], metrics: TestSuiteMetrics) -> str:
        """
    ...
    def generate_html_report(self, test_results: List[TestResult], metrics: TestSuiteMetrics) -> str:
        """
...
<body>
    ...
    def generate_markdown_report(self, test_results: List[TestResult], metrics: TestSuiteMetrics) -> str:
        """
...
"""
        ...
        for result in test_results:
            ...
            md_content += f"| {result.name} | {status} | {result.duration:.2f} | {error_msg} |\n"
    ...
    def generate_all_reports(self, test_results: List[TestResult]) -> Dict[str, str]:
        """
...
# Standalone function to run tests with reporting
def run_tests_with_reporting(test_functions, output_dir="test_reports"):
    """
    ...
    report_generator = TestReportGenerator(output_dir)
    test_results = []
    ...
    return report_generator.generate_all_reports(test_results)