...
This version tests the fixture system in isolation
"""
...
# Execute the module to define the classes and functions
spec.loader.exec_module(test_fixtures)

# Now we can access the classes and functions from the imported module
FixtureManager = test_fixtures.FixtureManager
fixture = test_fixtures.fixture
use_fixtures = test_fixtures.use_fixtures
fixture_context = test_fixtures.fixture_context
get_fixture_manager = test_fixtures.get_fixture_manager
reset_fixture_manager = test_fixtures.reset_fixture_manager
cleanup_test_resources = test_fixtures.cleanup_test_resources
...
DatabaseFixture = test_fixtures.DatabaseFixture
...
def test_get_fixture_function_scope():
    ...
    # For function-scoped fixtures, each call should create a new instance
    # So the IDs should be different, but the structure should be the same
    assert_equal(type(instance1), type(instance2))  # Same type
    assert_equal(list(instance1.keys()), list(instance2.keys()))  # Same keys

    # For function scope, there should be no cached instance in the registry
    # The fixture should be created fresh each time
    assert_false(manager._fixtures["sample"]["initialized"])
    assert_is_none(manager._fixtures["sample"]["instance"])
...
def test_fixture_decorator():
    ...
    # Create a fixture using the decorator
...
def test_mock_model_types():
    ...
    # Both models should have at least 1 layer
    assert_greater_equal(len(simple_model.model), 1)
    assert_greater_equal(len(conv_model.model), 1)
