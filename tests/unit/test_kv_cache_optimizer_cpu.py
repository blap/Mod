...
def test_low_rank_compressor_basic():
    ...
    # Create memory manager with smaller pool to avoid memory issues
    memory_config = MemoryConfig(memory_pool_size=1024*1024*64)  # 64MB
...
def test_create_optimized_attention_helper():
    ...
    hidden_states = torch.randn(batch_size, seq_len, 256, device='cpu')
