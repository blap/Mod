"""
Consolidated tests for the Memory Optimization System

This test suite combines and consolidates the functionality from:
- test_memory_optimization_system.py
- test_memory_optimization_system_simple.py
...
from src.qwen3_vl.components.memory.memory_optimization_system import (
    MemoryConfig, BuddyAllocator, TensorCache, MemoryPool,
    MemoryDefragmenter, GradientCheckpointingMemoryIntegrator,
    VisionEncoderMemoryOptimizer, MemoryManager,
    get_memory_manager, allocate_tensor_with_manager, free_tensor_with_manager
)


class TestMemoryOptimizationSystem(unittest.TestCase):
    """Test suite for the comprehensive memory optimization system"""
    ...
    def test_memory_manager_basic(self):
        ...
        manager = MemoryManager(memory_pool_size=2**20)  # 1MB
    ...
    def test_gradient_checkpointing_integration(self):
        """Test gradient checkpointing memory integration"""
        manager = MemoryManager(memory_pool_size=2**20)
        ...
        result = chkpt_integrator.checkpoint_tensors(test_tensors, "test_key")

        self.assertIn('checkpoint_key', result)
        self.assertGreaterEqual(result['saved_tensors'], 2)

        # Restore tensors
        restored_tensors = chkpt_integrator.restore_tensors("test_key")
        self.assertEqual(len(restored_tensors), 2)

        print(f"Checkpointed and restored {len(restored_tensors)} tensors")

    def test_vision_memory_optimization(self):
        """Test vision encoder memory optimization"""
        optimizer = VisionEncoderMemoryOptimizer(shared_memory_per_block=48*1024)

        # Test patch processing optimization
        patch_result = optimizer.optimize_patch_processing_memory(
            batch_size=1, image_size=(224, 224), patch_size=16
        )

        self.assertIn('memory_layout', patch_result)
        self.assertIn('total_memory_mb', patch_result)
        self.assertGreater(patch_result['total_memory_mb'], 0)

        # Test convolutional memory optimization
        conv_result = optimizer.optimize_convolutional_memory((1, 3, 224, 224))

        self.assertIn('memory_layout', conv_result)
        self.assertIn('total_memory_bytes', conv_result)
        self.assertGreater(conv_result['total_memory_bytes'], 0)

        print(f"Patch processing memory: {patch_result['total_memory_mb']:.2f} MB")
        print(f"Convolutional memory: {conv_result['total_memory_bytes'] / (1024*1024):.2f} MB")

    def test_memory_manager_with_vision_optimization(self):
        """Test memory manager with vision-specific optimizations"""
        manager = MemoryManager(self.config)

        # Test that vision optimization is integrated
        self.assertIsNotNone(manager.vision_optimizer)
        self.assertIsInstance(manager.vision_optimizer, VisionEncoderMemoryOptimizer)

        # Allocate vision-specific tensors
        img_tensor = manager.allocate_tensor((1, 3, 224, 224), torch.float32)
        patch_tensor = manager.allocate_tensor((1, 196, 768), torch.float32)

        self.assertEqual(img_tensor.shape, (1, 3, 224, 224))
        self.assertEqual(patch_tensor.shape, (1, 196, 768))

        # Free tensors
        manager.free_tensor(img_tensor)
        manager.free_tensor(patch_tensor)
    ...
    def test_error_handling(self):
        ...
        self.assertIsNotNone(tensor)  # Should have fallen back to standard allocation

        # Test with negative dimensions
        tensor = manager.allocate_tensor((-1, 10), torch.float32)
