...
from src.qwen3_vl.optimization.simd_optimizations import (
    SIMDOptimizationConfig,
    AVX2OptimizedOperations,
    SSEOptimizedOperations,
    apply_simd_optimizations_to_model,
...
class TestSIMDOptimizations(unittest.TestCase):
    ...
    def test_vectorized_layer_norm(self):
        ...
        # Test basic functionality
        result = simd_ops.vectorized_layer_norm(self.test_tensor, weight, bias, 1e-5)
        ...
        native_ln = torch.layer_norm(self.test_tensor, [self.config.hidden_size], weight, bias, 1e-5)
        similarity = torch.cosine_similarity(
            result.flatten(),
            native_ln.flatten(),
            dim=0
    ...
    def test_vectorized_relu(self):
        ...
        self.assertTrue(torch.all(result >= 0 - 1e-6),  # Small epsilon for floating point precision
...
class TestModelIntegration(unittest.TestCase):
    ...
    def test_apply_simd_optimizations(self):
        ...
        optimized_model = apply_simd_optimizations_to_model(mock_model, simd_config)
