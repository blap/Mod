...
if __name__ == "__main__":
    ...
    for dir_name, files in results.items():
        ...
        for file_name, result in files.items():
            ...
            print(f"{status} {file_name}: {result.get('function_count', 0)} functions, {result.get('type_hint_count', 0)} type hints")
    ...
    print(f"Success rate: {files_with_type_hints/total_files*100:.1f}% " if total_files > 0 else "No files found")
