...
from src.inference_pio.common.unified_benchmark_discovery import UnifiedBenchmarkDiscoverer
...
def validate_benchmark_interfaces():
    ...
    try:
        ...
        benchmarks_to_test = [
            ("InferenceSpeedBenchmark", lambda: InferenceSpeedBenchmark(plugin, model_name)),
            ...
            ("BatchProcessingBenchmark", lambda: BatchProcessingBenchmark(plugin, model_name)),
            ("ModelLoadingTimeBenchmark", lambda: ModelLoadingTimeBenchmark(plugin, model_name))
        ]
...
def validate_regression_framework():
    ...
    try:
        ...
        print(f"✓ Regression detection test: regression detected = {result.regression_detected}")
        validation_results['regression_validated'] = True
        validation_results['tests_run'] = 1
        validation_results['regressions_detected'] = 1 if result.regression_detected else 0
    ...
    try:
        ...
        for fixture_name, fixture_creator in fixture_types:
            try:
                ...
                if fixture_name == "Performance":
                    ...
                    print(f"  ✓ Performance fixture generated {len(inputs)} test inputs")
                elif fixture_name == "Accuracy":
                    test_cases = list(fixture.get_test_cases())
                    print(f"  ✓ Accuracy fixture generated {len(test_cases)} test cases")
                elif fixture_name == "Memory":
                    scenarios = list(fixture.get_memory_test_scenarios())
                    print(f"  ✓ Memory fixture generated {len(scenarios)} test scenarios")
        ...
        try:
            with performance_benchmark_fixture() as perf_fixture:
                ...
                print(f"  ✓ Performance fixture context manager worked with {len(inputs)} inputs")
            
            with accuracy_benchmark_fixture() as acc_fixture:
                test_cases = list(acc_fixture.get_test_cases())
                print(f"  ✓ Accuracy fixture context manager worked with {len(test_cases)} cases")
            
            with memory_benchmark_fixture() as mem_fixture:
                scenarios = list(mem_fixture.get_memory_test_scenarios())
                print(f"  ✓ Memory fixture context manager worked with {len(scenarios)} scenarios")
...
def run_unit_tests_for_benchmarks():
    ...
    if benchmark_test_dir.exists():
        benchmark_suite = test_loader.discover(
            start_dir=str(benchmark_test_dir),
            pattern="*test*.py"
        )
