...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run inference with Inference-PIO models")
    parser.add_argument("--model", type=str, required=True, help=f"Model name (available: {', '.join(ModelFactory.list_supported_models())})")
    parser.add_argument("--prompt", type=str, required=True, help="Text prompt for generation")
    parser.add_argument("--image", type=str, help="Path to image file (for multimodal models)")
    parser.add_argument("--virtual-execution", action="store_true", help="Enable virtual execution (simulation) mode")
    ...
    try:
        ...
        if args.image:
            if "vl" not in args.model.lower():
                logger.warning(f"Image provided but model {args.model} might not be multimodal.")
        ...
        if args.virtual_execution:
            if hasattr(plugin, 'execute_with_distributed_simulation'):
                ...
                logger.warning(f"Plugin {args.model} does not support virtual execution explicitly, using infer()")
            ...
            if isinstance(input_data, dict) and "vl" in args.model.lower():
                ...
                if hasattr(plugin, 'generate_text'):
                    result = plugin.generate_text(input_data, max_new_tokens=args.max_tokens)
                else:
                    result = plugin.infer(input_data)
            else:
                result = plugin.infer(input_data)
