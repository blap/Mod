...
def run_pytest_with_reporting(
    pytest_args: List[str], reports_dir: str = "test_reports"
) -> Dict[str, Any]:
    """
...
"""
    ...
    failed_tests = [
        tc
        for tc in summary.get("test_cases", [])
    ...
    if xml_report.exists():
        ...
        if os.getenv("GITHUB_ACTIONS"):
            ...
            with open(os.environ.get("GITHUB_STEP_SUMMARY", "/dev/null"), "a") as f:
                ...
                f.write(
                    f"- Passed: {summary['tests'] - summary['failures'] - summary['errors'] - summary['skipped']}\n"
                )
            ...
            with open(os.environ.get("GITHUB_OUTPUT", "/dev/null"), "a") as f:
                ...
                f.write(
                    f"passed_tests={summary['tests'] - summary['failures'] - summary['errors'] - summary['skipped']}\n"
                )
                f.write(f"failed_tests={summary['failures']}\n")
                f.write(f"errors={summary['errors']}\n")
                f.write(
                    f"has_failures={'true' if summary['failures'] > 0 else 'false'}\n"
                )
...
def main():
    ...
    parser.add_argument(
        "--reports-dir", default="test_reports", help="Directory to store test reports"
    )
    parser.add_argument(
        "--no-run-tests",
        action="store_true",
