...
class IntelCometLakePlugin(HardwareProcessorPluginInterface):
    ...
    def initialize(self, config: Dict[str, Any]) -> bool:
        ...
        if config.get("enable_ipex", False) and _IPEX_AVAILABLE:
            ...
            logger.warning(
                "IPEX enabled in config but not installed. Falling back to MKL."
            )
    ...
    def get_layer_distribution(
        ...
        layer_size_gb = (
            estimated_model_size_gb / total_layers if total_layers > 0 else 0
        )
        ...
        if (
            layers_on_gpu == 0 and safe_vram > 0.3
        ):  # If we have at least 300MB, maybe fit 1 layer?
        ...
        logger.info(
            f"Hybrid Plan: {layers_on_gpu} layers on GPU, {layers_on_cpu} layers on CPU "
