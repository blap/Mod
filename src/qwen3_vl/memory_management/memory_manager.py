...
4. Tensor allocation patterns optimized for Intel i5-10210U + NVIDIA SM61 + NVMe SSD
5. Integration with gradient checkpointing system
6. Memory layout optimization for vision encoder operations
7. Hardware-specific memory access patterns
8. Error handling and validation for memory operations
"""
...
class BuddyAllocator:
    ...
    def __init__(self, initial_size: int):
        ...
        # Calculate the number of orders needed (max order for initial_size)
        self.max_order = int(math.log2(initial_size))
        
        # Initialize free lists for each order (size = 2^order)
        self.free_blocks = defaultdict(list)  # {order: [start_address]}
        
        # Initially, the entire pool is one free block
        self.free_blocks[self.max_order].append(0)
    ...
    def allocate(self, size: int) -> Optional[int]:
        ...
        for o in range(order, self.max_order + 1):
            if self.free_blocks[o]:
                # Found a block, allocate it
                addr = self.free_blocks[o].pop()
                
                # Split if the block is larger than needed
                while o > order:
                    o -= 1
                    buddy_addr = addr ^ (1 << o)  # Calculate buddy address
                    self.free_blocks[o].append(buddy_addr)
                
                actual_size = 1 << o  # Correct size after potential splitting
                
                # Record allocation
                self.allocated_blocks[addr] = (actual_size, o)
    ...
    def deallocate(self, addr: int) -> bool:
        ...
        while current_order < self.max_order:
            ...
            # Check if buddy is in the same order free list
            if buddy_addr in self.free_blocks[current_order]:
                ...
                # Buddy not free, can't merge anymore
                break
        
        # Add merged block to free list
        self.free_blocks[current_order].append(current_addr)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        allocated_memory = self.stats['total_allocated'] - self.stats['internal_fragmentation']
...
class TensorCache:
    ...
    def __init__(self, max_cache_size: int = 100):
        ...
        self.cache: Dict[Tuple[Tuple[int, ...], torch.dtype, str], List[torch.Tensor]] = defaultdict(list)  # {(shape, dtype, device): [tensor, ...]}
        self.stats = {
            'hits': 0,
            'misses': 0,
            'total_requests': 0,
...
class MemoryDefragmenter:
    """
    Memory defragmentation routines to reduce memory fragmentation
    """
    ...
    def defragment_memory(self) -> Dict:
        ...
        # Also clear our tensor cache to prevent memory bloat
        cache = self.memory_pool.tensor_cache
        original_size = cache.stats['cached_tensors']
        cache.clear_cache()
        final_size = cache.stats['cached_tensors']
        ...
        return {
            ...
            'cached_tensors_cleared': original_size - final_size,
            'success': True
        }
...
class MemoryPool:
    ...
    """
    def __init__(self, initial_size: int = 2**30, use_tensor_caching: bool = True):  # 1GB default
        ...
        self.allocated_tensors: Dict[int, Tuple[Optional[int], Tuple[int, ...], torch.dtype, torch.device]] = {}  # {id(tensor): (address, shape, dtype, device)}
    ...
    def deallocate_tensor(self, tensor: torch.Tensor) -> bool:
        ...
        if tensor_id in self.allocated_tensors:
            # Get tensor metadata before deleting from allocated tensors
            tensor_meta = self.allocated_tensors.get(tensor_id)
            ...
            is_common_shape = any(
                key[0] == common_shape and key[1] == common_dtype
                for common_shape, common_dtype in self.common_shapes
            )
...
if __name__ == "__main__":
    ...
    config = MemoryConfig(memory_pool_size=2**28)  # 256MB for testing
    ...
    retrieved_tensor = cache.get_tensor((10, 20), torch.float32)
    ...
    global_tensor = allocate_tensor_with_manager((50, 50))
