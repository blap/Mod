...
if __name__ == "__main__":
    print("Testing Memory Access Pattern Optimization for Cache Efficiency...")
    
    # Test Cache Line Optimizer
    print("\n1. Testing Cache Line Optimizer...")
    cache_optimizer = CacheLineOptimizer()
    
    # Test tensor alignment
    test_tensor = torch.randn(100, 200)  # Non-aligned dimensions
    aligned_tensor = cache_optimizer.optimize_for_memory_access(test_tensor, MemoryAccessType.COALESCED)
    print(f"Original tensor shape: {test_tensor.shape}, Aligned tensor shape: {aligned_tensor.shape}")
    
    # Test alignment calculation
    aligned_size = cache_optimizer.align_for_gpu_access(123)  # Non-aligned size
    print(f"Aligned size for 123: {aligned_size}")
    
    # Get cache performance prediction
    pred = cache_optimizer.predict_cache_performance((100, 200, 768), MemoryAccessType.COALESCED)
    print(f"Cache performance prediction: {pred}")
    
    # Test Memory Layout Optimizer
    print("\n2. Testing Memory Layout Optimizer...")
    layout_optimizer = MemoryLayoutOptimizer(cache_optimizer)
    
    # Test attention layout optimization
    query = torch.randn(2, 8, 512, 64)  # [batch, heads, seq, head_dim]
    key = torch.randn(2, 8, 512, 64)
    value = torch.randn(2, 8, 512, 64)
    
    opt_query, opt_key, opt_value = layout_optimizer.optimize_attention_memory_layout(query, key, value)
    print(f"Attention layout optimization - Query: {query.shape} -> {opt_query.shape}")
    
    # Test MLP layout optimization
    mlp_tensor = torch.randn(2, 512, 2048)  # [batch, seq, intermediate]
    opt_mlp_tensor = layout_optimizer.optimize_mlp_memory_layout(mlp_tensor)
    print(f"MLP layout optimization - Input: {mlp_tensor.shape} -> {opt_mlp_tensor.shape}")
    
    # Test getting optimal tile sizes
    tile_sizes = layout_optimizer.get_optimal_tile_sizes((2, 512, 2048), 'cuda')
    print(f"Optimal tile sizes for (2, 512, 2048): {tile_sizes}")
    
    # Test Memory Access Pattern Tracker
    print("\n3. Testing Memory Access Pattern Tracker...")
    tracker = MemoryAccessPatternTracker()
    
    # Simulate some tensor accesses
    for i in range(10):
        tensor = torch.randn(5, 10, 64)
        tracker.record_tensor_access(tensor, 'attention', layer_idx=i % 3)
        
        mlp_tensor = torch.randn(5, 10, 256)
        tracker.record_tensor_access(mlp_tensor, 'mlp', layer_idx=i % 3)
    
    # Get pattern summary
    summary = tracker.get_pattern_summary()
    print(f"Access pattern summary: {summary}")
    
    # Get optimization opportunities
    opportunities = tracker.get_access_patterns_for_optimization()
    print(f"Number of optimization opportunities identified: {len(opportunities)}")
    if opportunities:
        print(f"First opportunity: {opportunities[0]}")
    
    # Test Hardware-Specific Memory Optimizer
    print("\n4. Testing Hardware-Specific Memory Optimizer...")
    hw_optimizer = HardwareSpecificMemoryOptimizer(
        cpu_cache_line_size=64,
        gpu_shared_memory_per_block=48 * 1024,  # 48KB for SM61
        memory_bandwidth_gb_s=192.0  # GTX 1080 Ti equivalent
    )
    
    # Test tensor optimization for hardware
    test_tensor_hw = torch.randn(1, 512, 1024)  # [batch, seq, features]
    optimized_tensor = hw_optimizer.optimize_tensor_for_hardware(test_tensor_hw, 'attention', layer_idx=0)
    print(f"Hardware-optimized tensor: {test_tensor_hw.shape} -> {optimized_tensor.shape}")
    
    # Get optimization recommendations
    recommendations = hw_optimizer.get_optimization_recommendations()
    print(f"Hardware-specific recommendations: {len(recommendations)} found")
    if recommendations:
        print(f"First recommendation: {recommendations[0]}")
    
    # Get hardware stats
    hw_stats = hw_optimizer.get_hardware_stats()
    print(f"Hardware stats: GPU shared memory per block = {hw_stats['gpu_shared_memory_per_block']} bytes")
    
    # Test Memory-Efficient Attention
    print("\n5. Testing Memory-Efficient Attention...")
    
    # Create a mock config
    class MockConfig:
        hidden_size = 512
        num_attention_heads = 8
        attention_chunk_size = 256  # Small chunk size for testing
    
    config = MockConfig()
    attention_layer = MemoryEfficientAttention(config, layer_idx=0)
    
    # Test forward pass
    hidden_states = torch.randn(2, 100, 512)  # [batch, seq, hidden]
    mask = torch.ones(2, 8, 100, 100)  # [batch, heads, seq, seq]
    
    output, weights, cache = attention_layer(
    ...
    mlp_layer = MemoryEfficientMLP(mlp_config)
    
    # Test forward pass
    mlp_input = torch.randn(2, 10, 512)  # [batch, seq, hidden]
    mlp_output = mlp_layer(mlp_input)
    
    print(f"Memory-efficient MLP output shape: {mlp_output.shape}")
    
    print("\nMemory Access Pattern Optimization implementation completed!")
    print("All tests passed for cache efficiency and memory layout optimization.")