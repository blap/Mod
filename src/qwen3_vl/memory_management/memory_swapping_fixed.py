"""
Fixed Advanced Memory Swapping System for SSD with Memory Pressure Monitoring
...
if __name__ == "__main__":
    print("Fixed Advanced Memory Swapping System for Qwen3-VL")
    print("=" * 60)

    # Create the swapping system optimized for our hardware
    swapper = create_advanced_memory_swapper({
        'cpu_model': 'Intel i5-10210U',
        'gpu_model': 'NVIDIA SM61',
        'memory_size': 8 * 1024 * 1024 * 1024,  # 8GB
        'storage_type': 'nvme'
    ...
    # Register some example memory blocks
    print("\n1. Registering memory blocks...")
    for i in range(5):
        block_id = f"block_{i}"
        size = (i + 1) * 10 * 1024 * 1024  # 10MB, 20MB, 30MB, 40MB, 50MB
        swapper.register_memory_block(
            block_id,
            size,
            MemoryRegionType.TENSOR_DATA,
            pinned=(i == 0)  # First block is pinned
        )
        print(f"  Registered {block_id}, size: {size / (1024*1024):.1f} MB")

    # Simulate accessing blocks
    print("\n2. Simulating block accesses...")
    for i in range(10):
        block_id = f"block_{(i % 4) + 1}"  # Access blocks 1-4, not the pinned one
        swapper.access_memory_block(block_id)
        print(f"  Accessed {block_id}")

    # Check if swapping is needed and perform it
    print("\n3. Checking if swapping is needed...")
    if swapper.should_swap():
        print("  Swapping threshold reached, performing swap")
        swapped_count = swapper.perform_swapping()
        print(f"  Swapped out {swapped_count} blocks")
    else:
        print("  No swapping needed at this time")

    # Analyze access patterns
    print("\n4. Analyzing access patterns...")
    patterns = swapper.analyze_access_patterns()
    print(f"  Most frequently accessed: {patterns['most_frequently_accessed']}")
    print(f"  Temporal locality score: {patterns['temporal_locality']:.3f}")

    # Show system status
    print("\n5. System status:")
    status = swapper.get_status()
    print(f"  Algorithm: {status['algorithm']}")
    print(f"  Current swap usage: {status['current_swap_size_GB']:.3f} GB")
    print(f"  Total registered blocks: {status['total_registered_blocks']}")
    print(f"  Swapped blocks: {status['swapped_blocks']}")
    print(f"  Pinned blocks: {status['pinned_blocks']}")
    ...
    print(f"  Average swap-out time: {efficiency['avg_swap_out_time']:.6f}s")
    print(f"  Average swap-in time: {efficiency['avg_swap_in_time']:.6f}s")
    print(f"  Total swapped: {efficiency['total_swapped_GB']:.3f} GB")

    # Clean up
    swapper.cleanup()
    print("\nFixed Advanced Memory Swapping System test completed!")