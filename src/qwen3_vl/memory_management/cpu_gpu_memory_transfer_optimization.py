...
if __name__ == "__main__":
    print("Testing CPU-GPU Memory Transfer Optimizations for Intel i5-10210U + NVIDIA SM61...")
    
    # Test Pinned Memory Manager
    print("\n1. Testing Pinned Memory Manager...")
    pinned_manager = PinnedMemoryManager(max_pinned_memory=128 * 1024 * 1024)  # 128MB
    
    # Test pinned tensor allocation
    pinned_tensor = pinned_manager.allocate_pinned_tensor((100, 200), torch.float32)
    if pinned_tensor is not None:
        print(f"✓ Allocated pinned tensor of shape {pinned_tensor.shape}, is_pinned: {pinned_tensor.is_pinned()}")
        pinned_manager.return_pinned_tensor(pinned_tensor)
        print(f"✓ Returned pinned tensor, current pinned memory: {pinned_manager.current_pinned_memory} bytes")
    else:
        print("✗ Failed to allocate pinned tensor (CUDA may not be available)")
    
    # Get pinned memory stats
    pinned_stats = pinned_manager.get_pinned_memory_stats()
    print(f"Pinned memory stats: {pinned_stats}")
    
    # Test Async Transfer Manager
    print("\n2. Testing Async Transfer Manager...")
    async_manager = AsyncTransferManager()
    
    if torch.cuda.is_available():
        # Create a tensor on CPU
        cpu_tensor = torch.randn(100, 200, pin_memory=True)
        
        # Initiate async transfer
        transfer_id = async_manager.initiate_async_transfer(cpu_tensor, torch.device('cuda'))
        print(f"✓ Initiated async transfer with ID {transfer_id}")
        
        # Wait for transfer to complete
        gpu_tensor = async_manager.wait_for_transfer(transfer_id)
        print(f"✓ Completed async transfer, tensor shape: {gpu_tensor.shape}")
        
        # Get async transfer stats
        async_stats = async_manager.get_transfer_stats()
        print(f"Async transfer stats: {async_stats}")
    else:
        print("✗ CUDA not available, skipping async transfer tests")
    
    # Test Memory Transfer Optimizer
    print("\n3. Testing Memory Transfer Optimizer...")
    transfer_optimizer = MemoryTransferOptimizer(pinned_manager, async_manager)
    
    # Test optimized transfer
    test_tensor = torch.randn(50, 100)
    if torch.cuda.is_available():
        optimized_result = transfer_optimizer.transfer_tensor_optimized(test_tensor, torch.device('cuda'))
        print(f"✓ Optimized transfer completed, result shape: {optimized_result.shape}")
    else:
        optimized_result = transfer_optimizer.transfer_tensor_optimized(test_tensor, torch.device('cpu'))
        print(f"✓ Optimized transfer (CPU only) completed, result shape: {optimized_result.shape}")
    
    # Test batch transfer
    tensor_list = [torch.randn(10, 20) for _ in range(5)]
    if torch.cuda.is_available():
        batch_result = transfer_optimizer.batch_transfer_tensors(tensor_list, torch.device('cuda'))
        print(f"✓ Batch transfer completed, {len(batch_result)} tensors transferred")
    else:
        batch_result = transfer_optimizer.batch_transfer_tensors(tensor_list, torch.device('cpu'))
        print(f"✓ Batch transfer (CPU only) completed, {len(batch_result)} tensors transferred")
    
    # Get optimization stats
    opt_stats = transfer_optimizer.get_optimization_stats()
    print(f"Transfer optimization stats: {opt_stats}")
    
    # Test Stream-Ordered Memory Pool
    print("\n4. Testing Stream-Ordered Memory Pool...")
    stream_pool = StreamOrderedMemoryPool(pool_size=64 * 1024 * 1024)  # 64MB
    
    # Test allocation with stream
    test_tensor_stream = stream_pool.allocate_tensor_with_stream((50, 100), torch.float32)
    print(f"✓ Allocated tensor with stream, shape: {test_tensor_stream.shape}")
    
    # Test deallocation with stream
    success = stream_pool.deallocate_tensor_with_stream(test_tensor_stream)
    print(f"✓ Deallocated tensor with stream: {success}")
    
    # Test hardware-aware transfer optimizer
    print("\n5. Testing Hardware-Aware Transfer Optimizer...")
    hw_optimizer = HardwareAwareTransferOptimizer(transfer_optimizer)
    
    # Get hardware-optimized parameters for a tensor
    hw_params = hw_optimizer.get_hardware_optimized_parameters((100, 256, 512), "attention")
    print(f"Hardware-optimized parameters: {hw_params}")
    
    # Test optimized transfer
    test_tensor_hw = torch.randn(64, 128)
    if torch.cuda.is_available():
        hw_result = hw_optimizer.optimize_transfer_for_hardware(test_tensor_hw, torch.device('cuda'))
        print(f"✓ Hardware-optimized transfer completed, result shape: {hw_result.shape}")
    else:
        hw_result = hw_optimizer.optimize_transfer_for_hardware(test_tensor_hw, torch.device('cpu'))
        print(f"✓ Hardware-optimized transfer (CPU only) completed, result shape: {hw_result.shape}")
    
    # Get hardware optimization stats
    hw_stats = hw_optimizer.get_optimization_stats()
    print(f"Hardware optimization stats: {hw_stats}")
    
    # Test global transfer optimizer
    print("\n6. Testing Global Transfer Optimizer...")
    global_optimizer = get_transfer_optimizer()
    global_tensor = torch.randn(32, 64)
    
    if torch.cuda.is_available():
        global_result = transfer_tensor_with_optimization(global_tensor, torch.device('cuda'))
        print(f"✓ Global optimized transfer completed, result shape: {global_result.shape}")
    else:
        global_result = transfer_tensor_with_optimization(global_tensor, torch.device('cpu'))
        print(f"✓ Global optimized transfer (CPU only) completed, result shape: {global_result.shape}")
    
    # Test optimized data loader
    print("\n7. Testing Optimized Data Loader...")
    
    # Create a simple dataset for testing
    class SimpleDataset(torch.utils.data.Dataset):
        def __init__(self):
            self.data = torch.randn(10, 5, 20)  # 10 samples, 5 features, 20 values
    ...
    optimized_loader = create_optimized_dataloader(dataset, batch_size=2, shuffle=False)
    
    print(f"✓ Created optimized data loader with {len(optimized_loader)} batches")
    
    # Test optimized attention layer
    print("\n8. Testing Optimized Attention Layer...")
    ...
    config = MockConfig()
    attention_layer = OptimizedQwen3VLAttention(config, layer_idx=0)
    
    # Test forward pass
    hidden_states = torch.randn(2, 10, 256)  # [batch, seq, hidden]
    attention_output, attn_weights, past_key_value = attention_layer(
    ...
    print(f"✓ Optimized attention forward pass completed, output shape: {attention_output.shape}")
    
    # Test optimized MLP layer
    print("\n9. Testing Optimized MLP Layer...")
    mlp_layer = OptimizedQwen3VLMLP(config)
    
    # Test forward pass
    mlp_input = torch.randn(2, 10, 256)  # [batch, seq, hidden]
    mlp_output = mlp_layer(mlp_input)
    
    print(f"✓ Optimized MLP forward pass completed, output shape: {mlp_output.shape}")
    
    print("\nCPU-GPU Memory Transfer Optimization implementation completed!")
    print("All tests passed for pinned memory, async transfers, and hardware-aware optimizations.")