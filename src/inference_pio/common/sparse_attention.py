...
class SparseAttention(nn.Module):
    ...
    def _create_block_sparse_mask(self, seq_len: int, device: torch.device) -> torch.Tensor:
        ...
        for i in range(seq_len):
            ...
            for j in range(max(0, block_idx - num_connected_blocks//2), 
    ...
    def _create_random_sparse_mask(self, seq_len: int, device: torch.device) -> torch.Tensor:
        """
...
def create_sparse_attention(config: Any, layer_idx: Optional[int] = None, sparse_pattern: str = "longformer"):
    """
