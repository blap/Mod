...
class EfficientMultimodalCrossAttention(nn.Module):
    ...
    def forward(
        ...
        for query_modality, query in queries.items():
            ...
            q = q.view(query.size(0), query.size(1), self.nhead, self.head_dim).transpose(1, 2)  # (bsz, nhead, seq_len, head_dim)
            q = q * self.scaling
            ...
            for key_modality, key in keys.items():
                ...
                k = k.view(key.size(0), key.size(1), self.nhead, self.head_dim).transpose(1, 2)  # (bsz, nhead, seq_len, head_dim)
                all_keys.append(k)

                # Project value
                v = self.modality_projections[key_modality]['v'](key)
                v = v.view(key.size(0), key.size(1), self.nhead, self.head_dim).transpose(1, 2)  # (bsz, nhead, seq_len, head_dim)
                all_values.append(v)

            # Concatenate keys and values across modalities
            concat_k = torch.cat(all_keys, dim=2)  # (bsz, nhead, total_seq_len, head_dim)
            concat_v = torch.cat(all_values, dim=2)  # (bsz, nhead, total_seq_len, head_dim)

            # Compute attention scores
            if self.use_flash_attention and torch.cuda.is_available():
                # Use efficient attention computation
                attn_weights = torch.matmul(q, concat_k.transpose(-1, -2))  # (bsz, nhead, query_seq_len, total_key_seq_len)
                ...
                if attention_masks is not None and query_modality in attention_masks:
                    ...
                    if mask.dim() == 2:  # (seq_len, seq_len) -> (bsz, nhead, seq_len, total_seq_len)
                        mask = mask.unsqueeze(0).unsqueeze(0).expand(-1, self.nhead, -1, -1)
                    elif mask.dim() == 3:  # (bsz, seq_len, seq_len) -> (bsz, nhead, seq_len, total_seq_len)
                ...
                attn_output = torch.matmul(attn_weights, concat_v)  # (bsz, nhead, query_seq_len, head_dim)
            else:
                attn_weights = torch.matmul(q, concat_k.transpose(-1, -2))  # (bsz, nhead, query_seq_len, total_key_seq_len)
                ...
                if self.is_causal:
                    causal_mask = torch.triu(torch.ones(query.size(1), concat_k.size(2), dtype=torch.bool, device=query.device), diagonal=1)
    ...
    def _apply_sparse_attention(self, attn_weights: torch.Tensor) -> torch.Tensor:
        ...
        topk_values, topk_indices = torch.topk(attn_weights, k=min(self.sparse_topk, attn_weights.size(-1)), dim=-1)
        ...
        sparse_attn = torch.softmax(sparse_attn, dim=-1, dtype=torch.float32).to(attn_weights.dtype)
...
class ModalitySpecificAttention(nn.Module):
    ...
    def forward(
        ...
        k = k.view(bsz, src_len, self.nhead, self.head_dim).transpose(1, 2)  # (bsz, nhead, src_len, head_dim)
        v = v.view(bsz, src_len, self.nhead, self.head_dim).transpose(1, 2)  # (bsz, nhead, src_len, head_dim)

        # Scale query
        q = q * self.scaling

        # Compute attention scores
        attn_weights = torch.matmul(q, k.transpose(-2, -1))  # (bsz, nhead, tgt_len, src_len)
        ...
        # Apply causal mask if needed
        if self.is_causal:
            causal_mask = torch.triu(torch.ones(tgt_len, src_len, dtype=torch.bool, device=query.device), diagonal=1)
            attn_weights.masked_fill_(causal_mask, float("-inf"))
    ...
    def __init__(
        ...
        if adaptive_strategy == "input_dependent":
            ...
            self.adaptation_network = nn.Sequential(
                nn.Linear(d_model, d_model // 4),
                nn.ReLU(),
                nn.Linear(d_model // 4, 4),  # Predict 4 parameters: sparsity, temperature, dropout, window_size
    ...
    def forward(
        ...
        if self.adaptive_strategy == "input_dependent":
            ...
            adaptive_params = self.adaptation_network(
                torch.mean(representative_input, dim=1)  # Average across sequence dimension
            )  # Shape: (batch_size, 4)
            ...
            if attention_weights is not None:
                for modality, weights in attention_weights.items():
                    ...
                    temp = adaptive_params[:, 0].unsqueeze(1).unsqueeze(2).unsqueeze(3) + 1e-8  # Prevent division by zero
    ...
    def forward(
        ...
        if self.alignment_method == "learned_projection":
            ...
            for modality, representation in modalities_dict.items():
                aligned_outputs[modality] = self.projection_layers[modality](representation)
...
def create_multimodal_attention(config: Any, layer_idx: Optional[int] = None) -> EfficientMultimodalCrossAttention:
    """
