...
def create_unimodal_cuda_kernels(d_model: int,
                               nhead: int,
                               intermediate_size: int,
                               use_flash_attention: bool = True,
                               use_swiglu: bool = True) -> Dict[str, nn.Module]:
    """
    ...
    kernels['layernorm'] = UnimodalLayerNormKernel(
        normalized_shape=d_model
    )

    # Create unimodal RMSNorm kernel
    kernels['rmsnorm'] = UnimodalRMSNormKernel(
        normalized_shape=d_model
    )
...
def apply_unimodal_cuda_optimizations_to_model(model: nn.Module,
                                             d_model: int,
                                             nhead: int,
                                             intermediate_size: int,
                                             config: Any = None,
                                             model_type: str = "general") -> nn.Module:
    """
    ...
    use_swiglu = getattr(config, 'use_swiglu', False) if config else False  # Model-specific config should define this
    ...
    for name, module in model.named_modules():
        ...
        # Replace LayerNorm modules
            ...
            use_rms_norm = getattr(config, 'use_rms_norm', False)  # Model-specific config should define this
            if use_rms_norm:
...
def get_unimodal_cuda_optimization_report(model: nn.Module,
                                        d_model: int,
                                        nhead: int,
                                        intermediate_size: int,
                                        config: Any = None,
                                        model_type: str = "general") -> Dict:
    """
