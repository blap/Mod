...
class FeedbackController:
    ...
    def record_metrics(self, model_id: str, metrics: PerformanceMetrics):
        ...
        # Add to event queue
        event = FeedbackEvent(
            event_type=FeedbackEventType.ACCURACY_UPDATE if metrics.accuracy > 0 else FeedbackEventType.LATENCY_UPDATE,
            model_id=model_id,
            timestamp=time.time(),
            metrics={
    ...
    def add_adjustment_callback(self, model_id: str, callback: Callable[[OptimizationAdjustment], None]):
        """Add a callback to handle optimization adjustments for a specific model."""
        self.adjustment_callbacks[model_id].append(callback)
        
    def remove_adjustment_callback(self, model_id: str, callback: Callable[[OptimizationAdjustment], None]):
    ...
    def set_performance_target(self, metric_name: str, target_value: float, weight: float = 1.0):
    ...
    def get_historical_metrics(self, model_id: str, count: int = 10) -> List[PerformanceMetrics]:
    ...
    def _evaluate_performance_and_adjust(self, model_id: str) -> Optional[OptimizationAdjustment]:
        ...
        # Calculate trends
        avg_accuracy = np.mean([m.accuracy for m in history[-10:]]) if len(history) >= 10 else current.accuracy
        avg_latency = np.mean([m.latency for m in history[-10:]]) if len(history) >= 10 else current.latency
        
        # Check if we need to adjust based on targets
        accuracy_deviation = abs(avg_accuracy - self.performance_targets["accuracy"]["target"])
        latency_deviation = abs(avg_latency - self.performance_targets["latency"]["target"])
        
        # Determine if adjustment is needed
        needs_adjustment = (
            accuracy_deviation > self.adjustment_threshold or
            latency_deviation > self.adjustment_threshold
        )
        ...
        accuracy_trend_stable = self._calculate_trend_stability([m.accuracy for m in history])
        latency_trend_stable = self._calculate_trend_stability([m.latency for m in history])
    ...
    def _apply_adjustment(self, model_id: str, adjustment: OptimizationAdjustment):
        ...
        self.logger.info(f"Applying adjustment to {model_id}: {adjustment.strategy} - {adjustment.reason}")
        ...
        for callback in self.adjustment_callbacks[model_id]:
            try:
                ...
                self.logger.error(f"Error in adjustment callback for {model_id}: {str(e)}")
        ...
        event = FeedbackEvent(
            ...
            model_id=model_id,
            timestamp=time.time(),
            metrics={"strategy": adjustment.strategy, "parameters": adjustment.parameters},
    ...
    def _process_event(self, event: FeedbackEvent):
        ...
        # Log the event
        self.logger.debug(f"Processing event: {event.event_type.value} for {event.model_id}")
