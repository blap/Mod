...
class NASConfig:
    ...
    strategy: ArchitectureAdaptationStrategy = ArchitectureAdaptationStrategy.COMBINED_ADAPTIVE
    ...
    accuracy_tradeoff_factor: float = 0.7  # Trade-off factor between accuracy and speed (0-1)
...
class ContinuousNASController:
    ...
    def __init__(self, config: NASConfig):
        ...
        logger.info(f"Initialized Continuous NAS Controller with strategy: {strategy_str}")
    ...
    def adapt_architecture(
        ...
        complexity_metrics = self.complexity_analyzer.analyze_input_complexity(input_data)
        ...
        if hasattr(complexity_metrics, 'complexity_score'):
            ...
            if isinstance(complexity_metrics, dict) and 'complexity_score' in complexity_metrics:
                input_complexity = complexity_metrics['complexity_score']
            else:
        ...
        should_adapt = self._should_adapt(input_complexity, current_latency, current_memory)
    ...
    def _should_adapt(self, complexity: float, latency: float, memory: float) -> bool:
        ...
        if len(self.performance_history) >= 2:
            recent_complexities = [m.input_complexity for m in self.performance_history[-5:]]
            if len(recent_complexities) > 1:
                complexity_change = abs(recent_complexities[-1] - recent_complexities[-2])
    ...
    def _calculate_adaptation_ratios(
        ...
        if strategy_value == ArchitectureAdaptationStrategy.DEPTH_ADAPTIVE.value:
            ...
            latency_factor = max(0.8, 1.0 - (latency / self.config.latency_target_ms - 1.0) * 0.5)
            memory_factor = max(0.8, 1.0 - (memory / self.config.memory_budget_mb - 1.0) * 0.5)

            target_depth *= min(latency_factor, memory_factor)
            target_width *= min(latency_factor, memory_factor)

            # Clamp to bounds
            target_depth = max(min(target_depth, self.config.max_depth_ratio), self.config.min_depth_ratio)
            target_width = max(min(target_width, self.config.max_width_ratio), self.config.min_width_ratio)
    ...
    def _apply_architecture_changes(
        ...
        logger.debug(f"Applying architecture changes - Depth: {depth_ratio}, Width: {width_ratio}")
    ...
    def _estimate_current_latency(self) -> float:
        ...
        recent_latencies = [m.processing_time_ms for m in self.performance_history[-10:]]
