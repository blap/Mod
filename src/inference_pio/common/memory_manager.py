...
class MemoryManager:
    ...
    def __init__(self,
        ...
        logger.info(f"Memory manager initialized with max_memory_ratio={max_memory_ratio}, "
    ...
    def allocate_page(self, tensor: Tensor, page_id: str, priority: MemoryPriority = MemoryPriority.MEDIUM) -> bool:
        """
    ...
    def _handle_memory_pressure(self):
        ...
        for page_id, page in pages_to_consider:
            if not self.is_memory_pressure_high() or swaps_performed >= max_swaps_per_call:
                break
    ...
    def _perform_proactive_management(self):
        ...
        with self.lock:
            ...
            if predicted_usage_ratio > self.max_memory_ratio * 0.9:  # 90% of threshold
                logger.info(f"Predicted high memory usage ({predicted_usage_ratio:.2%}), performing proactive swaps")
    ...
    def cleanup(self):
        ...
        with self.lock:
            ...
            try:
                ...
                logger.error(f"Failed to clean up swap directory {self.swap_directory}: {e}")
...
class TensorPagingManager:
    ...
    def page_tensor(self, tensor: Tensor, tensor_id: str, priority: MemoryPriority = MemoryPriority.MEDIUM) -> bool:
        """
...
def create_memory_manager(max_memory_ratio: float = 0.8,
                         swap_directory: Optional[str] = None,
                         page_size_mb: int = 16,
                         eviction_policy: str = "lru") -> MemoryManager:
    """
