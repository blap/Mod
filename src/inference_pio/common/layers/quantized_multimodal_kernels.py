...
class GenericQuantizedVisionLanguageAttentionKernel(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            lang_to_vision_attn = (
                lang_to_vision_attn + attention_mask.transpose(-1, -2)
                if attention_mask.dim() > 2
                else lang_to_vision_attn + attention_mask
            )
...
def create_generic_quantized_cross_attention_kernel(
    config: GenericQuantizedMultimodalConfig, layer_idx: int = 0
):
    """
...
def create_generic_quantized_fusion_kernel(
    config: GenericQuantizedMultimodalConfig, layer_idx: int = 0
):
    """
...
def create_generic_quantized_vision_language_attention_kernel(
    config: GenericQuantizedMultimodalConfig, layer_idx: int = 0
):
    """
...
def create_generic_quantized_position_encoding_kernel(
    config: GenericQuantizedMultimodalConfig,
):
    """
...
def apply_generic_quantized_cuda_optimizations_to_model(
    model: nn.Module, config: GenericQuantizedMultimodalConfig
) -> nn.Module:
    """
...
def get_generic_quantized_optimization_report(
    model: nn.Module, config: GenericQuantizedMultimodalConfig
) -> Dict:
    """
