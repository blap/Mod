...
class GenericVisionPatchEmbeddingKernel(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        if isinstance(self.projection, nn.Conv2d):
            ...
            patches = (
                pixel_values.unfold(2, self.patch_size, self.patch_size)
                .unfold(3, self.patch_size, self.patch_size)
                .flatten(2, 3)
            )  # (batch_size, channels, grid_h, grid_w, patch_h*patch_w)
            patches = patches.transpose(1, 2).transpose(
                2, 3
            )  # (batch_size, grid_h, grid_w, channels*patch_h*patch_w)
...
class GenericVisionSelfAttentionKernel(nn.Module):
    ...
    def __init__(self, config: VisionTransformerConfig):
        ...
        if self.use_sparse_attention:
            self.register_buffer(
                "sparse_attention_mask", self._create_sparse_attention_mask()
            )
    ...
    def _create_sparse_attention_mask(self) -> torch.Tensor:
        ...
        mask = (
            torch.rand(self.num_attention_heads, self.max_seq_len, self.max_seq_len)
            < self.sparse_attention_density
        )
        return mask.float()
    ...
    def forward(
        ...
        if self.use_sparse_attention:
            ...
            sparse_mask = self.sparse_attention_mask[:, :seq_len, :seq_len].unsqueeze(
                0
    ...
    def __init__(self, config: VisionTransformerConfig):
        ...
        vision_config = VisionTransformerConfig(
            ...
            num_attention_heads=getattr(
                config, "vision_num_attention_heads", config.num_attention_heads
            ),
            num_hidden_layers=getattr(
                config, "vision_num_hidden_layers", config.num_hidden_layers
            ),
            ...
            intermediate_size=getattr(
                config, "vision_intermediate_size", config.intermediate_size
            ),
            layer_norm_eps=getattr(
                config, "vision_layer_norm_eps", config.layer_norm_eps
            ),
            use_flash_attention=getattr(
                config, "use_vision_flash_attention", config.use_flash_attention
            ),
            use_cuda_kernels=getattr(
                config, "use_cuda_kernels", config.use_cuda_kernels
            ),
        )
...
def create_generic_vision_patch_embedding_kernel(
...
def create_generic_vision_self_attention_kernel(
    config: VisionTransformerConfig,
...
def create_generic_vision_mlp_kernel(
    config: VisionTransformerConfig,
...
def create_generic_vision_transformer_block_kernel(
...
def create_generic_vision_encoder_kernel(
    config: VisionTransformerConfig,
...
def apply_generic_vision_cuda_optimizations_to_model(
    ...
    for name, module in model.named_modules():
        if (
            ...
            if (
                ...
                vision_patch_embed = create_generic_vision_patch_embedding_kernel(
                    config
                )
