...
class TensorCompressor:
    ...
    def __init__(self, 
        ...
        logger.info(f"TensorCompressor initialized with method: {compression_method}, "
                   f"ratio: {compression_ratio}, max_components: {max_components}")

    def compress_tensor(self, 
                       tensor: torch.Tensor, 
                       tensor_id: str = "default") -> Tuple[torch.Tensor, Dict[str, Any]]:
        """
        ...
        try:
            ...
            compression_metadata["compression_saved_bytes"] = (original_size - compressed_size) * tensor.element_size()
            ...
            self.compression_stats[tensor_id] = {
                ...
                "saved_bytes": (original_size - compressed_size) * tensor.element_size()
            }
            
            logger.debug(f"Compressed tensor {tensor_id}: {original_size} -> {compressed_size} "
    ...
    def decompress_tensor(self, 
        ...
        try:
            ...
            logger.debug(f"Decompressed tensor {tensor_id}, shape: {decompressed_tensor.shape}")
            return decompressed_tensor
            
        except Exception as e:
            logger.error(f"Error decompressing tensor {tensor_id}: {e}")
            # Return compressed data if decompression fails
            if isinstance(compressed_data, dict) and "original_tensor" in compressed_data:
                return compressed_data["original_tensor"]
            else:
                return compressed_data

    def _compress_with_pca(self, 
                          tensor: np.ndarray, 
                          tensor_id: str, 
                          n_components: int) -> Tuple[Dict[str, torch.Tensor], Dict[str, Any]]:
        """
        ...
        components = torch.from_numpy(self.pca_models[tensor_id].components_).to(self.device)
        ...
        metadata = {
            ...
            "explained_variance_ratio": float(np.sum(self.pca_models[tensor_id].explained_variance_ratio_)),
            "compression_type": "pca"
        }
        
        return compressed_data, metadata

    def _decompress_with_pca(self, 
    ...
    def _compress_with_svd(self, 
        ...
        # Convert to torch tensors
        U = torch.from_numpy(self.svd_models[tensor_id].components_).to(self.device)  # Actually V^T
        S = torch.from_numpy(self.svd_models[tensor_id].singular_values_).to(self.device)
    ...
    def _reshape_for_compression(self, tensor: np.ndarray) -> Tuple[np.ndarray, Dict[str, Any]]:
        """
    ...
    def _restore_from_compression(self, matrix: np.ndarray, shape_info: Dict[str, Any]) -> np.ndarray:
        """
...
class AdaptiveTensorCompressor(TensorCompressor):
    ...
    def adjust_compression_ratio(self) -> float:
        ...
        logger.debug(f"Memory usage: {avg_memory:.3f}, Adjusted compression ratio: {adjusted_ratio:.3f}")
...
def compress_model_weights(model: nn.Module,
                          compression_ratio: float = 0.5,
...
def decompress_model_weights(compressed_model: nn.Module,
                           compression_metadata: Dict[str, Any]) -> nn.Module:
    """
