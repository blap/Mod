...
class ModelSharder:
    ...
    def __init__(self, storage_path: str = "./shards"):
        ...
        self.background_thread = threading.Thread(target=self._background_manager, daemon=True)
        ...
        for i in range(num_shards):
            ...
            shard_info = ShardInfo(
                ...
                compute_requirements={
                    'min_gpu_memory_mb': max(10, shard_size // (1024 * 1024)),  # Rough estimate
                    'precision': 'float16' if next(model.parameters()).dtype == torch.float16 else 'float32'
                }
            )
        ...
        with self.shard_locks[shard_id]:
            ...
            try:
                ...
                self.stats['avg_load_time'] = (
                    (self.stats['avg_load_time'] * (self.stats['total_loads'] - 1) + load_time) 
        ...
        with self.shard_locks[shard_id]:
            ...
            try:
                ...
                self.stats['avg_unload_time'] = (
                    (self.stats['avg_unload_time'] * (self.stats['total_unloads'] - 1) + unload_time) 
    ...
    def get_required_shards(self, input_shape: Tuple, inference_step: str = "forward") -> List[str]:
        """
        ...
        if inference_step == "forward":
            ...
            shard_subset_size = max(1, len(self.shards) // 10)  # Load 10% of shards at a time
            
            # Calculate which shards to load based on input position
            start_shard_idx = (input_seq_len * 7) % len(self.shards)  # Pseudo-random based on input
    ...
    def execute_with_shards(self, input_tensor: torch.Tensor, required_shards: List[str], 
    ...
    def get_memory_usage(self) -> Dict[str, Any]:
        ...
        loaded_size = sum(
            self.loaded_shards[shard_id].numel() * next(self.loaded_shards[shard_id].parameters()).element_size() 
            for shard_id in self.loaded_shards
        ) if self.loaded_shards else 0
        ...
        return {
            ...
            'memory_utilization_ratio': loaded_size / total_size if total_size > 0 else 0,
...
class StreamingModelLoader:
    ...
    def prepare_inference_context(self, context_id: str, input_shape: Tuple, 
        ...
        # Load required shards
        loaded_shards = []
        for shard_id in required_shards[:self.max_loaded_shards]:  # Limit to max loaded shards
            try:
                self.sharder.load_shard(shard_id)
                loaded_shards.append(shard_id)
            except Exception as e:
                logger.error(f"Failed to load shard {shard_id} for context {context_id}: {e}")
    ...
    def execute_in_context(self, context_id: str, input_tensor: torch.Tensor) -> torch.Tensor:
        """
...
def create_extreme_sharding_system(storage_path: str = "./shards", num_shards: int = 500) -> Tuple[ModelSharder, StreamingModelLoader]:
    """
    ...
    loader = StreamingModelLoader(sharder, max_loaded_shards=10)  # Limit to 10 shards at a time
