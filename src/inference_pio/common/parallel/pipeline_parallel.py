...
class PipelineConfig:
    ...
    num_stages: int = 1
    stage_device_mapping: Optional[List[str]] = (
...
class PipelineParallel(nn.Module):
    ...
    def _split_model_into_stages(self):
        ...
        if hasattr(self.model, "transformer") and hasattr(self.model.transformer, "h"):
            ...
            for child_name, child_module in self.model.named_children():
                if isinstance(child_module, nn.ModuleList):
                    ...
                    logger.info(
                        f"Using ModuleList '{child_name}' for pipeline splitting"
                    )
                    break
        ...
        for i in range(self.config.num_stages):
            ...
            logger.info(
                f"Stage {i}: {start_idx}-{end_idx-1} ({len(stage_layers)} layers) on {device_mapping[i]}"
            )
...
class PipelineParallelManager:
    ...
    def create_pipeline_model(
        self, model: nn.Module, config: Optional[PipelineConfig] = None
    ) -> PipelineParallel:
