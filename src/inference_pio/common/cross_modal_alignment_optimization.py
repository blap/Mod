    ...
    def __init__(self, config: CrossModalAlignmentConfig):
        ...
        if config.use_attention_alignment:
            self.alignment_attention = nn.MultiheadAttention(
                ...
                num_heads=max(1, config.alignment_projection_dim // 64),  # Heuristic for number of heads
                dropout=0.1,
                batch_first=True
            )
...
def create_generic_cross_modal_alignment(config: CrossModalAlignmentConfig) -> CrossModalAlignmentManager:
    """
...
def get_cross_modal_alignment_report(model: nn.Module, config: CrossModalAlignmentConfig) -> Dict:
    ...
    report = {
        ...
        "alignment_config": {
            ...
            "use_contrastive_alignment": getattr(config, 'use_contrastive_alignment', True),
            "contrastive_margin": getattr(config, 'contrastive_margin', 0.2),
            "enable_dynamic_alignment": getattr(config, 'enable_dynamic_alignment', True),
            ...
            "alignment_projection_dim": getattr(config, 'alignment_projection_dim', getattr(config, 'hidden_size', 2048)),
            "enable_similarity_alignment": getattr(config, 'enable_similarity_alignment', True),
