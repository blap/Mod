...
from .optimization_config import get_config_manager, ModelFamily, ModelOptimizationConfig
import torch
...
def apply_model_family_optimizations(
    ...
    if profile_name:
        if not config_manager.set_active_profile(profile_name):
            logger.warning(f"Profile '{profile_name}' not found, using default settings")
    ...
    if model_config.priority_order:
        ...
        optimization_names = [
            name for name in model_config.priority_order 
            if name in optimization_names
        ]
    
    logger.info(f"Applying optimizations to {model_family.value} model: {optimization_names}")
    return opt_manager.apply_optimizations(model, optimization_names)


def apply_optimizations_by_config(model: nn.Module, config: ModelOptimizationConfig) -> nn.Module:
    """
    ...
    if config.priority_order:
        # Sort by priority order specified in config
        optimization_names = [
            name for name in config.priority_order 
            if name in optimization_names
        ]
...
def get_model_optimization_status(model: nn.Module) -> Dict[str, Any]:
    ...
    return {
        ...
        "optimization_details": {name: status for name, status in all_statuses.items() if name in applied_opts},
...
def create_optimization_pipeline(
    ...
    Returns:
        Function that takes a model and returns an optimized model
    """
    def optimization_pipeline(model: nn.Module) -> nn.Module:
        # Apply model family optimizations
        optimized_model = apply_model_family_optimizations(model, model_family, profile_name)
        ...
        if custom_configs:
            ...
            optimized_model = opt_manager.apply_optimizations(optimized_model, custom_opt_names)
...
def legacy_apply_flash_attention(model: nn.Module, config: Any) -> nn.Module:
    ...
    from .optimization_manager import get_optimization_manager, OptimizationConfig, OptimizationType
...
def legacy_apply_tensor_compression(model: nn.Module, config: Any) -> nn.Module:
    ...
    opt_config = OptimizationConfig(
        ...
        parameters={
            "compression_method": getattr(config, 'tensor_compression_method', 'incremental_pca'),
