...
class ActivationOffloader:
    ...
    def __init__(self,
        ...
        if offload_directory:
            ...
            self.offload_directory = Path(tempfile.mkdtemp(prefix="pio_activation_offload_"))
        ...
        self.activation_cache: deque = deque(maxlen=activation_cache_size)  # Recently accessed activations
        ...
        logger.info(f"Activation offloading system initialized with max_memory_ratio={max_memory_ratio}, "
    ...
    def allocate_activation(self, 
        ...
        with self.lock:
            if activation_id in self.activations:
                logger.warning(f"Activation {activation_id} already exists, overwriting")
            ...
            self.access_analyzer.record_access(activation_id, time.time(), access_pattern.value)

            # Check if we need to offload activations due to memory pressure
            self._handle_memory_pressure()

            logger.debug(f"Allocated activation {activation_id} ({size_bytes} bytes) in RAM")
            return True
    ...
    def deallocate_activation(self, activation_id: str) -> bool:
        ...
        with self.lock:
            ...
            if activation_id in self.ram_activations:
                ...
                if activation_page.file_path and os.path.exists(activation_page.file_path):
                    try:
                        os.remove(activation_page.file_path)
                    except OSError as e:
                        logger.error(f"Failed to delete offload file {activation_page.file_path}: {e}")
    ...
    def offload_activation_to_disk(self, activation_id: str) -> bool:
        ...
        with self.lock:
            ...
            if activation_id not in self.ram_activations:
                logger.warning(f"Activation {activation_id} is not in RAM, nothing to offload")
            ...
            try:
                ...
                with open(offload_file, 'wb') as f:
                    pickle.dump({
                        'activation': activation_page.activation.cpu() if activation_page.activation is not None else None,
                ...
                logger.debug(f"Offloaded activation {activation_id} to disk: {offload_file}")
                return True

            except Exception as e:
                logger.error(f"Failed to offload activation {activation_id} to disk: {e}")
                return False
    ...
    def restore_activation_to_ram(self, activation_id: str) -> bool:
        ...
        with self.lock:
            ...
            if activation_id not in self.disk_activations:
                logger.warning(f"Activation {activation_id} is not on disk, nothing to restore")
                return True  # Already in RAM or invalid state

            if not activation_page.file_path or not os.path.exists(activation_page.file_path):
                logger.error(f"Offload file for activation {activation_id} does not exist: {activation_page.file_path}")
                return False
            ...
            try:
                ...
                logger.debug(f"Restored activation {activation_id} to RAM from: {activation_page.file_path}")
                return True

            except Exception as e:
                logger.error(f"Failed to restore activation {activation_id} to RAM: {e}")
                return False
    ...
    def access_activation(self, activation_id: str) -> Optional[Tensor]:
        ...
        with self.lock:
            ...
            self.access_analyzer.record_access(activation_id, current_time, activation_page.access_pattern)
    ...
    def _handle_memory_pressure(self):
        ...
        if self.eviction_policy == "lru":
            ...
            current_time = time.time()
            activations_to_consider.sort(key=lambda x: self._calculate_prediction_score(x[0], current_time))
        else:
            # Default to LRU
            activations_to_consider.sort(key=lambda x: self.access_times[x[0]])
        ...
        max_offloads_per_call = min(len(activations_to_consider), 10)  # Limit to 10 offloads per call
        offloads_performed = 0

        for activation_id, activation_page in activations_to_consider:
            if not self.is_memory_pressure_high() or offloads_performed >= max_offloads_per_call:
                break
            ...
            if self.offload_activation_to_disk(activation_id):
                logger.debug(f"Evicted activation {activation_id} due to memory pressure")
                offloads_performed += 1
            else:
                logger.warning(f"Failed to evict activation {activation_id} due to memory pressure")

    def _calculate_prediction_score(self, activation_id: str, current_time: float) -> float:
        """
        ...
        access_score = self.access_analyzer.get_access_score(activation_id, current_time)
        ...
        priority_factor = activation_page.priority.value  # Higher priority = less likely to be evicted
        size_factor = activation_page.size_bytes / (1024 * 1024)  # Size in MB, larger = more likely to be evicted
    ...
    def get_activation_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            total_size = sum(activation.size_bytes for activation in self.activations.values())
            ram_size = sum(self.activations[aid].size_bytes for aid in self.ram_activations)
            disk_size = sum(self.activations[aid].size_bytes for aid in self.disk_activations)
    ...
    def start_proactive_management(self, interval: float = 5.0):
        ...
        logger.info(f"Started proactive activation management with interval {interval}s")
    ...
    def stop_proactive_management(self):
        ...
        if self.proactive_thread is not None:
            ...
            self.proactive_thread.join(timeout=2.0)  # Wait up to 2 seconds for thread to finish
    ...
    def _perform_proactive_management(self):
        ...
        with self.lock:
            ...
            if predicted_usage_ratio > self.max_memory_ratio * 0.9:  # 90% of threshold
                logger.info(f"Predicted high memory usage ({predicted_usage_ratio:.2%}), performing proactive offloads")

                # Identify activations that are unlikely to be accessed soon
                activations_to_offload = []
                for activation_id in self.ram_activations:
                    if not self.activations[activation_id].pinned:  # Don't offload pinned activations
                        access_score = self.access_analyzer.get_access_score(activation_id, current_time)

                        # If access score is low (meaning not likely to be accessed soon), consider for offloading
                        if access_score < 0.3:  # Threshold for "not likely to be accessed soon"
                ...
                for activation_id, access_score in activations_to_offload:
                    if self.is_memory_pressure_high():  # Re-check actual memory pressure
                        if self.offload_activation_to_disk(activation_id):
                            logger.debug(f"Proactively offloaded activation {activation_id} (access_score: {access_score:.2f})")
                        else:
                            logger.warning(f"Failed to proactively offload activation {activation_id}")
                    else:
    ...
    def cleanup(self):
        ...
        with self.lock:
            ...
            try:
                ...
                logger.error(f"Failed to clean up offload directory {self.offload_directory}: {e}")
...
class ActivationOffloadingManager:
    ...
    def offload_activation(self, 
        ...
        success = self.activation_offloader.allocate_activation(
            activation, page_id, priority, access_pattern, layer_index, sequence_position
        )
        if success:
    ...
    def set_activation_priority(self, activation_id: str, priority: ActivationPriority) -> bool:
        """
...
class ActivationAccessPatternAnalyzer:
    ...
    def __init__(self, history_size: int = 1000):
        ...
        self.access_intervals = defaultdict(deque)  # Maps activation_id to access intervals
        self.access_patterns = defaultdict(lambda: "unknown")  # Maps activation_id to access pattern

    def record_access(self, activation_id: str, timestamp: float, access_pattern: str = "unknown"):
        ...
        if activation_id in self.access_intervals:
            prev_access = self.access_intervals[activation_id][-1] if self.access_intervals[activation_id] else None
            if prev_access is not None:
                interval = timestamp - prev_access
                self.access_intervals[activation_id].append(interval)
                if len(self.access_intervals[activation_id]) > 10:  # Keep last 10 intervals
    ...
    def predict_next_access(self, activation_id: str, current_time: float) -> float:
        ...
        if activation_id not in self.access_intervals or not self.access_intervals[activation_id]:
    ...
    def get_access_score(self, activation_id: str, current_time: float) -> float:
        ...
        freq_score = min(self.activation_frequency[activation_id] / 10.0, 1.0)  # Normalize
        ...
        prediction_score = 1.0 / (time_to_next + 1.0)  # Higher score for sooner predicted access
...
class ActivationAccessPredictor:
    ...
    def record_activation_access(self, activation_id: str, access_time: float, pattern: str = "unknown"):
    ...
    def predict_next_activation_access(self, activation_id: str, current_time: float) -> float:
...
def create_activation_offloader(max_memory_ratio: float = 0.7,
                               offload_directory: Optional[str] = None,
                               page_size_mb: int = 8,
                               eviction_policy: str = "predictive") -> ActivationOffloader:
    """
