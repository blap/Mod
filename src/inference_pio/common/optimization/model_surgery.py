...
class ModelSurgerySystem:
    ...
    def identify_removable_components(
        ...
        for name, module in model.named_modules():
            ...
            if component_type:
                ...
                can_remove, reason, priority = self._can_safely_remove(
    ...
    def _can_safely_remove(
        self, module: nn.Module, component_type: ComponentType, name: str
    ) -> Tuple[bool, str, int]:
        """
        ...
        # Batch norm layers can often be folded into preceding linear layers during inference
            ...
            ComponentType.BATCH_NORM,
            ComponentType.LAYER_NORM,
            ComponentType.GROUP_NORM,
            ComponentType.INSTANCE_NORM,
        ]:
            ...
            return (
                True,
                f"{component_type.value} can often be simplified during inference",
                2,
            )
        ...
        # Multimodal-specific components
            ...
            return (
                False,
                f"{component_type.value} is essential for multimodal processing",
                90,
            )
    ...
    def perform_surgery(
        ...
        for name in components_to_remove:
            try:
                ...
                replacement_module = self._create_replacement_module(
                    original_module, component_type
                )
                ...
                logger.info(
                    f"Surgically removed component: {name} ({type(original_module).__name__}) "
        ...
        surgery_record["model_after_params"] = sum(
            p.numel() for p in model_copy.parameters()
        )
        ...
        logger.info(
            f"Model surgery completed. Removed {len(surgery_record['components_removed'])} components. "
    ...
    def _create_replacement_module(
        self, original_module: nn.Module, component_type: Optional[ComponentType]
    ) -> nn.Module:
        """
    ...
    def restore_model(
        self, model: nn.Module, surgery_id: Optional[str] = None
    ) -> nn.Module:
        """
        ...
        for removed_component in surgery_record["components_removed"]:
            ...
            if backup_key:
                ...
                setattr(
                    parent_module,
                    backup_data["child_name"],
                    backup_data["original_module"],
                )
                restored_count += 1
                ...
                logger.info(
                    f"Restored component: {component_name} (type: {backup_data.get('component_type', 'unknown')})"
                )
        ...
        logger.info(
            f"Model restoration completed. Restored {restored_count} components."
        )
...
def apply_model_surgery(
    ...
    return surgery_system.perform_surgery(
        model, components_to_remove, preserve_components
    )


def restore_model_from_surgery(
    model: nn.Module, surgery_id: Optional[str] = None
) -> nn.Module:
    """
