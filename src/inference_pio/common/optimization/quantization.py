...
class QuantizationConfig:
    ...
    def _validate_config(self):
        ...
        if self.scheme == QuantizationScheme.INT4:
            if self.bits != 4:
                logger.warning(
                    f"INT4 scheme expects 4 bits, got {self.bits}. Setting to 4."
                )
                self.bits = 4
            ...
            if self.bits != 8:
                logger.warning(
                    f"INT8 scheme expects 8 bits, got {self.bits}. Setting to 8."
                )
                self.bits = 8
            if self.quant_min is None:
            ...
            if self.bits != 16:
                logger.warning(
                    f"FP16 scheme expects 16 bits, got {self.bits}. Setting to 16."
                )
                self.bits = 16
        elif self.scheme == QuantizationScheme.NF4:
            if self.bits != 4:
                logger.warning(
                    f"NF4 scheme expects 4 bits, got {self.bits}. Setting to 4."
                )
                self.bits = 4
...
class QuantizationObserver:
    ...
    def get_scale_zero_point(self) -> Tuple[float, int]:
        ...
        if self.config.symmetric:
            ...
            scale = abs_max / (
                float(self.config.quant_max)
                if self.config.symmetric
                else float(self.config.quant_max)
            )
...
class QuantizedLinear(nn.Module):
    ...
    def __init__(
        self, weight, bias=None, quantization_config: QuantizationConfig = None
    ):
        super().__init__()
...
class QuantizationManager:
    ...
    def _quantize_fp16(self, model: nn.Module, config: QuantizationConfig) -> nn.Module:
        ...
        logger.info(
            f"FP16 quantization applied. Memory usage reduced by ~50% (params: {original_params})"
        )
    ...
    def _quantize_int(
        ...
        original_params = sum(p.numel() for p in model.parameters())
        logger.info(
            f"{config.scheme.value} quantization applied. Memory usage reduced by ~{int((1-config.bits/32)*100)}% (params: {original_params})"
        )
    ...
    def _quantize_nf4(self, model: nn.Module, config: QuantizationConfig) -> nn.Module:
        ...
        original_params = sum(p.numel() for p in model.parameters())
        logger.info(
            f"NF4 quantization applied. Memory usage reduced by ~87.5% (params: {original_params})"
        )
    ...
    def _replace_linear_layers(
        ...
        for name, module in model.named_children():
            if isinstance(module, nn.Linear):
                ...
                quantized_linear = QuantizedLinear(module.weight, module.bias, config)
    ...
    def quantize_tensor(
        self, tensor: torch.Tensor, config: QuantizationConfig
    ) -> torch.Tensor:
...
def initialize_default_quantization_schemes():
    ...
    fp16_config = QuantizationConfig(
        scheme=QuantizationScheme.FP16, bits=16, symmetric=False, per_channel=False
    )
    quantization_manager.register_quantization_scheme("fp16", fp16_config)

    # NF4 quantization
    nf4_config = QuantizationConfig(
        scheme=QuantizationScheme.NF4, bits=4, symmetric=True, per_channel=True
    )
