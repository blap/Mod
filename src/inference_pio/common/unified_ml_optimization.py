...
from .ml_optimization_selector import AutoOptimizationSelector, get_auto_selector, PerformanceMetrics
from .hyperparameter_optimizer import get_performance_optimizer, PerformanceHyperparameterOptimizer
...
class UnifiedMLOptimizationSystem:
    ...
    def __init__(self):
        ...
        self.model_configs: Dict[str, Dict[str, Any]] = {}  # Store optimal configs per model
        self.hardware_info: Dict[str, Any] = self._get_hardware_info()

    def register_model_type(self, model_type: ModelType, config: MLBasedOptimizationConfig):
        ...
        elif model_type in [ModelType.QWEN3_4B_INSTRUCT_2507,
    ...
    def optimize_model_for_input(
        ...
        if model_type not in self.configs:
            logger.warning(f"No configuration found for model type {model_type}, using defaults")
        ...
        should_reoptimize = (self.call_counts[model_id] % config.optimization_frequency) == 0
        ...
        if config.enable_ml_selection or should_reoptimize:
            ...
            if str(model_id) in self.model_configs:
                optimized_model = self._apply_stored_config(model, self.model_configs[str(model_id)])
            else:
    ...
    def _apply_ml_optimizations(
        ...
        try:
            ...
            selected_optimizations = self.auto_selector.select_optimizations(
                ...
                target_metric=config.target_metric + "_ms" if config.target_metric == "latency" else config.target_metric,
                max_optimizations=config.max_optimizations
            )

            logger.info(f"Selected optimizations for {model_type.value}: {selected_optimizations}")
            ...
            # Apply optimizations
            optimized_model = opt_manager.apply_optimizations(model, selected_optimizations)
    ...
    def _apply_hyperparameter_tuning(
        ...
        try:
            ...
            logger.info(f"Hyperparameter optimization completed. Best params: {optimization_result.best_params}")
    ...
    def _apply_stored_config(self, model: nn.Module, config: Dict[str, Any]) -> nn.Module:
    ...
    def _get_hardware_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'cpu_cores': len(os.sched_getaffinity(0)) if hasattr(os, 'sched_getaffinity') else os.cpu_count() or 4,
