...
class GenericCrossModalFusionKernel(nn.Module):
    ...
    def forward(
        ...
        if self.fusion_method == "add":
            ...
            fused_output = vision_padded[:, :min_len, :] * language_padded[:, :min_len, :]
            ...
            attention_weights = F.softmax(attention_scores, dim=-1, dtype=torch.float32).to(vision_features.dtype)
        ...
        language_to_vision_combined = torch.cat([
            language_avg.expand_as(vision_padded),
            vision_padded
        ], dim=-1)
        ...
        vision_output = vision_features + (fused_output[:, :vision_seq_len, :] * vision_gate_part)
        language_output = language_features + (fused_output[:, :lang_seq_len, :] * language_gate_part)
