...
class TextAccessPatternAnalyzer:
    ...
    def record_access(self, page_id: str, timestamp: float, access_pattern: str = "unknown", 
                      sequence_position: Optional[int] = None, layer_index: Optional[int] = None):
    ...
    def get_access_frequency(self, page_id: str, current_time: float) -> float:
        ...
        with self.lock:
            ...
            recent_accesses = sum(1 for t, pid, pos in self.access_history
...
class UnimodalTensorPaginationSystem:
    ...
    def __init__(
        ...
        self.swap_directory = Path(swap_directory) if swap_directory else Path("./text_tensor_swap")
        ...
        logger.info(f"Initialized UnimodalTensorPaginationSystem with swap_directory={self.swap_directory}, "
    ...
    def _calculate_intelligent_score(self, page_id: str, current_time: float) -> float:
        ...
        if page.position_in_sequence is not None:
            position_factor = min(page.position_in_sequence / 1000.0, 1.0)  # Normalize by 1000
        else:
            position_factor = 0.5  # Default if not specified

        # Is past key-value (KV cache tensors)
        kv_factor = 0.3 if page.is_past_key_value else 0.7  # Past KV can be evicted more easily
