"""
Tensor Utilities for Inference-PIO System
...
def apply_chunking_to_forward(
    ...
    *input_tensors
    ...
    if chunk_size > 0:
        ...
        input_tensors_chunks = [input_tensor.chunk(num_chunks, dim=chunk_dim) for input_tensor in input_tensors]
...
def validate_tensor_shape(
    ...
    if len(tensor.shape) != len(expected_shape):
        raise ValueError(f"{tensor_name} has incorrect number of dimensions: {len(tensor.shape)} expected {len(expected_shape)}")
    
    for i, (actual, expected) in enumerate(zip(tensor.shape, expected_shape)):
        if expected is not None and actual != expected:
            raise ValueError(f"{tensor_name} has incorrect size at dimension {i}: {actual} expected {expected}")
