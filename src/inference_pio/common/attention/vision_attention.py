...
class VisionPatchAttention(nn.Module):
    ...
    def _init_relative_position_embeddings(self):
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros(
                (2 * max_positions - 1) * (2 * max_positions - 1), self.num_heads
            )
        )
    ...
    def _calculate_spatial_attention(
        self, attention_weights: torch.Tensor, height: int, width: int
    ) -> torch.Tensor:
        """
    ...
    def forward(
        ...
        q = q.view(B, N + 1, self.num_heads, self.head_dim).transpose(
            1, 2
        )  # (B, num_heads, N+1, head_dim)
        k = k.view(B, N + 1, self.num_heads, self.head_dim).transpose(
            1, 2
        )  # (B, num_heads, N+1, head_dim)
        v = v.view(B, N + 1, self.num_heads, self.head_dim).transpose(
            1, 2
        )  # (B, num_heads, N+1, head_dim)
        ...
        if self.use_relative_position:
            attn_weights = self._calculate_spatial_attention(
                attn_weights, int(math.sqrt(N)), int(math.sqrt(N))
            )
...
class SparseVisionAttention(nn.Module):
    ...
    def __init__(
        ...
        if use_global_tokens:
            self.global_tokens = nn.Parameter(
                torch.randn(1, num_global_tokens, embed_dim)
            )
    ...
    def forward(
        ...
        # Reshape for multi-head attention
        q = q.view(B, N_total, self.num_heads, self.head_dim).transpose(
            1, 2
        )  # (B, num_heads, N_total, head_dim)
        k = k.view(B, N_total, self.num_heads, self.head_dim).transpose(
            1, 2
        )  # (B, num_heads, N_total, head_dim)
        v = v.view(B, N_total, self.num_heads, self.head_dim).transpose(
            1, 2
        )  # (B, num_heads, N_total, head_dim)
        ...
        if self.use_global_tokens:
            full_sparse_mask = torch.ones(
                N_total, N_total, dtype=torch.bool, device=x.device
            )
            ...
            full_sparse_mask[self.num_global_tokens :, self.num_global_tokens :] = (
                sparse_mask[0, 0, :, :]
            )
...
class EfficientVisionAttention(nn.Module):
    ...
    def __init__(
        ...
        if use_linear_attention:
            self.q_low_rank = nn.Linear(
                embed_dim, self.low_rank_dim * num_heads, bias=False
            )
            self.k_low_rank = nn.Linear(
                embed_dim, self.low_rank_dim * num_heads, bias=False
            )
            self.v_low_rank = nn.Linear(
                embed_dim, self.low_rank_dim * num_heads, bias=False
            )
    ...
    def forward(
        ...
        if self.use_linear_attention:
            ...
            q_low = (
                self.q_low_rank(x)
                .view(B, N, self.num_heads, self.low_rank_dim)
                .transpose(1, 2)
            )
            k_low = (
                self.k_low_rank(x)
