...
class GenericMultimodalAttentionOptimizer(nn.Module):
    ...
    def __init__(
        ...
        self.vision_to_language_proj = nn.Linear(
            self.hidden_size, self.hidden_size, bias=False
        )
        self.language_to_vision_proj = nn.Linear(
            self.hidden_size, self.hidden_size, bias=False
        )
        ...
        self.cross_modal_fusion = nn.Linear(
    ...
    def forward(
        ...
        vision_query = vision_query.view(
            vision_bsz, vision_seq_len, self.num_attention_heads, self.head_dim
        ).transpose(1, 2)
        vision_key = vision_key.view(
            vision_bsz, vision_seq_len, self.num_key_value_heads, self.head_dim
        ).transpose(1, 2)
        vision_value = vision_value.view(
            vision_bsz, vision_seq_len, self.num_key_value_heads, self.head_dim
        ).transpose(1, 2)

        language_query = language_query.view(
            lang_bsz, lang_seq_len, self.num_attention_heads, self.head_dim
        ).transpose(1, 2)
        language_key = language_key.view(
            lang_bsz, lang_seq_len, self.num_key_value_heads, self.head_dim
        ).transpose(1, 2)
        language_value = language_value.view(
            lang_bsz, lang_seq_len, self.num_key_value_heads, self.head_dim
        ).transpose(1, 2)

        # Repeat key and value for GQA (Grouped-Query Attention)
        vision_key = torch.repeat_interleave(
            vision_key, dim=1, repeats=self.num_key_value_groups
        )
        vision_value = torch.repeat_interleave(
            vision_value, dim=1, repeats=self.num_key_value_groups
        )

        language_key = torch.repeat_interleave(
            language_key, dim=1, repeats=self.num_key_value_groups
        )
        language_value = torch.repeat_interleave(
            language_value, dim=1, repeats=self.num_key_value_groups
        )

        # Compute cross-modal attention: vision attending to language and vice versa
        # Vision features attend to language features
        vision_attn_weights = torch.matmul(
            vision_query, language_key.transpose(-1, -2)
        ) / (self.head_dim**0.5)
        ...
        if attention_mask is not None:
            ...
            if (
                ...
                and attention_mask.size(-2) == vision_seq_len
                and attention_mask.size(-1) == lang_seq_len
            ):
                ...
                expanded_mask = attention_mask.unsqueeze(2).expand(
                    -1, -1, vision_seq_len, -1
                )
                ...
                expanded_mask = (
                    ...
                    .unsqueeze(0)
                    .expand(vision_bsz, 1, vision_seq_len, lang_seq_len)
                )
                vision_attn_weights = vision_attn_weights + expanded_mask

        vision_attn_weights = F.softmax(
            vision_attn_weights, dim=-1, dtype=torch.float32
        ).to(vision_query.dtype)
        ...
        language_attn_weights = torch.matmul(
            language_query, vision_key.transpose(-1, -2)
        ) / (self.head_dim**0.5)
        ...
        if attention_mask is not None:
            ...
            if (
                attention_mask.dim() == 4
                and attention_mask.size(-2) == lang_seq_len
                and attention_mask.size(-1) == vision_seq_len
            ):
                language_attn_weights = language_attn_weights + attention_mask
            elif (
                attention_mask.dim() == 3 and attention_mask.size(-1) == vision_seq_len
            ):
                # Expand mask to match attention weights shape
                expanded_mask = attention_mask.unsqueeze(2).expand(
                    -1, -1, lang_seq_len, -1
                ...
                expanded_mask = (
                    attention_mask.unsqueeze(0)
                    .unsqueeze(0)
                    .expand(lang_bsz, 1, lang_seq_len, vision_seq_len)
                )
                language_attn_weights = language_attn_weights + expanded_mask

        language_attn_weights = F.softmax(
            language_attn_weights, dim=-1, dtype=torch.float32
        ).to(language_query.dtype)
        if self.dropout is not None:
        ...
        vision_attn_output = (
            vision_attn_output.transpose(1, 2)
            .contiguous()
            .view(vision_bsz, vision_seq_len, self.num_attention_heads * self.head_dim)
        )
        language_attn_output = (
            language_attn_output.transpose(1, 2)
            .contiguous()
            .view(lang_bsz, lang_seq_len, self.num_attention_heads * self.head_dim)
        )
        ...
        if vision_seq_len < lang_seq_len:
            vision_output = F.pad(
                vision_output, (0, 0, 0, lang_seq_len - vision_seq_len)
            )
        elif lang_seq_len < vision_seq_len:
            language_output = F.pad(
                language_output, (0, 0, 0, vision_seq_len - lang_seq_len)
            )
...
class GenericMultimodalAttentionManager:
    ...
    def __init__(self, config: Any):
        ...
        for layer_idx in range(config.num_hidden_layers):
            self.attention_optimizers[f"layer_{layer_idx}"] = (
    ...
    def apply_optimizations_to_model(self, model: nn.Module) -> nn.Module:
                ...
                try:
                    ...
                    if hasattr(layer, "self_attn"):
                        ...
                        if optimized_attn is not None:
                            ...
                            replaced_count += 1
                            logger.debug(
                                f"Replaced attention in layer {idx} with optimized multimodal attention"
                            )
        ...
        logger.info(
            f"Generic multimodal attention optimizations applied: {replaced_count}/{total_layers} attention layers replaced"
        )

        return model


def create_generic_multimodal_attention_optimizer(
    config: Any, layer_idx: int = 0
) -> GenericMultimodalAttentionOptimizer:
    """
...
def apply_generic_multimodal_attention_optimizations_to_model(
...
def get_generic_multimodal_attention_optimization_report(
    model: nn.Module, config: Any
) -> Dict[str, Any]:
    """
    ...
    report = {
        ...
        "optimizations_applied": {
            ...
            "grouped_query_attention": getattr(
                config, "use_grouped_query_attention", False
            ),
