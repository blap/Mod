...
class TextPreprocessor:
    ...
    def preprocess(
        ...
        logger.debug(
            f"Text preprocessing took {elapsed_time:.4f}s for input length {len(text)}"
        )
    ...
    def batch_preprocess(
        ...
        elapsed_time = time.time() - start_time
        logger.debug(
            f"Batch text preprocessing took {elapsed_time:.4f}s for {len(texts)} texts"
        )
...
class UnimodalPreprocessor:
    ...
    def preprocess(
        self, text: str, return_tensors: str = "pt", model_type: Optional[str] = None
    ) -> Dict[str, torch.Tensor]:
        """
    ...
    def batch_preprocess(
        ...
        logger.debug(
            f"Batch unimodal preprocessing took {elapsed_time:.4f}s for {len(texts)} inputs"
        )
...
def create_unimodal_preprocessor(
    model_path: str, max_text_length: int = 32768
    ...
    try:
        ...
        preprocessor = UnimodalPreprocessor(
            tokenizer=tokenizer, max_text_length=max_text_length
        )
...
def apply_unimodal_preprocessing_to_model(
    model: nn.Module, preprocessor: UnimodalPreprocessor
) -> nn.Module:
    """
