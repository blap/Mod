...
class UnimodalModelSurgerySystem(ModelSurgerySystem):
    ...
    def identify_removable_components(self, model: nn.Module) -> List[UnimodalSurgicalComponent]:
        """
        ...
        for name, module in model.named_modules():
            ...
            if component_type:
                ...
                if can_remove:
                    ...
                    language_specificity = self._analyze_language_specificity(module, name)
                    semantic_importance = self._analyze_semantic_importance(module, name)
                    computational_overhead = self._analyze_computational_overhead(module)
    ...
    def _classify_unimodal_module(self, module: nn.Module) -> Optional[UnimodalComponentType]:
        """
        ...
        elif 'text' in module_class_name and ('norm' in module_class_name or 'layernorm' in module_class_name):
    ...
    def _can_safely_remove_unimodal(self, module: nn.Module,
                                    component_type: UnimodalComponentType,
        ...
        if component_type == UnimodalComponentType.EMBEDDING_LAYER:
            ...
            return False, "Embedding layers are essential for language understanding", 100
        ...
        elif component_type in [UnimodalComponentType.TEXT_EMBEDDING_LAYER, 
            ...
            if 'auxiliary' in name.lower() or 'extra' in name.lower():
                return True, f"{component_type.value} appears auxiliary and can be simplified", 6
            else:
                return False, f"{component_type.value} is likely essential for text processing", 90
    ...
    def _analyze_language_specificity(self, module: nn.Module, name: str) -> float:
        ...
        if any(keyword in name_lower for keyword in ['text', 'language', 'word', 'token', 'sentence', 'phrase']):
            specificity += 0.3
    ...
    def _analyze_semantic_importance(self, module: nn.Module, name: str) -> float:
        ...
        if any(keyword in name_lower for keyword in ['encoder', 'decoder', 'attention', 'transformer']):
            importance += 0.3
        elif any(keyword in name_lower for keyword in ['embedding', 'vocab', 'classification']):
            importance += 0.2
            
        # Decrease for auxiliary components
        if any(keyword in name_lower for keyword in ['aux', 'auxiliary', 'extra', 'debug']):
            importance -= 0.3
    ...
    def perform_unimodal_surgery(self, model: nn.Module,
        ...
        if components_to_remove is None:
            ...
            components_to_remove = [
                comp.name for comp in all_removable 
    ...
    def analyze_unimodal_model_for_surgery(self, model: nn.Module) -> Dict[str, Any]:
        ...
        for name, module in model.named_modules():
            ...
            name_lower = name.lower()
            if any(keyword in name_lower for keyword in ['text', 'language', 'word', 'token']):
        ...
        if removable:
            ...
            for lang_spec, count in analysis['language_specificity_distribution'].items():
                analysis['recommendations'].append(
...
def apply_unimodal_model_surgery(model: nn.Module,
                                components_to_remove: Optional[List[str]] = None,
                                preserve_components: Optional[List[str]] = None,
                                preserve_semantic_importance_threshold: float = 0.7) -> nn.Module:
    """
    ...
    return surgery_system.perform_unimodal_surgery(
        model, components_to_remove, preserve_components, preserve_semantic_importance_threshold
    )
