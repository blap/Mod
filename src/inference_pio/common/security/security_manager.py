...
class SecurityManager:
    ...
    def create_security_context(
        ...
        logger.info(
            f"Created security context for plugin {plugin_id} with security level {security_level.value}"
        )
        return context
    ...
    def validate_path_access(self, plugin_id: str, path: str) -> bool:
        ...
        for forbidden_path in context.forbidden_paths:
            try:
                if path_obj.is_relative_to(Path(forbidden_path)):
                    logger.warning(
                        f"Plugin {plugin_id} attempted to access forbidden path: {path}"
                    )
                    return False
            except ValueError:
        ...
        logger.warning(
            f"Plugin {plugin_id} attempted to access unauthorized path: {path}"
        )
        return False
    ...
    def validate_network_access(self, plugin_id: str, host: str) -> bool:
        ...
        logger.warning(
            f"Plugin {plugin_id} attempted to connect to unauthorized host: {host}"
        )
        return False
    ...
    def enforce_resource_limits(self, plugin_id: str) -> bool:
        ...
        if limits.cpu_percent is not None:
            ...
            if cpu_percent > limits.cpu_percent:
                logger.warning(
                    f"Plugin {plugin_id} exceeded CPU limit: {cpu_percent}% > {limits.cpu_percent}%"
                )
                return False
        ...
        if limits.memory_gb is not None:
            ...
            if memory_gb > limits.memory_gb:
                logger.warning(
                    f"Plugin {plugin_id} exceeded memory limit: {memory_gb:.2f}GB > {limits.memory_gb}GB"
                )
                return False
        ...
        if limits.gpu_memory_gb is not None and torch.cuda.is_available():
            ...
            if gpu_memory_allocated > limits.gpu_memory_gb:
                logger.warning(
                    f"Plugin {plugin_id} exceeded GPU memory limit: {gpu_memory_allocated:.2f}GB > {limits.gpu_memory_gb}GB"
                )
                return False
        ...
        if context.sandbox_directory and limits.disk_space_gb is not None:
            try:
                ...
                if disk_usage_gb > limits.disk_space_gb:
                    logger.warning(
                        f"Plugin {plugin_id} exceeded disk space limit: {disk_usage_gb:.2f}GB > {limits.disk_space_gb}GB"
                    )
                    return False
    ...
    def start_resource_monitoring(self, plugin_id: str, interval: float = 1.0) -> bool:
        ...
        if (
            plugin_id in self.active_monitors
            and self.active_monitors[plugin_id].is_alive()
        ):
        ...
        def monitor_resources():
            while self.monitoring_active:
                try:
                    ...
                    logger.error(
                        f"Error in resource monitoring for plugin {plugin_id}: {e}"
                    )
                    break
    ...
    def cleanup_security_context(self, plugin_id: str) -> bool:
        ...
        if context.sandbox_directory:
            try:
                ...
                logger.info(
                    f"Removed sandbox directory for plugin {plugin_id}: {context.sandbox_directory}"
                )
            except Exception as e:
                logger.error(
                    f"Error removing sandbox directory for plugin {plugin_id}: {e}"
                )
...
class ResourceIsolationManager:
    ...
    def initialize_plugin_isolation(
        ...
        try:
            ...
            logger.error(
                f"Failed to initialize resource isolation for plugin {plugin_id}: {e}"
            )
            return False
    ...
    def begin_operation(self, plugin_id: str) -> Optional[str]:
        ...
        if not self.security_manager.enforce_resource_limits(plugin_id):
            logger.error(
                f"Resource limits exceeded for plugin {plugin_id}, denying operation"
            )
            return None
        ...
        logger.debug(
            f"Started operation for plugin {plugin_id}, active operations: {self.plugin_resources[plugin_id]['active_operations']}"
        )
    ...
    def end_operation(self, plugin_id: str, operation_token: str) -> bool:
        ...
        with self.lock:
            if "operation_tokens" in self.plugin_resources[plugin_id]:
                if (
                    operation_token
                    in self.plugin_resources[plugin_id]["operation_tokens"]
                ):
                    del self.plugin_resources[plugin_id]["operation_tokens"][
                        operation_token
                    ]
                    self.plugin_resources[plugin_id]["active_operations"] -= 1
                    return True
    ...
    def get_plugin_resource_usage(self, plugin_id: str) -> Dict[str, Any]:
        ...
        usage_info = {
            ...
            - self.plugin_resources[plugin_id]["start_time"],
        ...
        try:
            ...
            if torch.cuda.is_available():
                usage_info["gpu_memory_allocated_gb"] = (
                    torch.cuda.memory_allocated() / (1024**3)
                )
                usage_info["gpu_memory_reserved_gb"] = torch.cuda.memory_reserved() / (
                    1024**3
                )
        except Exception as e:
...
def initialize_plugin_isolation(
    ...
    return manager.initialize_plugin_isolation(
        plugin_id, security_level, resource_limits
    )
