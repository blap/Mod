"""
Refactored Model Adapter for Continuous NAS System
...
This version eliminates cross-model dependencies by using a registry pattern.
"""
...
_MODEL_ADAPTER_REGISTRY = {}

def register_model_adapter(model_identifier: str, adapter_class):
    """Register a model adapter class for a specific model identifier."""
    _MODEL_ADAPTER_REGISTRY[model_identifier] = adapter_class
    logger.debug(f"Registered model adapter for {model_identifier}: {adapter_class.__name__}")

def get_model_adapter_from_registry(model_identifier: str):
    """Get a model adapter class from the registry."""
    return _MODEL_ADAPTER_REGISTRY.get(model_identifier)
...
def get_model_adapter(
    ...
    This version eliminates the cross-model dependency by using a registry pattern.
    """
    # Try to identify the model type based on its attributes
    model_str = str(type(model)).lower()

    # First, try to identify using the registry approach
    if "glm" in model_str and "4" in model_str:
        # Check if GLM47 adapter is registered
        adapter_class = get_model_adapter_from_registry("glm47")
        if adapter_class:
        ...
        # Check if Qwen3-4B adapter is registered
        adapter_class = get_model_adapter_from_registry("qwen3_4b_instruct_2507")
        if adapter_class:
            return adapter_class(model, nas_controller)
        else:
        ...
        # Check if Qwen3-Coder-30B adapter is registered
        adapter_class = get_model_adapter_from_registry("qwen3_coder_30b")
        if adapter_class:
            return adapter_class(model, nas_controller)
        else:
        ...
        # For Qwen3-VL-2B, check if it's registered in the registry
        adapter_class = get_model_adapter_from_registry("qwen3_vl_2b")
        if adapter_class:
            return adapter_class(model, nas_controller)
        else:
            # Previously, this would import directly from the model directory
            # Now we'll create the adapter class inline to avoid the dependency
            # OR we expect it to be registered elsewhere
            
            # Define the Qwen3VL2BModelAdapter class here to avoid external import
            class Qwen3VL2BModelAdapter(BaseModelAdapter):
                """
                Model adapter for Qwen3-VL-2B model architecture.
                This is defined inline to avoid cross-model dependencies.
                """
                ...
                def adapt_depth(self, depth_ratio: float) -> nn.Module:
                    ...
                    logger.info(
                        f"Qwen3-VL-2B depth adapted: {num_layers} -> {target_layers} layers"
                    )
                ...
                def adapt_width(self, width_ratio: float) -> nn.Module:
                    ...
                    logger.info(
                        f"Qwen3-VL-2B width adapted: hidden_size {hidden_size} -> {target_hidden_size}"
                    )
