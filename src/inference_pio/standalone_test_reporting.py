...
class TestReportGenerator:
    ...
    def _collect_system_info(self) -> SystemInfo:
        ...
        try:
            ...
            gpu_names = [torch.cuda.get_device_name(i) for i in range(gpu_count)] if gpu_count else None
            gpu_usage = torch.cuda.utilization() if cuda_available and hasattr(torch.cuda, 'utilization') else None
            gpu_memory = torch.cuda.get_device_properties(0).total_memory / (1024**3) if gpu_count and gpu_count > 0 else None
        except ImportError:
        ...
        info = SystemInfo(
            ...
            os_version=getattr(os, 'uname', lambda: type('', (), {'version': 'Unknown'})())().version if hasattr(os, 'uname') else "Unknown",
    ...
    def generate_json_report(self, test_results: list, metrics: TestSuiteMetrics) -> str:
        """
    ...
    def generate_html_report(self, test_results: list, metrics: TestSuiteMetrics) -> str:
        """
...
<body>
    ...
    def generate_markdown_report(self, test_results: list, metrics: TestSuiteMetrics) -> str:
        """
