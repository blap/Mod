    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                isinstance(n, ast.ClassDef) for n in ast.walk(node)
            ):
                functions.append(
                    {
                        "name": node.name,
...
def map_tests_to_sources(
    test_files: List[str], source_analysis: List[Dict[str, Any]]
) -> Dict[str, Any]:
    ...
    for test_file in test_files:
        ...
        for imp in test_analysis["imported_modules"]:
            ...
            for source_path in source_map.keys():
                if (
                    imp_path in source_path
                    or os.path.basename(imp_path).replace(".py", "") in source_path
                ):
...
def calculate_coverage(
    source_analysis: List[Dict[str, Any]], test_mappings: Dict[str, List[str]]
) -> Dict[str, Any]:
    ...
    overall_coverage = (
        (covered_elements / total_public_elements * 100)
    ...
    return {
        ...
        "untested_files": [
            detail for detail in coverage_details if not detail["has_tests"]
        ],
    }
...
def generate_coverage_report():
    ...
    coverage_result = calculate_coverage(
        source_analysis, mapping_result["test_mappings"]
    )
    ...
    report = {
        "summary": {
            ...
            "tested_files_count": len(source_files)
            - len(coverage_result["untested_files"]),
...
def print_coverage_report(report: Dict[str, Any]):
    ...
    if report["untested_files"]:
        ...
        for untested in report["untested_files"][:10]:  # Show first 10
            print(
                f"  - {untested['source_file']} ({untested['total_public_elements']} public elements)"
            )
    ...
    print(f"\nCoverage by File:")
    covered_files = [
        detail for detail in report["coverage_details"] if detail["has_tests"]
    ]
    for detail in covered_files[:10]:  # Show first 10
        test_count = len(detail["test_files"])
        print(
            f"  {detail['source_file']}: {detail['total_public_elements']} elements, {test_count} test files"
        )
...
def main():
    ...
    if report["summary"]["untested_files_count"] > 0:
        print(
            f"- {report['summary']['untested_files_count']} files have no tests, prioritize these"
        )
    if (
        report["test_distribution"]["unit_tests"]
        < report["test_distribution"]["integration_tests"]
    ):
        print(
            "- Consider adding more unit tests relative to integration tests for better granularity"
        )
