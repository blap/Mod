...
def test_config_runtime_optimization_settings():
    ...
    from inference_pio.models.qwen3_4b_instruct_2507.config import (
...
def test_backward_compatibility():
    ...
    for method in original_abstract_methods:
        assert (
            method in interface_abstracts
        ), f"Original method {method} missing from abstract methods"
    ...
    for method in text_original_abstracts:
        assert (
            method in text_interface_abstracts
        ), f"Text model method {method} missing from abstract methods"
