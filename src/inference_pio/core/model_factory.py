...
class ModelFactory:
    ...
    def create_model(
        ...
        resolved_path = ModelLoader.resolve_model_path(model_name, hf_repo_id=hf_id)
        ...
        try:
            if "qwen3_0_6b" in model_name or "qwen3_0.6b" in model_name:
                ...
                raise ValueError(
                    f"Unsupported model name: {model_name}. Available models: qwen3-0.6b, qwen3-vl-2b, glm-4-7-flash, qwen3-4b, qwen3-coder"
                )
            ...
            raise ImportError(
                f"Model plugin for {model_name} could not be loaded. Ensure dependencies are installed."
            ) from e
...
def get_model_class(model_name: str) -> Type[ModelPluginInterface]:
    ...
    # For now, we'll raise an exception since this functionality isn't fully implemented
...
__all__ = ["ModelFactory", "create_model", "get_model_class", "register_model"]
