"""
Test suite for Intelligent Scheduling System for Qwen3-0.6B Model.
"""
...
from src.inference_pio.models.qwen3_0_6b.scheduling.intelligent_scheduler import (
...
class TestIntelligentSchedulerIntegration(unittest.TestCase):
    ...
    def test_create_scheduler_for_qwen3_0_6b(self):
        """Test creating an intelligent scheduler specifically for Qwen3-0.6B."""
        mock_config = MagicMock()
        mock_config.intelligent_scheduling_max_concurrent_ops = 12
        mock_config.intelligent_scheduling_policy = 'intelligent'
        mock_config.intelligent_scheduling_enable_prediction = True
        mock_config.intelligent_scheduling_prediction_horizon = 8
        mock_config.intelligent_scheduling_enable_adaptive = True
        mock_config.intelligent_scheduling_adaptive_window = 75
        mock_config.intelligent_scheduling_enable_resource_opt = True
        mock_config.intelligent_scheduling_resource_buffer = 0.08
        mock_config.intelligent_scheduling_enable_priority_boost = True
        mock_config.intelligent_scheduling_priority_decay = 0.95
        mock_config.intelligent_scheduling_enable_load_balancing = True
        mock_config.intelligent_scheduling_load_balance_interval = 0.12
        mock_config.intelligent_scheduling_performance_log_interval = 40

        scheduler = create_intelligent_scheduler_for_qwen3_0_6b(mock_config)
        
        self.assertIsInstance(scheduler, IntelligentOperationScheduler)
        self.assertEqual(scheduler.config.max_concurrent_ops, 12)
        ...
        self.assertEqual(scheduler.config.prediction_horizon, 8)
