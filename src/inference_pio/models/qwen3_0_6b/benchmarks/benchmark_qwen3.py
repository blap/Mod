...
def benchmark_qwen3():
    ...
    for prompt in prompts:
        ...
        if hasattr(plugin, "tokenize"):
            ...
            if isinstance(input_ids[0], list) or (
                torch.is_tensor(input_ids) and input_ids.ndim > 1
            ):
                input_ids = input_ids[0]

            output_ids = plugin.tokenize(output)["input_ids"]
            if isinstance(output_ids[0], list) or (
                torch.is_tensor(output_ids) and output_ids.ndim > 1
            ):
