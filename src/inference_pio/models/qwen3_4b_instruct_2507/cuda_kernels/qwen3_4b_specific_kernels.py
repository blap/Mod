...
class Qwen34BAttentionKernel(nn.Module):
    ...
    def forward(
        ...
        v = v.view(batch_size, seq_len, self.head_dim, self.nhead).transpose(1, 2).contiguous().view(
            batch_size, seq_len, self.nhead * self.head_dim
        ).view(batch_size, seq_len, self.nhead, self.head_dim).transpose(1, 2)
...
def create_qwen3_4b_cuda_kernels(d_model: int,
                               nhead: int,
                               intermediate_size: int,
                               use_flash_attention: bool = True) -> Dict[str, nn.Module]:
    """
    ...
    kernels['rmsnorm'] = Qwen34BRMSNormKernel(
...
def apply_qwen3_4b_specific_cuda_optimizations_to_model(model: nn.Module,
                                                      d_model: int,
                                                      nhead: int,
                                                      intermediate_size: int,
                                                      config) -> nn.Module:
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, nn.MultiheadAttention):
            ...
            logger.info(f"Replaced attention module {name} with Qwen3-4B optimized kernel")
        ...
        # Replace LayerNorm modules with Qwen3-4B RMSNorm
            ...
            logger.info(f"Replaced normalization module {name} with Qwen3-4B RMSNorm kernel")

        # Replace MLP/feed-forward modules with Qwen3-4B optimized versions
        elif hasattr(module, 'forward') and any(name_part in name.lower() for name_part in ['mlp', 'ffn', 'feed_forward', 'intermediate']):
            ...
            if hasattr(module, 'fc1') and hasattr(module, 'fc2'):
                ...
                logger.info(f"Replaced MLP module {name} with Qwen3-4B optimized kernel")
...
def get_qwen3_4b_cuda_optimization_report(model: nn.Module,
                                        d_model: int,
                                        nhead: int,
                                        intermediate_size: int,
                                        config) -> Dict:
    ...
    attention_count = sum(1 for m in model.modules() if isinstance(m, nn.MultiheadAttention))
    ...
    mlp_count = sum(1 for name, m in model.named_modules()
                   if hasattr(m, 'forward') and any(part in name.lower() for part in ['mlp', 'ffn', 'feed_forward']))
