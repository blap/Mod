"""
Qwen3-4B-Instruct-2507 Paged KV Cache Implementation

This module implements a vLLM-style paged KV cache for efficient memory management.
It divides the KV cache into fixed-size pages managed by a page table.
"""
...
class PagedKVCache:
    """
    Paged KV cache implementation for Qwen3-4B-Instruct-2507 model.
    """
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized Qwen3-4B PagedKVCache with {max_num_blocks} blocks of size {block_size}"
        )
    ...
    def append(
        ...
        Args:
            ...
            seq_lens: Length of each sequence (including new tokens)
        """
        batch_size, seq_len, num_heads, head_dim = key.shape

        # Split sequences into blocks
        for batch_idx in range(batch_size):
            # The seq_lens passed here typically includes the new tokens.
            # We need to map the new tokens to the correct blocks.
            # Assuming 'key' contains only the *new* tokens to be appended.

            # NOTE: For this implementation, we assume 'key' is just the new tokens (usually length 1 or chunk)
            # and 'seq_lens' is the *total* length after appending.
            ...
            # Fill blocks
            # This logic iterates through the new tokens and places them into the correct physical block
            # retrieved from block_tables[batch_idx]
            ...
            while tokens_processed < num_new_tokens:
                ...
                if block_idx >= len(block_tables[batch_idx]):
                    # This should be handled by the scheduler/allocator before calling append,
                    # but we can auto-allocate here if strictly necessary.
                    # Ideally, the scheduler guarantees block_tables has enough entries.
                    raise IndexError(
                        f"Block table too short for sequence length {current_total_len}"
                    )
                ...
                num_to_copy = min(
                    num_new_tokens - tokens_processed, self.block_size - block_offset
                )
                ...
                self.key_cache[
                    ...
                    batch_idx, tokens_processed : tokens_processed + num_to_copy, :, :
                ]
    ...
    def get_kv_cache(
        ...
        """
        Retrieve the logical KV cache for the given sequences (for debugging/verification or standard attention).

        NOTE: For highly optimized kernels (FlashAttention with Paging), you normally pass
        block_tables directly to the kernel instead of reconstructing the big tensor.
        This function reconstructs the contiguous tensor, which is slower but compatible with standard SDPA.
        """
