...
class Qwen34BSlidingWindowAttention(BaseCausalAttention):
    ...
    def forward(
        ...
        q = q.view(bsz, tgt_len, self.num_heads, self.head_dim).transpose(
            1, 2
        ...
        if self.num_key_value_heads == self.num_heads:
            # Standard MHA
            k = k.view(bsz, src_len, self.num_heads, self.head_dim).transpose(
                1, 2
            )  # (bsz, num_heads, src_len, head_dim)
            v = v.view(bsz, src_len, self.num_heads, self.head_dim).transpose(
                1, 2
            )  # (bsz, num_heads, src_len, head_dim)
        else:
            # GQA/MQA: repeat K and V for grouped attention
            k = k.view(bsz, src_len, self.num_key_value_heads, self.head_dim).transpose(
                1, 2
            )  # (bsz, num_key_value_heads, src_len, head_dim)
            v = v.view(bsz, src_len, self.num_key_value_heads, self.head_dim).transpose(
                1, 2
            )  # (bsz, num_key_value_heads, src_len, head_dim)
        ...
        sliding_window_mask = self._create_sliding_window_mask(
            tgt_len, src_len, self.window_size
        )
        sliding_window_mask = sliding_window_mask.to(attn_weights.device).to(
            attn_weights.dtype
        )
    ...
    def _create_sliding_window_mask(
        self, tgt_len: int, src_len: int, window_size: int
    ) -> torch.Tensor:
        """
...
__all__ = ["Qwen34BSlidingWindowAttention", "create_qwen3_4b_sliding_window_attention"]
