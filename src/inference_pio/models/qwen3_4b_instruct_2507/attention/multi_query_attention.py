    ...
    def __init__(
        ...
        if use_gqa and num_key_value_groups is not None:
            self.num_key_value_heads = (
                config.num_attention_heads // num_key_value_groups
            )
        else:
        ...
        if not output_attentions and hasattr(
            ...
            if attention_mask is None:
                causal_mask = torch.tril(
                    torch.ones(
                        (tgt_len, src_len), dtype=torch.bool, device=attn_weights.device
                    )
...
def create_mqa_gqa_attention(
    ...
    return Qwen34BMultiQueryAttention(
        config, layer_idx, use_gqa=use_gqa, num_key_value_groups=num_key_value_groups
    )


__all__ = ["Qwen34BMultiQueryAttention", "create_mqa_gqa_attention"]
