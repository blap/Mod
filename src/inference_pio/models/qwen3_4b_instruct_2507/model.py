...
class Qwen34BInstruct2507Model(nn.Module):
    """
    Qwen3-4B-Instruct-2507 model implementation with all optimizations integrated.

    This is the main model class for the Qwen3-4B-Instruct-2507 model with all optimizations
    ...
    def __init__(self, config: Qwen34BInstruct2507Config):
        ...
        # Memory optimization settings specific to Qwen3-4B-Instruct-2507 model
        self._memory_config = {
        ...
        self._processor_plugin.initialize(
            {"num_threads": self._system_profile.get_cpu_allocation()}
        )
        ...
        if getattr(config, "enable_continuous_nas", False):
            ...
            nas_config = NASConfig(
                ...
                accuracy_tradeoff_factor=getattr(
            ...
            self._model_adapter = (
                get_model_adapter(self._model, self._nas_controller)
                if self._model
                else None
            )
        ...
        if self._system_profile.is_weak_hardware:
            ...
            logger.info(
                "Weak hardware detected. Forcing disk offloading enabled for Qwen3-4B."
            )
    ...
    def _initialize_disk_offloading(self):
        ...
        try:
            logger.info(
                "Initializing advanced disk offloading system for Qwen3-4B-Instruct-2507 model..."
            )
            ...
            self._disk_offloader = create_disk_offloader(
                ...
                enable_adaptive=getattr(
                    self.config, "enable_adaptive_offloading", True
                ),
            )
    ...
    def _initialize_pagination_system(self):
        ...
        try:
            logger.info(
                "Initializing intelligent pagination system for Qwen3-4B-Instruct-2507 model..."
            )

            # Create pagination system with advanced settings
            self._pagination_system, self._unimodal_pager = (
                create_unimodal_pagination_system(
                    swap_directory=getattr(
                        self.config, "pagination_swap_directory", "./text_tensor_swap"
                    ),
                    page_size_mb=getattr(self.config, "pagination_page_size_mb", 16),
                    eviction_policy=getattr(
                        self.config, "pagination_eviction_policy", "intelligent"
                    ),
                    max_memory_ratio=getattr(
                        self.config, "pagination_max_memory_ratio", 0.8
                    ),
                )
            )
            ...
            logger.info(
                "Intelligent pagination system for unimodal text data initialized successfully"
            )
    ...
    def _apply_disk_offloading_to_model(self):
        ...
        if not self._tensor_offloader:
            logger.warning(
                "Disk offloading not initialized, skipping offloading application"
            )
            return

        try:
            logger.info(
                "Applying disk offloading to Qwen3-4B-Instruct-2507 model components..."
            )
            ...
            offload_results = self._tensor_offloader.offload_model_components(
                self._model,
                component_filter=(
                    components_to_offload if components_to_offload else None
                ),
                priority=self._get_offload_priority(
                    getattr(self.config, "offloading_priority", "medium")
                ),
            )

            successful_offloads = sum(
                1 for success in offload_results.values() if success
            )
            total_components = len(offload_results)

            logger.info(
                f"Disk offloading applied: {successful_offloads}/{total_components} components offloaded successfully"
            )
    ...
    def _initialize_model(self):
        """
        Initialize the Qwen3-4B-Instruct-2507 model with appropriate optimizations.
        """
        try:
            logger.info(
                f"Loading Qwen3-4B-Instruct-2507 model from: {self._model_name}"
            )
            ...
            if self._system_profile.is_weak_hardware:
                ...
                if safe_limit_mb > 0:
                    ...
                    logger.info(
                        f"Qwen3-4B: Applying safe VRAM limit: {safe_limit_mb} MB"
                    )
                else:
            ...
            # Handle torch_dtype string conversion to actual torch.dtype
            if isinstance(load_kwargs["torch_dtype"], str):
                if load_kwargs["torch_dtype"] == "float16":
                    load_kwargs["torch_dtype"] = torch.float16
                elif load_kwargs["torch_dtype"] == "float32":
                    load_kwargs["torch_dtype"] = torch.float32
                elif load_kwargs["torch_dtype"] == "bfloat16":
                    load_kwargs["torch_dtype"] = torch.bfloat16
                else:
                    # Default to float16 for unknown types
                    load_kwargs["torch_dtype"] = torch.float16

            # Remove problematic offload options that might cause the disk offload error
            # This is a workaround for the specific error we're seeing
            if "disk" in str(load_kwargs.get("device_map", "")) or (
                isinstance(load_kwargs.get("max_memory"), dict)
                and any(
                    "disk" in str(v) for v in load_kwargs.get("max_memory", {}).values()
                )
            ):
                # If disk offloading is detected, we'll handle it differently
                if "max_memory" in load_kwargs:
                    # Only keep RAM limits, remove disk limits
                    ram_only_memory = {}
                    for k, v in load_kwargs["max_memory"].items():
                        if "disk" not in str(v):
                            ram_only_memory[k] = v
                    if ram_only_memory:
                        load_kwargs["max_memory"] = ram_only_memory
                    else:
                        del load_kwargs["max_memory"]

            # Try to load the model from the local path
            try:
                # Check if the model path is a local directory that might cause disk offloading
                import os
                ...
                if (
                    drive_exists
                    and os.path.exists(self._model_name)
                    and os.path.isdir(self._model_name)
                ):
                    # If it's a local directory, we need to be careful about device mapping
                    # Remove device_map temporarily to avoid automatic disk offloading
                    temp_device_map = load_kwargs.pop("device_map", None)
                    temp_max_memory = load_kwargs.pop("max_memory", None)

                    self._model = AutoModelForCausalLM.from_pretrained(
                        self._model_name, **load_kwargs
                    )

                    # If we removed device_map, try to move model to appropriate device after loading
                    if temp_device_map and temp_device_map != "auto":
                        try:
                            if temp_device_map == "cpu":
                                self._model = self._model.cpu()
                            elif (
                                temp_device_map == "cuda" and torch.cuda.is_available()
                            ):
                                self._model = self._model.cuda()
                            else:
                                # For other device maps, try to apply them after loading
                                self._model = self._model.to(temp_device_map)
                        except Exception as e:
                            logger.warning(
                                f"Could not apply device_map {temp_device_map}: {e}. Using default device."
                            )
                            # Put it back in the kwargs for next attempt
                            load_kwargs["device_map"] = temp_device_map
                            if temp_max_memory:
                                load_kwargs["max_memory"] = temp_max_memory
                    elif temp_device_map == "auto":
                        # Re-add auto device_map for the model to handle automatically
                        load_kwargs["device_map"] = temp_device_map
                        if temp_max_memory:
                            load_kwargs["max_memory"] = temp_max_memory

                else:
                    # Not a local directory, proceed normally
                    self._model = AutoModelForCausalLM.from_pretrained(
                        self._model_name, **load_kwargs
                    )
            except OSError:
                ...
                # Restore device_map if it was removed for local loading
                if "device_map" not in load_kwargs:
                    load_kwargs["device_map"] = self._memory_config.get(
                        "device_map", "auto"
                    )
                if "max_memory" not in load_kwargs and self._memory_config.get(
                    "max_memory"
                ):
                    load_kwargs["max_memory"] = self._memory_config["max_memory"]

                self._model = AutoModelForCausalLM.from_pretrained(
                    "H:/Qwen3-4B-Instruct-2507",  # Local model path on drive H
                    **load_kwargs,
                )
                self._model_name = "H:/Qwen3-4B-Instruct-2507"
            except ValueError as e:
                if "You are trying to offload the whole model to the disk" in str(e):
                    # If disk offloading error occurs, retry without device_map
                    logger.warning(
                        f"Disk offloading error occurred: {e}. Retrying without device_map..."
                    )
                    load_kwargs_no_device = {
                        k: v
                        for k, v in load_kwargs.items()
                        if k not in ["device_map", "max_memory"]
                    }
                    self._model = AutoModelForCausalLM.from_pretrained(
                        self._model_name, **load_kwargs_no_device
                    )
                    # Then move to desired device if possible
            ...
            if self._nas_controller is not None:
                self._model_adapter = get_model_adapter(
                    self._model, self._nas_controller
                )

        except Exception as e:
            logger.error(f"Failed to initialize Qwen3-4B-Instruct-2507 model: {e}")
            raise
    ...
    def _apply_configured_optimizations(self):
        ...
        # Apply Qwen3-4B-Instruct-2507 specific optimizations
    ...
    def _apply_traditional_optimizations(self):
        ...
        if getattr(self.config, "use_tensor_decomposition", False):
            ...
            self._apply_tensor_decomposition(
                rank_ratio=rank_ratio, decomposition_method=decomposition_method
            )
    ...
    def _apply_qwen3_specific_optimizations(self):
        ...
        """
        logger.info("Applying Qwen3-4B-Instruct-2507 specific optimizations...")

        # Apply Qwen3-specific attention optimizations
        if getattr(self.config, "use_qwen3_attention_optimizations", True):
            logger.info("Applying Qwen3-specific attention optimizations...")
            self._model = apply_qwen3_attention_optimizations(self._model, self.config)

        # Apply Qwen3-specific KV-cache optimizations
        if getattr(self.config, "use_qwen3_kv_cache_optimizations", True):
            logger.info("Applying Qwen3-specific KV-cache optimizations...")
            self._model = apply_qwen3_kv_cache_optimizations(self._model, self.config)

        # Apply Qwen3-specific instruction tuning optimizations
        if getattr(self.config, "use_qwen3_instruction_optimizations", True):
            logger.info("Applying Qwen3-specific instruction tuning optimizations...")
            self._model = apply_qwen3_instruction_tuning_optimizations(
                self._model, self.config
            )
            self._model = apply_qwen3_generation_optimizations(self._model, self.config)

        # Apply Qwen3-specific RoPE optimizations
        if getattr(self.config, "use_qwen3_rope_optimizations", True):
            logger.info("Applying Qwen3-specific RoPE optimizations...")
            self._model = apply_qwen3_rope_optimizations(self._model, self.config)

        # Apply Qwen3-specific GQA optimizations
        if getattr(self.config, "use_qwen3_gqa_optimizations", True):
            logger.info("Applying Qwen3-specific GQA optimizations...")
            self._model = apply_qwen3_gqa_optimizations(self._model, self.config)

        logger.info(
            "All Qwen3-4B-Instruct-2507 specific optimizations applied successfully"
        )
    ...
    def _apply_tensor_parallelism(self):
        ...
        try:
            ...
            if tensor_parallel_size > 1:
                logger.info(
                    f"Applying tensor parallelism with size {tensor_parallel_size}"
                )
                ...
                self._model, success, error_msg = safe_convert_to_tensor_parallel(
                    self._model, tp_config
                )
                if not success:
                    logger.warning(
                        f"Failed to convert model to tensor parallel: {error_msg}"
                    )
                    logger.info(
                        "Disabling tensor parallelism and proceeding with regular model"
                    )
                    self.config.use_tensor_parallelism = False
                else:
                    logger.info(
                        f"Model converted to tensor parallel with size {tensor_parallel_size}"
                    )
        except Exception as e:
    ...
    def _apply_flash_attention_2_optimization(self):
        ...
        try:
            ...
            if use_paged_attention:
                logger.info(
                    "Applying paged attention optimization to Qwen3-4B-Instruct-2507 model..."
                )
                self._apply_paged_attention_optimization()
            elif use_sliding_window_attention:
                logger.info(
                    "Applying sliding window attention optimization to Qwen3-4B-Instruct-2507 model..."
                )
                self._apply_sliding_window_attention_optimization()
            elif use_multi_query_attention or use_grouped_query_attention:
                logger.info(
                    "Applying Multi-Query/Grouped-Query attention optimization to Qwen3-4B-Instruct-2507 model..."
                )
                self._apply_mqa_gqa_optimization()
            elif use_sparse_attention:
                logger.info(
                    "Applying sparse attention optimization to Qwen3-4B-Instruct-2507 model..."
                )
                self._apply_sparse_attention_optimization()
            elif use_flash_attention:
                logger.info(
                    "Applying FlashAttention 2.0 optimization to Qwen3-4B-Instruct-2507 model..."
                )
                self._apply_flash_attention_optimization()
            else:
                logger.info(
                    "Attention optimization disabled for Qwen3-4B-Instruct-2507 model"
                )

            # Always apply optimized rotary embeddings regardless of attention optimization choice
            logger.info(
                "Applying optimized rotary embeddings to Qwen3-4B-Instruct-2507 model..."
            )
    ...
    def _apply_sparse_attention_optimization(self):
        ...
        try:
            ...
            if hasattr(self._model, "transformer") and hasattr(
                self._model.transformer, "layers"
            ):
                # Qwen3-4B-Instruct-2507-style architecture
            ...
            elif hasattr(self._model, "transformer") and hasattr(
                self._model.transformer, "h"
            ):
                ...
                if hasattr(layer, "self_attn") or hasattr(layer, "attn"):
                    try:
                        ...
                        if hasattr(layer, "self_attn"):
                            ...
                            if getattr(
                                self.config, "use_adaptive_sparse_attention", False
                            ):
                                ...
                                sparse_attn = create_qwen3_4b_sparse_attention(
                                    self.config, layer_idx=idx
                                )
                        ...
                        replaced_count += 1
                        logger.debug(
                            f"Replaced attention with sparse attention in layer {idx}"
                        )
                    except Exception as layer_e:
                        logger.warning(
                            f"Could not replace attention in layer {idx}: {layer_e}"
                        )
                        continue

            logger.info(
                f"Sparse attention optimization applied: {replaced_count}/{total_layers} attention layers replaced"
            )

        except ImportError as e:
            logger.warning(
                f"Sparse attention not available, skipping optimization: {e}"
            )
        except Exception as e:
    ...
    def _apply_flash_attention_optimization(self):
        ...
        try:
                ...
                if hasattr(layer, "self_attn") or hasattr(layer, "attn"):
                    try:
                        ...
                        if hasattr(layer, "self_attn"):
                            ...
                            flash_attn = create_qwen3_4b_flash_attention_2(
                                self.config, layer_idx=idx
                            )
            ...
            logger.info(
                f"FlashAttention 2.0 optimization applied: {replaced_count}/{total_layers} attention layers replaced"
            )

        except ImportError as e:
            logger.warning(
                f"FlashAttention 2.0 not available, skipping optimization: {e}"
            )
        except Exception as e:
    ...
    def _apply_sliding_window_attention_optimization(self):
        ...
        try:
            ...
            for idx, layer in enumerate(layers):
                ...
                if hasattr(layer, "self_attn") or hasattr(layer, "attn"):
                    try:
                        ...
                        replaced_count += 1
                        logger.debug(
                            f"Replaced attention with sliding window attention in layer {idx}"
                        )
                    except Exception as layer_e:
                        logger.warning(
                            f"Could not replace attention in layer {idx}: {layer_e}"
            ...
            logger.info(
                f"Sliding window attention optimization applied: {replaced_count}/{total_layers} attention layers replaced"
            )

        except ImportError as e:
            logger.warning(
                f"Sliding window attention not available, skipping optimization: {e}"
            )
        except Exception as e:
    ...
    def _apply_mqa_gqa_optimization(self):
        ...
        try:
                ...
                if hasattr(layer, "self_attn") or hasattr(layer, "attn"):
                    try:
                        ...
                        if hasattr(layer, "self_attn"):
                            ...
                            mqa_gqa_attn = create_mqa_gqa_attention(
                                self.config, layer_idx=idx
                            )
            ...
            logger.info(
                f"MQA/GQA optimization applied: {replaced_count}/{total_layers} attention layers replaced"
            )

        except ImportError as e:
            logger.warning(
                f"MQA/GQA attention not available, skipping optimization: {e}"
            )
        except Exception as e:
    ...
    def _apply_paged_attention_optimization(self):
        ...
        try:
                ...
                if hasattr(layer, "self_attn") or hasattr(layer, "attn"):
                    try:
                        ...
                        if hasattr(layer, "self_attn"):
                            ...
                            paged_attn = create_qwen3_4b_paged_attention(
                                self.config,
                                layer_idx=idx,
                        ...
                        replaced_count += 1
                        logger.debug(
                            f"Replaced attention with paged attention in layer {idx}"
                        )
                    except Exception as layer_e:
                        logger.warning(
                            f"Could not replace attention in layer {idx}: {layer_e}"
            ...
            logger.info(
                f"Paged attention optimization applied: {replaced_count}/{total_layers} attention layers replaced"
            )
                ...
                if hasattr(layer, "self_attn") or hasattr(layer, "attn"):
                    try:
                        ...
                        if hasattr(attn_module, "rotary_emb"):
                            ...
                            optimized_emb = Qwen34BRotaryEmbedding(
                                dim=(
                                    original_emb.dim
                                    if hasattr(original_emb, "dim")
                                    else self.config.hidden_size
                                    // self.config.num_attention_heads
                                ),
                                max_position_embeddings=self.config.max_position_embeddings,
                                base=self.config.rope_theta,
                                precision=self._memory_config.get(
                            ...
                            # This is for models that compute rotary embeddings differently
                            head_dim = (
                            ...
                            replaced_count += 1
                            logger.debug(
                                f"Added optimized rotary embedding in layer {idx}"
                            )

                    except Exception as layer_e:
                        logger.warning(
                            f"Could not replace/add rotary embedding in layer {idx}: {layer_e}"
                        )
                        continue

            logger.info(
                f"Optimized rotary embedding applied: {replaced_count}/{total_layers} attention layers updated"
            )

        except ImportError as e:
            logger.warning(
                f"Optimized rotary embedding not available, skipping optimization: {e}"
            )
        except Exception as e:
    ...
    def _apply_fused_layer_norm(self):
        ...
        try:
            logger.info(
                "Applying fused layer normalization optimization to Qwen3-4B-Instruct-2507 model..."
            )
            ...
            logger.info(
                f"Fused layer normalization applied: {original_norm_count} LayerNorm modules replaced"
            )
    ...
    def _apply_bias_removal_optimization(self):
        ...
        try:
            logger.info(
                "Applying bias removal optimization to Qwen3-4B-Instruct-2507 model..."
            )
            self._model, bias_report = apply_bias_removal_to_model(
                self._model, model_type="qwen3_4b"
            )
    ...
    def _apply_kv_cache_compression(self):
        ...
        try:
            logger.info(
                "Applying KV-cache compression to Qwen3-4B-Instruct-2507 model..."
            )
            from .kv_cache.compression_techniques import CompressedKVCacheConfig
            ...
            logger.warning(
                f"KV-cache compression not available, skipping optimization: {e}"
            )
        except Exception as e:
    ...
    def _apply_prefix_caching(self):
        ...
        """
        try:
            logger.info("Applying prefix caching to Qwen3-4B-Instruct-2507 model...")

            # Create prefix cache configuration
            from .prefix_caching.prefix_cache_manager import PrefixCacheConfig
            ...
            cache_precision = precision_map.get(
                self.config.prefix_cache_precision, torch.float16
            )
    ...
    def _apply_cuda_kernels(self):
        ...
        try:
            logger.info(
                "Applying CUDA kernels optimization to Qwen3-4B-Instruct-2507 model..."
            )
            ...
            # Apply existing Qwen3-4B specific CUDA kernel optimizations
            self._model = apply_qwen3_4b_optimizations_to_model(
                self._model, self.config
            )
    ...
    def _apply_linear_bias_optimization(self):
        ...
        try:
            logger.info(
                "Applying linear bias optimization to Qwen3-4B-Instruct-2507 model..."
            )
    ...
    def _apply_structured_pruning(
        ...
        try:
            logger.info(
                f"Applying structured pruning to Qwen3-4B-Instruct-2507 model with ratio {pruning_ratio}, "
            ...
            logger.info(
                f"Structured pruning completed: {pruning_result.compression_ratio:.2%} "
                f"compression achieved, accuracy preserved: {pruning_result.accuracy_preserved}"
            )
            logger.info(
                f"Removed {len(pruning_result.removed_layers)} layers/components"
            )
    ...
    def _apply_tensor_decomposition(
        self, rank_ratio: float = 0.5, decomposition_method: str = "cp_decomposition"
    ):
        """
        ...
        try:
            logger.info(
                f"Applying tensor decomposition to Qwen3-4B-Instruct-2507 model with rank_ratio {rank_ratio}, "
            ...
            total_params_after = (
                total_params_before  # Placeholder - actual implementation would vary
            )

            logger.info(
                f"Tensor decomposition applied: rank_ratio={rank_ratio}, "
                f"method={decomposition_method}"
            )
            logger.info(
                f"Note: Actual weight replacement requires model-specific wrapper implementation"
            )
    ...
    def _initialize_pipeline_parallelism(self):
        ...
        try:
            logger.info(
                "Initializing pipeline parallelism for Qwen3-4B-Instruct-2507 model..."
            )

            # Get pipeline configuration from model config
            num_pipeline_stages = getattr(
                self.config, "pipeline_parallel_num_stages", 1
            )
            pipeline_microbatch_size = getattr(
                self.config, "pipeline_parallel_microbatch_size", 1
            )
            pipeline_schedule = getattr(
                self.config, "pipeline_parallel_schedule", "1f1b"
            )
            enable_activation_offloading = getattr(
                self.config, "pipeline_parallel_enable_activation_offloading", True
            )
            ...
            self._pipeline_parallel_model = PipelineParallel(
                model=self._model, config=pipeline_config
            )

            logger.info(
                f"Pipeline parallelism initialized with {num_pipeline_stages} stages"
            )
    ...
    def _initialize_sequence_parallelism(self):
        ...
        try:
            logger.info(
                "Initializing sequence parallelism for Qwen3-4B-Instruct-2507 model..."
            )

            # Get sequence parallelism configuration from model config
            num_sequence_segments = getattr(
                self.config, "sequence_parallel_num_segments", 1
            )
            sequence_split_method = getattr(
                self.config, "sequence_parallel_split_method", "chunk"
            )
            enable_sequence_overlap = getattr(
                self.config, "sequence_parallel_enable_overlap", True
            )
            overlap_size = getattr(self.config, "sequence_parallel_overlap_size", 64)
            sequence_algorithm = getattr(
                self.config, "sequence_parallel_algorithm", "1d"
            )
            ...
            self._sequence_parallel_model = SequenceParallel(
                model=self._model, config=sequence_config
            )

            logger.info(
                f"Sequence parallelism initialized with {num_sequence_segments} segments"
            )
    ...
    def _apply_quantization(self):
        ...
        """
        try:
            logger.info("Applying quantization to Qwen3-4B-Instruct-2507 model...")
            ...
            if quantization_scheme.lower() == "int4":
                ...
                logger.warning(
                    f"Unknown quantization scheme: {quantization_scheme}, defaulting to INT8"
                )
                scheme = QuantizationScheme.INT8
                bits = 8

            quant_config = QuantizationConfig(
                scheme=scheme, bits=bits, symmetric=symmetric, per_channel=per_channel
            )
            ...
            logger.info(
                f"Quantization applied successfully with scheme: {quantization_scheme}"
            )
            ...
            estimated_compressed_size = sum(
                p.numel() * (bit_size // 8) for p in self._model.parameters()
            )
            memory_reduction = (
                (original_size - estimated_compressed_size) / original_size * 100
            )
    ...
    def _apply_snn_conversion(self):
        ...
        try:
            logger.info(
                "Applying SNN conversion to Qwen3-4B-Instruct-2507 model for energy efficiency..."
            )
            ...
            snn_config = {
                ...
                "temporal_encoding": getattr(
                    self.config, "snn_temporal_encoding", False
                ),
            }
            ...
            if getattr(self.config, "enable_snn_optimizations", False):
                optimization_config = {
                    ...
                    "quantization_bits": getattr(
                        self.config, "snn_quantization_bits", 8
                    ),
                    "temporal_sparsity": getattr(
                        self.config, "snn_temporal_sparsity", True
                    ),
                    "neural_efficiency": getattr(
                        self.config, "snn_neural_efficiency", True
                    ),
                }
            ...
            try:
                if hasattr(self._model, "config"):
                    ...
                    energy_estimation = estimate_energy_savings(
    ...
    def _apply_intelligent_pagination_to_model(self):
        ...
        if not self._unimodal_pager:
            logger.warning(
                "Intelligent pagination not initialized, skipping pagination application"
            )
            return

        try:
            logger.info(
                "Applying intelligent pagination to Qwen3-4B-Instruct-2507 model components..."
            )
            ...
            for name, param in self._model.named_parameters():
                ...
                if any(
                    ...
                    for skip_name in ["norm", "ln", "embedding"]
                ):
                    continue
            ...
            logger.info(
                f"Intelligent pagination setup: {successful_paginations}/{total_components} components prepared for pagination"
            )
    ...
    def _initialize_caching(self):
        ...
        try:
            logger.info(
                "Initializing intelligent unimodal caching for Qwen3-4B-Instruct-2507 model..."
            )

            # Get cache configuration from model config
            cache_size_mb = getattr(self.config, "intelligent_cache_size_mb", 512.0)
            eviction_policy_str = getattr(
                self.config, "intelligent_cache_eviction_policy", "predictive"
            )
            enable_similarity_caching = getattr(
                self.config, "intelligent_cache_enable_similarity", True
            )
            similarity_threshold = getattr(
                self.config, "intelligent_cache_similarity_threshold", 0.85
            )
            enable_ttl = getattr(self.config, "intelligent_cache_enable_ttl", True)
            default_ttl = getattr(
                self.config, "intelligent_cache_default_ttl", 3600.0
            )  # 1 hour
            enable_compression = getattr(
                self.config, "intelligent_cache_enable_compression", True
            )
            compression_ratio = getattr(
                self.config, "intelligent_cache_compression_ratio", 0.6
            )
            ...
            eviction_policy = policy_map.get(
                eviction_policy_str.lower(), CacheEvictionPolicy.PREDICTIVE
            )

            # Create caching manager with Qwen3-4B specific configurations
            self._caching_manager = create_unimodal_caching_manager(
                cache_size_mb=cache_size_mb, language_model_type="qwen3_4b_instruct"
            )

            # Apply caching to the model
            self._model = apply_intelligent_unimodal_caching_to_model(
                model=self._model, caching_manager=self._caching_manager
            )

            logger.info(
                f"Intelligent unimodal caching initialized successfully for Qwen3-4B-Instruct-2507 model "
    ...
    def _initialize_async_processing(self):
        ...
        try:
            logger.info(
                "Initializing async unimodal processing for Qwen3-4B-Instruct-2507 model..."
            )

            # Create async manager for the model
            self._async_manager = AsyncUnimodalManager(
                model=self._model, config=self.config, model_type="qwen3_4b"
            )
            ...
            if success:
                logger.info(
                    "Async unimodal processing initialized successfully for Qwen3-4B-Instruct-2507 model"
                )
            else:
                logger.error(
                    "Failed to initialize async unimodal processing for Qwen3-4B-Instruct-2507 model"
                )
                self._async_manager = None
    ...
    def _access_paginated_tensor(self, tensor_id: str):
        ...
        if not self._unimodal_pager:
            logger.warning(
                "Unimodal pager not initialized, cannot access paginated tensor"
            )
            return None
