...
def _apply_qwen3_sparse_attention_optimizations(
...
def _apply_qwen3_gqa_optimizations(model: nn.Module, config: Any) -> nn.Module:
    ...
    for name, module in model.named_modules():
        # Look for attention modules by attribute rather than specific class
        if hasattr(module, "num_key_value_groups") and hasattr(
...
def _apply_qwen3_flash_attention_optimizations(
    model: nn.Module, config: Any
) -> nn.Module:
    """
        ...
        if hasattr(module, "num_key_value_groups") and hasattr(module, "is_causal"):
            ...
            if module.num_key_value_groups > 1:
                ...
                if hasattr(module, "softmax_scale"):
                    ...
                    module.softmax_scale = getattr(config, "rope_theta", 1000000.0) ** (
                        -1.0
                    )
            ...
            if hasattr(rotary_emb, "inv_freq"):
                ...
                try:
                    ...
                    inv_freq = 1.0 / (
                        config.rope_theta
                        ** (
                            torch.arange(0, emb_dim, 2, dtype=torch.int64).float()
                            / emb_dim
                        )
                    )
        ...
        if (
            ...
            if hasattr(module, "hidden_size") and hasattr(module, "num_heads"):
                ...
                if module.hidden_size != config.hidden_size:
                    logger.warning(
                        f"Mismatch in hidden size for {name}: module={module.hidden_size}, config={config.hidden_size}"
                    )
