...
from src.inference_pio.models.qwen3_4b_instruct_2507.kv_cache.paged_kv_cache import PagedKVCache
from src.inference_pio.models.qwen3_4b_instruct_2507.attention.paged_attention import Qwen34BPagedAttention
from src.inference_pio.models.qwen3_4b_instruct_2507.kv_cache.scheduler import ContinuousBatchingScheduler
...
def run_benchmark(num_requests=10, prompt_len=128, gen_len=128, batch_size=4):
    print(f"Running Benchmark: Qwen3-4B (Requests={num_requests}, Prompt={prompt_len}, Gen={gen_len}, Batch={batch_size})")
    ...
    attn = Qwen34BPagedAttention(config, layer_idx=0, kv_cache=kv_cache).to(device).to(dtype)
    ...
    while scheduler.waiting_queue or scheduler.running_queue:
        ...
        # Run Attention
        with torch.no_grad():
            output, _, _ = attn(hidden_states, hidden_states, hidden_states, block_tables=block_tables, seq_lens=seq_lens)
