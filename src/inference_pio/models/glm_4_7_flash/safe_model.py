...
class GLM47SafeModel(nn.Module):
    ...
    def __init__(self, config: GLM47Config):
        ...
        if hasattr(config, "supported_modalities"):
            if (
                "vision" in config.supported_modalities
                or "image" in config.supported_modalities
            ):
                raise ValueError(
                    "GLM-4.7 is a unimodal text model and should not support vision/image modalities"
                )
    ...
    def _validate_unimodal_operation(self, inputs: Any) -> bool:
        ...
        if isinstance(inputs, dict):
            ...
            for key in vision_keys:
                if key in inputs:
                    logger.error(
                        f"Invalid multimodal input detected: '{key}' key found in inputs"
                    )
                    return False
        ...
        if torch.is_tensor(inputs):
            ...
            if len(inputs.shape) == 4:
                logger.warning(
                    "Input tensor has 4 dimensions, which may indicate vision data in text-only model"
                )
                return False
    ...
    def _apply_flash_attention_2_optimization(self):
        ...
        try:
            ...
            if use_paged_attention:
                ...
                logger.info(
                    "Applying sliding window attention optimization to GLM-4.7 model..."
                )
                self._apply_sliding_window_attention_optimization()
            elif use_multi_query_attention or use_grouped_query_attention:
                logger.info(
                    "Applying Multi-Query/Grouped-Query attention optimization to GLM-4.7 model..."
                )
                self._apply_mqa_gqa_optimization()
            elif use_sparse_attention:
                logger.info(
                    "Applying sparse attention optimization to GLM-4.7 model..."
                )
                self._apply_sparse_attention_optimization()
            elif use_flash_attention:
                logger.info(
                    "Applying FlashAttention 2.0 optimization to GLM-4.7 model..."
                )
    ...
    def _apply_fused_layer_norm(self):
        ...
        try:
            logger.info(
                "Applying fused layer normalization optimization to GLM-4.7 model..."
            )
    ...
    def _apply_tensor_decomposition(
        ...
        try:
            logger.info(
                f"Applying tensor decomposition to GLM-4.7 model with rank_ratio {rank_ratio}, "
    ...
    def _apply_structured_pruning(
        ...
        try:
            logger.info(
                f"Applying structured pruning to GLM-4.7 model with ratio {pruning_ratio}, "
    ...
    def _apply_snn_conversion(self):
        ...
        try:
            logger.info(
                "Applying SNN conversion to GLM-4.7 model for energy efficiency..."
            )
