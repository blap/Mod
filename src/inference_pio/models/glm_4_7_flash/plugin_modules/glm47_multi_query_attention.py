...
class GLM47MultiQueryAttention(BaseAttention):
    ...
    def __init__(self, config: GLM47FlashConfig, layer_idx: Optional[int] = None):
        ...
        self.num_key_value_groups = self.num_heads  # Each query head gets its own KV head (but they're identical)
...
class GLM47GroupedQueryAttention(BaseAttention):
    ...
    def __init__(self, config: GLM47FlashConfig, layer_idx: Optional[int] = None):
        ...
        self.num_key_value_groups = getattr(config, 'num_key_value_groups', 4)  # Default to 4 groups
...
class GLM47MQAGQAAttentionSelector(nn.Module):
    ...
    def update_attention_type(self, attention_type: str, num_key_value_groups: Optional[int] = None):
        """
        ...
        if attention_type not in ["mha", "gqa", "mqa"]:
            raise ValueError(f"Invalid attention_type: {attention_type}. Must be 'mha', 'gqa', or 'mqa'.")
        ...
        if attention_type == "mqa":
            ...
            self.attention_impl = GLM47GroupedQueryAttention(self.config, self.layer_idx)
