...
class PagedKVCache:
    ...
    def __init__(
        ...
        logger.info(f"Initialized PagedKVCache with {max_num_blocks} blocks of size {block_size}")
    ...
    def append(
        ...
        for batch_idx in range(batch_size):
            ...
            num_blocks_needed = (seq_len_for_batch + self.block_size - 1) // self.block_size
            
            # Allocate blocks if needed
            if len(block_tables[batch_idx]) < num_blocks_needed:
                new_blocks = self.allocate_blocks(num_blocks_needed - len(block_tables[batch_idx]))
...
class GLM47PagedAttentionCore(nn.Module):
    ...
    def forward(
        ...
        attn_weights = torch.matmul(
            ...
            cached_keys.transpose(1, 2).transpose(-1, -2)  # (batch_size, num_heads, head_dim, cached_seq_len)
        ) / (self.head_dim ** 0.5)
        ...
        output = torch.matmul(
            attn_weights,
            cached_values.transpose(1, 2)  # (batch_size, num_heads, cached_seq_len, head_dim)
        )
    ...
    def _apply_sliding_window(
        ...
        attn_weights = attn_weights.masked_fill(~window_mask, torch.finfo(attn_weights.dtype).min)
