...
class GLM47SlidingWindowAttention(BaseAttention):
    ...
    def _apply_sliding_window_attention(self, query_states, key_states, value_states, attention_mask):
        """
        ...
        for i in range(q_len):
            ...
            window_query = query_states[:, :, i:i+1, :]  # (bsz, num_heads, 1, head_dim)
            window_keys = key_states[:, :, start_idx:end_idx, :]  # (bsz, num_heads, window_size, head_dim)

            # Calculate attention scores for this window
            window_attn_scores = torch.matmul(
                window_query, window_keys.transpose(2, 3)
            ) / math.sqrt(head_dim)  # (bsz, num_heads, 1, window_size)
        ...
        if self.use_causal_mask:
            ...
            causal_mask = causal_mask.unsqueeze(0).unsqueeze(0).expand(bsz, num_heads, q_len, q_len)
        ...
        if self.use_global_attention and self.global_attention_indices:
            ...
            for global_idx in self.global_attention_indices:
                if 0 <= global_idx < q_len:
                    ...
                    global_query = query_states[:, :, global_idx:global_idx+1, :]  # (bsz, num_heads, 1, head_dim)
                    all_keys = key_states  # (bsz, num_heads, q_len, head_dim)
                    
                    global_attn_scores = torch.matmul(
                        global_query, all_keys.transpose(2, 3)
                    ) / math.sqrt(head_dim)  # (bsz, num_heads, 1, q_len)
                    ...
                    global_key = key_states[:, :, global_idx:global_idx+1, :]  # (bsz, num_heads, 1, head_dim)
                    
                    global_attn_scores_reverse = torch.matmul(
                        all_queries, global_key.transpose(2, 3)
                    ) / math.sqrt(head_dim)  # (bsz, num_heads, q_len, 1)
                    
                    attn_weights[:, :, :, global_idx] = global_attn_scores_reverse.squeeze(3)
...
def create_glm47_sliding_window_attention(config: GLM47Config, layer_idx: Optional[int] = None):
    """
