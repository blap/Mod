...
class GLM47SlidingWindowAttention(BaseAttention):
    ...
    def __init__(self, config: GLM47Config, layer_idx: Optional[int] = None):
        ...
        self.window_size = getattr(config, 'sliding_window_size', 1024)  # Default window size
    ...
    def forward(
        ...
        if position_ids is not None:
            try:
                from .....common.tensor_utils import apply_rotary_pos_emb, rotate_half
    ...
    def _apply_sliding_window_attention(self, query_states, key_states, value_states, attention_mask):
        ...
        attn_weights = torch.zeros_like(torch.matmul(
        ...
        for i in range(q_len):
            ...
            current_query = query_states[:, :, i:i+1, :]  # (bsz, num_heads, 1, head_dim)
        ...
        causal_mask = torch.tril(torch.ones(q_len, q_len, dtype=torch.bool, device=query_states.device))
