...
class GLM47SparseAttention(BaseAttention):
    ...
    def _apply_longformer_attention(
        ...
        sparse_mask = self._create_longformer_sparse_mask(
            q_len, device=query_states.device
        )
    ...
    def _apply_block_sparse_attention(
        ...
        sparse_mask = self._create_block_sparse_mask(
            q_len, num_blocks, device=query_states.device
        )
    ...
    def _create_block_sparse_mask(self, seq_len, num_blocks, device):
        ...
        for block_row in range(num_blocks):
            for block_col in range(num_blocks):
                ...
                if (
                    torch.rand(1, device=device).item() < self.sparsity_ratio
...
__all__ = ["GLM47SparseAttention", "create_glm47_sparse_attention"]
