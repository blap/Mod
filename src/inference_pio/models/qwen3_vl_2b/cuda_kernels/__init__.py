...
class Qwen3VL2BAttentionKernel(nn.Module):
    ...
    def forward(
        ...
        query = query.view(batch_size, seq_len, self.config.num_attention_heads, self.head_dim).transpose(1, 2)
        key = key.view(batch_size, seq_len, self.config.num_attention_heads, self.head_dim).transpose(1, 2)
        value = value.view(batch_size, seq_len, self.config.num_attention_heads, self.head_dim).transpose(1, 2)
        ...
        if self.attention_dropout > 0.0:
            attention_probs = torch.nn.functional.dropout(attention_probs, p=self.attention_dropout, training=self.training)
...
def create_qwen3_vl_kernel_manager(config: Qwen3VL2BConfig) -> Qwen3VL2BHardwareKernelManager:
    """
...
def get_qwen3_vl_optimization_report(model: nn.Module, config: Qwen3VL2BConfig) -> Dict[str, Any]:
    """
    ...
    report = {
        ...
        "kernel_manager_info": {
            ...
            "available_kernels": list(kernel_manager.get_kernel(kernel_type).__class__.__name__ 
                                    for kernel_type in ['gelu', 'matmul', 'softmax', 'attention', 'mlp', 'rms_norm']),
        },
