...
from inference_pio.common.base_model import BaseModel
from inference_pio.common.flash_attention_2 import FlashAttention2
    ...
    def forward(
        ...
        if 'text' in queries:
            ...
            if self.use_flash_attention:
                ...
                text_attn_out = FlashAttention2.apply(text_q, text_k, text_v, self.scale)
            else:
                # Standard attention
...
class Qwen3VL2BVisionLanguageAttentionKernel(nn.Module):
    ...
    def forward(
        ...
        vision_self_out, _ = self.vision_attn(vision_features, vision_features, vision_features)

        # Self-attention within language modality
        lang_self_out, _ = self.language_attn(language_features, language_features, language_features)
        ...
        if combined_vision.size(1) < max_len:
            ...
            combined_vision = torch.cat([
                combined_vision,
                torch.zeros(combined_vision.size(0), pad_size, combined_vision.size(2),
                          dtype=combined_vision.dtype, device=combined_vision.device)
            ], dim=1)
        if combined_language.size(1) < max_len:
            pad_size = max_len - combined_language.size(1)
            combined_language = torch.cat([
                combined_language,
                torch.zeros(combined_language.size(0), pad_size, combined_language.size(2),
                          dtype=combined_language.dtype, device=combined_language.device)
            ], dim=1)
...
class Qwen3VL2BVisionProcessingKernel(nn.Module):
    ...
    def forward(
        ...
        # Patch embedding
        patches = self.patch_embed(pixel_values)  # [batch, hidden_size, patch_h, patch_w]
...
def apply_qwen3_vl_cuda_optimizations_to_model(model: nn.Module, config: Qwen3VL2BConfig):
    """
...
def get_qwen3_vl_cuda_optimization_report(model: nn.Module, config: Qwen3VL2BConfig) -> Dict[str, Any]:
    """
