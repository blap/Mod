...
class VisionSelfAttentionKernel(nn.Module):
    ...
    def forward(
        ...
        q = q.view(batch_size, seq_len, self.num_attention_heads, self.head_dim).transpose(1, 2)  # (batch, num_heads, seq_len, head_dim)
        k = k.view(batch_size, seq_len, self.num_attention_heads, self.head_dim).transpose(1, 2)  # (batch, num_heads, seq_len, head_dim)
        v = v.view(batch_size, seq_len, self.num_attention_heads, self.head_dim).transpose(1, 2)  # (batch, num_heads, seq_len, head_dim)
        ...
        # Apply softmax
        attention_scores = torch.softmax(attention_scores, dim=-1, dtype=torch.float32).to(q.dtype)
        ...
        attn_output = torch.matmul(attention_scores, v)  # (batch, num_heads, seq_len, head_dim)
...
class Qwen3VL2BVisionEncoderKernel(nn.Module):
    ...
    def _create_vision_encoder(self, config: VisionTransformerConfig) -> nn.Module:
        ...
        class VisionEncoder(nn.Module):
            ...
            def forward(self, pixel_values: torch.Tensor, output_hidden_states: bool = False) -> Tuple[torch.Tensor, Optional[List[torch.Tensor]]]:
                # Patch embedding
...
def create_vision_patch_embedding_kernel(config: VisionTransformerConfig) -> VisionPatchEmbeddingKernel:
    """
...
def create_vision_self_attention_kernel(config: VisionTransformerConfig) -> VisionSelfAttentionKernel:
    """
...
def create_vision_transformer_block_kernel(config: VisionTransformerConfig, layer_idx: int = 0) -> VisionTransformerBlockKernel:
    """
...
def create_qwen3_vl_2b_vision_encoder_kernel(config: VisionTransformerConfig) -> Qwen3VL2BVisionEncoderKernel:
    """
...
def apply_vision_cuda_optimizations_to_model(model: nn.Module, config: VisionTransformerConfig) -> nn.Module:
    """
