...
class VisionEncoderOptimizer:
    ...
    def optimize_vision_encoder(self, 
                               vision_encoder: Qwen3VL2BVisionEncoderKernel,
                               model_config: Qwen3VL2BConfig) -> Qwen3VL2BVisionEncoderKernel:
        """
    ...
    def _optimize_patch_embedding(self, 
                                 patch_embedding: VisionPatchEmbeddingKernel, 
                                 config: VisionTransformerConfig) -> VisionPatchEmbeddingKernel:
        ...
        if self.config.use_convolution_fusion:
            ...
            optimized_patch_embed.load_state_dict(patch_embedding.state_dict(), strict=False)
            return optimized_patch_embed
        else:
            return patch_embedding
    
    def _optimize_attention_blocks(self, 
                                  blocks: nn.ModuleList, 
                                  config: VisionTransformerConfig) -> nn.ModuleList:
        ...
        for i, block in enumerate(blocks):
            if hasattr(block, 'attention'):
                ...
                if hasattr(block.attention, 'state_dict'):
                    optimized_attention.load_state_dict(block.attention.state_dict(), strict=False)
    ...
    def _optimize_transformer_blocks(self, 
                                    vision_encoder: Qwen3VL2BVisionEncoderKernel, 
                                    config: VisionTransformerConfig) -> Qwen3VL2BVisionEncoderKernel:
        ...
        if self.config.enable_gradient_checkpointing:
            for i, block in enumerate(vision_encoder.blocks):
                ...
                block.forward = make_checkpoint_func(i).__get__(block, VisionTransformerBlockKernel)
        
        return vision_encoder
    
    def _apply_memory_optimizations(self, vision_encoder: Qwen3VL2BVisionEncoderKernel) -> Qwen3VL2BVisionEncoderKernel:
    ...
    def _apply_quantization(self, vision_encoder: Qwen3VL2BVisionEncoderKernel) -> Qwen3VL2BVisionEncoderKernel:
        """Apply quantization to the vision encoder."""
        logger.info(f"Applying {self.config.quantization_method} quantization with {self.config.quantization_bits} bits...")
        ...
        if self.config.quantization_method == "linear":
            ...
            vision_encoder = torch.quantization.prepare(vision_encoder, quantization_config)
...
class OptimizedVisionSelfAttentionKernel(VisionSelfAttentionKernel):
    ...
    def forward(
        ...
        if self.use_flash_attention:
            ...
            context_layer = attn_output.reshape(batch_size, num_heads, seq_len, head_dim)
        else:
...
def create_vision_encoder_optimizer(config: VisionEncoderOptimizationConfig) -> VisionEncoderOptimizer:
    """
...
def apply_vision_encoder_optimizations_to_model(model: nn.Module, 
                                               model_config: Qwen3VL2BConfig,
                                               optimization_config: VisionEncoderOptimizationConfig) -> nn.Module:
    """
    ...
    for name, module in model.named_modules():
        if isinstance(module, Qwen3VL2BVisionEncoderKernel):
            ...
            optimized_vision_encoder = optimizer.optimize_vision_encoder(module, model_config)
