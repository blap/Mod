...
def test_complete_vision_encoder_optimization_system():
    ...
    optimized_vision_encoder = optimizer.optimize_vision_encoder(basic_vision_encoder, model_config)
    ...
    optimized_output, _ = optimized_vision_encoder(sample_images, output_hidden_states=False)
    print(f"Optimized encoder output shape: {optimized_output.shape}")
    
    # Verify shapes are the same
    assert basic_output.shape == optimized_output.shape, "Output shapes should be the same"
    print("[SUCCESS] Output shapes match")
    
    # Verify outputs are reasonable (not all zeros)
    assert not torch.allclose(basic_output, torch.zeros_like(basic_output)), "Basic output should not be all zeros"
    assert not torch.allclose(optimized_output, torch.zeros_like(optimized_output)), "Optimized output should not be all zeros"
    ...
    if opt_config.enable_patch_embedding_optimization:
        assert isinstance(optimized_vision_encoder.patch_embedding, OptimizedVisionPatchEmbeddingKernel), \
    ...
    if opt_config.enable_attention_optimization:
        ...
        assert isinstance(first_block.attention, OptimizedVisionSelfAttentionKernel), \
    ...
    if opt_config.enable_mlp_optimization:
        ...
        first_block = optimized_vision_encoder.blocks[0]
        assert isinstance(first_block.mlp, OptimizedVisionMLPKernel), \
...
def test_model_class_integration():
    ...
    assert '_apply_vision_encoder_optimizations' in model_methods, \
...
def test_optimization_application_logic():
    ...
    assert isinstance(optimized_encoder, Qwen3VL2BVisionEncoderKernel), \
...
def run_final_verification():
    ...
    try:
        ...
        print("Vision Encoder Optimization System is fully implemented and working correctly")
