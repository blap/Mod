...
class Qwen3VL2BCrossModalFusionKernel(nn.Module):
    ...
    def __init__(self, config: CrossModalFusionConfig, layer_idx: int = 0):
        ...
        self.fusion_up_proj = nn.Linear(config.hidden_size, config.intermediate_size, bias=False)
        self.fusion_gate_proj = nn.Linear(config.hidden_size, config.intermediate_size, bias=False)
        self.fusion_down_proj = nn.Linear(config.intermediate_size, config.hidden_size, bias=False)
    ...
    def forward(
        ...
        vision_to_lang_combined = torch.cat([
            vision_to_lang_avg.expand_as(language_padded), 
            language_padded
        ], dim=-1)
        ...
        language_to_vision_combined = torch.cat([
            language_to_vision_avg.expand_as(vision_padded), 
            vision_padded
        ], dim=-1)
        ...
        vision_output = vision_features + vision_fused[:, :vision_seq_len, :] * vision_gate_part
        language_output = language_features + language_fused[:, :lang_seq_len, :] * language_gate_part
...
class Qwen3VL2BCrossModalFusionManager:
    ...
    def _register_fusion_method(self, method_name: str, config: CrossModalFusionConfig):
        ...
        if method_name == "qwen3_vl_specific":
            self.fusion_methods[method_name] = Qwen3VL2BCrossModalFusionKernel(config, layer_idx=0)
        elif method_name == "attention":
            # Create a generic attention-based fusion kernel
            self.fusion_methods[method_name] = self._create_attention_fusion_kernel(config)
        elif method_name == "learned_projection":
            # Create a learned projection fusion kernel
            self.fusion_methods[method_name] = self._create_projection_fusion_kernel(config)
        else:
    ...
    def _create_attention_fusion_kernel(self, config: CrossModalFusionConfig):
        ...
        class AttentionFusionKernel(nn.Module):
            def __init__(self, cfg):
                super().__init__()
                self.projection = nn.Linear(cfg.hidden_size * 2, cfg.hidden_size, bias=False)
            ...
            def forward(self, vision_feat, language_feat, mask=None):
                ...
                if lang_len < max_len:
                    language_padded = F.pad(language_feat, (0, 0, 0, max_len - lang_len))
                else:
                ...
                return output[:, :min(vision_len, lang_len), :], vision_feat, language_feat
    ...
    def _create_projection_fusion_kernel(self, config: CrossModalFusionConfig):
        ...
        class ProjectionFusionKernel(nn.Module):
            def __init__(self, cfg):
                super().__init__()
                self.vision_proj = nn.Linear(cfg.hidden_size, cfg.hidden_size, bias=False)
                self.language_proj = nn.Linear(cfg.hidden_size, cfg.hidden_size, bias=False)
                self.fusion_proj = nn.Linear(cfg.hidden_size * 2, cfg.hidden_size, bias=False)
            ...
            def forward(self, vision_feat, language_feat, mask=None):
                ...
                if lang_len < max_len:
                    language_padded = F.pad(language_proj, (0, 0, 0, max_len - lang_len))
                else:
                ...
                return fused_output[:, :min(vision_len, lang_len), :], vision_feat, language_feat
    ...
    def _apply_simple_fusion(
        ...
        if method == "add":
            ...
            if vision_seq_len == lang_seq_len:
                ...
                max_len = max(vision_seq_len, lang_seq_len)
                if vision_seq_len < max_len:
                    vision_padded = F.pad(vision_features, (0, 0, 0, max_len - vision_seq_len))
                else:
                    vision_padded = vision_features

                if lang_seq_len < max_len:
                    language_padded = F.pad(language_features, (0, 0, 0, max_len - lang_seq_len))
                else:
            ...
            if vision_seq_len == lang_seq_len:
                ...
                fused_output = nn.Linear(2 * hidden_size, hidden_size).to(fused_output.device)(fused_output)
            else:
...
def create_qwen3_vl_cross_modal_fusion(config: CrossModalFusionConfig) -> Qwen3VL2BCrossModalFusionManager:
    """
...
def apply_cross_modal_fusion_to_qwen3_vl_model(model: nn.Module, fusion_manager: Qwen3VL2BCrossModalFusionManager) -> nn.Module:
    """
    ...
    def perform_cross_modal_fusion(self, vision_features, language_features, method="qwen3_vl_specific", attention_mask=None):
        """
