...
def demonstrate_quantization_compression():
    ...
    # Measure performance
    avg_time, avg_compression_ratio = measure_compression_performance(
        compressor, tensor
    )
    ...
    print(
        f"Compressed tensor size (bytes): {compressed.numel() * compressed.element_size()}"
    )
...
def demonstrate_visual_feature_compressor():
    ...
    compressed, metadata = feature_compressor.compress_features(
        features, layer_name="vision_encoder", feature_type="patch_embeddings"
    )
...
def demonstrate_different_compression_methods():
    ...
    for method, method_params in methods:
        ...
        # Create config
        config_params = {"compression_method": method, "enable_compression_cache": True}
        ...
        # Measure performance
        avg_time, avg_compression_ratio = measure_compression_performance(
            compressor, tensor, iterations=5
        )
