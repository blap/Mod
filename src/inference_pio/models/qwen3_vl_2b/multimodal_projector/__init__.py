...
class Qwen3VL2BProjectionLayer(nn.Module):
    ...
    def _initialize_weights(self):
        ...
        if hasattr(self, "vision_to_language_attn"):
            ...
            std = self.language_projection.in_features**-0.5
            nn.init.normal_(
                self.vision_to_language_attn.in_proj_weight, mean=0.0, std=std
            )
            nn.init.normal_(
                self.vision_to_language_attn.out_proj.weight, mean=0.0, std=std
            )

            std = self.vision_projection.in_features**-0.5
            nn.init.normal_(
                self.language_to_vision_attn.in_proj_weight, mean=0.0, std=std
            )
            nn.init.normal_(
                self.language_to_vision_attn.out_proj.weight, mean=0.0, std=std
            )
    ...
    def forward(
        ...
        if vision_seq_len < max_len:
            vision_padded = F.pad(
                projected_vision, (0, 0, 0, max_len - vision_seq_len), value=0
            )
        else:
        ...
        if lang_seq_len < max_len:
            language_padded = F.pad(
                projected_language, (0, 0, 0, max_len - lang_seq_len), value=0
            )
        else:
        ...
        if hasattr(self, "vision_to_language_attn") and hasattr(
            self, "language_to_vision_attn"
        ):
...
class Qwen3VL2BVisionLanguageProjector(nn.Module):
    ...
    def forward(
        ...
        if vision_seq_len < max_len:
            vision_padded = F.pad(
                vision_proj, (0, 0, 0, max_len - vision_seq_len), value=0
            )
        else:
        ...
        if language_seq_len < max_len:
            language_padded = F.pad(
                language_proj, (0, 0, 0, max_len - language_seq_len), value=0
            )
        else:
        ...
        if isinstance(self.attention, nn.MultiheadAttention):
            ...
            attended_features = (
                self.attention(
                    language_padded, vision_padded, attention_mask=attention_mask
                )
                if attention_mask is not None
                else self.attention(language_padded, vision_padded)
            )
...
def create_qwen3_vl_projection_layer(
    ...
    return Qwen3VL2BProjectionLayer(
        ...
        activation=getattr(
            config, "projection_activation", "silu"
        ),  # Qwen3-VL-2B uses SiLU
    ...
    for name, module in model.named_modules():
        if "projector" in name.lower() or "projection" in name.lower():
            if (
                ...
                if (
                    hasattr(new_projection, "use_low_rank")
                    and new_projection.use_low_rank
                ):
                    new_projection.vision_projection[0] = module
                else:
                    new_projection.vision_projection = module
