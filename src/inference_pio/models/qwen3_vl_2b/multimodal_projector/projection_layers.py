...
class Qwen3VL2BProjectionLayer(nn.Module):
    ...
    def _initialize_weights(self):
        ...
        if self.use_low_rank:
            ...
            if hasattr(self.vision_projection[0], 'bias') and self.vision_projection[0].bias is not None:
                nn.init.zeros_(self.vision_projection[0].bias)
            if hasattr(self.vision_projection[1], 'bias') and self.vision_projection[1].bias is not None:
                nn.init.zeros_(self.vision_projection[1].bias)
        else:
            nn.init.xavier_uniform_(self.vision_projection.weight)
            if hasattr(self.vision_projection, 'bias') and self.vision_projection.bias is not None:
        ...
        if self.use_low_rank:
            ...
            if hasattr(self.language_projection[0], 'bias') and self.language_projection[0].bias is not None:
                nn.init.zeros_(self.language_projection[0].bias)
            if hasattr(self.language_projection[1], 'bias') and self.language_projection[1].bias is not None:
                nn.init.zeros_(self.language_projection[1].bias)
        else:
            nn.init.xavier_uniform_(self.language_projection.weight)
            if hasattr(self.language_projection, 'bias') and self.language_projection.bias is not None:
    ...
    def forward(
        ...
        if seq_len_vision != seq_len_language:
            ...
            if seq_len_vision < max_seq_len:
                ...
                projected_vision = torch.cat([
                    projected_vision,
                    torch.zeros(batch_size, pad_size, projected_vision.shape[-1], 
                              dtype=projected_vision.dtype, device=projected_vision.device)
                ], dim=1)
            if seq_len_language < max_seq_len:
                ...
                projected_language = torch.cat([
                    projected_language,
                    torch.zeros(batch_size, pad_size, projected_language.shape[-1], 
                              dtype=projected_language.dtype, device=projected_language.device)
                ], dim=1)
                
        # Concatenate projected features for multimodal fusion
        concatenated_features = torch.cat([projected_vision, projected_language], dim=-1)
        ...
        if self.use_group_norm:
            ...
            multimodal_features = multimodal_features.permute(0, 2, 1)  # (batch, features, seq)
            multimodal_features = self.norm(multimodal_features)
            multimodal_features = multimodal_features.permute(0, 2, 1)  # Back to (batch, seq, features)
        else:
            multimodal_features = self.norm(multimodal_features)
        ...
        if seq_len_vision != seq_len_language:
            output_features = output_features[:, :max(seq_len_vision, seq_len_language), :]
...
class Qwen3VL2BVisionLanguageProjector(nn.Module):
    ...
    def forward(
        ...
        if self.use_conv_projection and len(vision_features.shape) == 4:
            ...
            projected_vision = projected_vision.view(batch_size, -1, projected_vision.shape[-1])
        else:
        ...
        if projected_vision.shape[1] != projected_language.shape[1]:
            ...
            if projected_vision.shape[1] < max_seq_len:
                ...
                projected_vision = torch.cat([
                    projected_vision,
                    torch.zeros(projected_vision.shape[0], pad_size, projected_vision.shape[-1], 
                              dtype=projected_vision.dtype, device=projected_vision.device)
                ], dim=1)
            if projected_language.shape[1] < max_seq_len:
                ...
                projected_language = torch.cat([
                    projected_language,
                    torch.zeros(projected_language.shape[0], pad_size, projected_language.shape[-1], 
                              dtype=projected_language.dtype, device=projected_language.device)
                ], dim=1)
        ...
        if isinstance(self.attention, nn.MultiheadAttention):
            ...
            ) if attention_mask is not None else self.attention(combined_features)
