...
from ...common.quantization import QuantizationConfig, QuantizationScheme, QuantizedLinear
...
class QuantizedMultimodalCrossAttentionKernel(nn.Module):
    ...
    def __init__(self, config: QuantizedMultimodalConfig):
        ...
        self.modality_norms = nn.ModuleDict({
            modality: nn.LayerNorm(self.d_model)
            for modality in self.modalities
        })
...
class QuantizedVisionLanguageAttentionKernel(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            lang_to_vision_attn = lang_to_vision_attn + attention_mask.transpose(-1, -2)  # Transpose for reverse direction
...
class QuantizedQwen3VL2BCrossAttentionKernel(QuantizedMultimodalCrossAttentionKernel):
    ...
    def __init__(self, config: QuantizedMultimodalConfig, layer_idx: int = 0):
        ...
        self.q_proj = self._create_quantized_linear(config.d_model, config.d_model, bias=False)
        self.k_proj = self._create_quantized_linear(config.d_model, config.d_model, bias=False)
        self.v_proj = self._create_quantized_linear(config.d_model, config.d_model, bias=False)

        # Qwen3-VL-2B specific quantized output projection
        self.o_proj = self._create_quantized_linear(config.d_model, config.d_model, bias=False)
...
class QuantizedQwen3VL2BFusionKernel(QuantizedMultimodalFusionKernel):
    ...
    def __init__(self, config: QuantizedMultimodalConfig, layer_idx: int = 0):
        ...
        self.mlp_gate_proj = self._create_quantized_linear(config.d_model, config.d_model * 4, bias=False)
        self.mlp_up_proj = self._create_quantized_linear(config.d_model, config.d_model * 4, bias=False)
        self.mlp_down_proj = self._create_quantized_linear(config.d_model * 4, config.d_model, bias=False)
...
def create_quantized_multimodal_kernels(config: QuantizedMultimodalConfig) -> Dict[str, nn.Module]:
    """
    ...
    if "text" in config.modalities and "image" in config.modalities:
        kernels['vision_language_attention'] = QuantizedVisionLanguageAttentionKernel(config)
...
def create_quantized_qwen3_vl_kernels(config: QuantizedMultimodalConfig, layer_idx: int = 0) -> Dict[str, nn.Module]:
    """
    ...
    kernels['cross_attention'] = QuantizedQwen3VL2BCrossAttentionKernel(config, layer_idx)
...
def apply_quantized_multimodal_optimizations_to_model(model: nn.Module,
                                                    config: QuantizedMultimodalConfig,
                                                    layer_indices: List[int] = None) -> nn.Module:
    """
    ...
    Returns:
        Optimized model
    """
    logger.info(f"Applying quantized multimodal optimizations for modalities: {config.modalities}")
    ...
    if layer_indices is None:
        ...
        for name, module in model.named_modules():
            if "layer" in name.lower() or "block" in name.lower() or "encoder" in name.lower():
                try:
    ...
    for name, module in model.named_modules():
        ...
        if any(mod in name.lower() for mod in config.modalities):
            if isinstance(module, nn.MultiheadAttention):
                ...
                quantized_attn = QuantizedQwen3VL2BCrossAttentionKernel(config, layer_idx)

                # Create a wrapper that maintains the same interface as MultiheadAttention
                quantized_attn_wrapper = QuantizedMultiheadAttentionWrapper(quantized_attn)

                # Replace the attention module
                parent_module, child_name = _get_parent_module(model, name)
                setattr(parent_module, child_name, quantized_attn_wrapper)

                logger.info(f"Replaced attention module {name} with quantized multimodal kernel (layer {layer_idx})")
...
def get_quantized_multimodal_optimization_report(model: nn.Module, config: QuantizedMultimodalConfig) -> Dict[str, Any]:
    """
