...
class Qwen3VL2BCrossAttentionKernel(nn.Module):
    ...
    def __init__(self,
        ...
        if config.use_quantized_kernels:
            ...
            from .....common.quantized_multimodal_kernels import QuantizedMultimodalConfig
            ...
            quant_scheme = scheme_map.get(config.quantization_scheme.lower(), QuantizationScheme.INT8)
            ...
            self.kernel_impl = QuantizedQwen3VL2BCrossAttentionKernel(quantized_config, layer_idx)
        else:
            # Use standard kernel
    ...
    def forward(
        ...
        if self.config.use_quantized_kernels:
            ...
            for query_modality, query in queries.items():
                ...
                q = self.q_proj(query)
                q = q.view(query.size(0), query.size(1), self.kernel_impl.nhead, self.kernel_impl.head_dim).transpose(1, 2)
                q = q * self.scale
                ...
                for key_modality, key in keys.items():
                    ...
                    k = self.k_proj(key)
                    k = k.view(key.size(0), key.size(1), self.kernel_impl.nhead, self.kernel_impl.head_dim).transpose(1, 2)
                    all_keys.append(k)

                    # Project value using Qwen3-VL-2B specific projections
                    v = self.v_proj(key)  # Using key as input for simplicity
                    v = v.view(key.size(0), key.size(1), self.kernel_impl.nhead, self.kernel_impl.head_dim).transpose(1, 2)
                    all_values.append(v)
                ...
                if self.kernel_impl.use_flash_attention and torch.cuda.is_available():
                    ...
                    if attention_masks is not None and query_modality in attention_masks:
                        ...
                        if mask.dim() == 2:
                            mask = mask.unsqueeze(0).unsqueeze(0).expand(-1, self.kernel_impl.nhead, -1, -1)
                        elif mask.dim() == 3:
                            mask = mask.unsqueeze(1).expand(-1, self.kernel_impl.nhead, -1, -1)
                ...
                attn_output = attn_output.transpose(1, 2).contiguous().view(
                    query.size(0), query.size(1), self.kernel_impl.d_model
                )
...
class Qwen3VL2BFusionKernel(nn.Module):
    ...
    def __init__(self,
        ...
        if config.use_quantized_kernels:
            ...
            # Use quantized kernel
            self.kernel_impl = QuantizedQwen3VL2BFusionKernel(quantized_config, layer_idx)
        else:
            # Use standard kernel
...
class Qwen3VL2BVisionLanguageAttentionKernel(nn.Module):
    ...
    def __init__(self,
        ...
        if config.use_quantized_kernels:
            ...
            self.vision_q_proj = nn.Linear(config.hidden_size, config.hidden_size, bias=False)
            self.vision_k_proj = nn.Linear(config.hidden_size, config.hidden_size, bias=False)
            self.vision_v_proj = nn.Linear(config.hidden_size, config.hidden_size, bias=False)

            self.language_q_proj = nn.Linear(config.hidden_size, config.hidden_size, bias=False)
            self.language_k_proj = nn.Linear(config.hidden_size, config.hidden_size, bias=False)
            self.language_v_proj = nn.Linear(config.hidden_size, config.hidden_size, bias=False)

            # Cross-modality projections
            self.vision_to_lang_proj = nn.Linear(config.hidden_size, config.hidden_size, bias=False)
            self.lang_to_vision_proj = nn.Linear(config.hidden_size, config.hidden_size, bias=False)
    ...
    def forward(
        ...
        if self.config.use_quantized_kernels:
            ...
            vision_features = vision_features + self.kernel_impl.vision_pos_embed[:, :num_patches, :]
            language_features = language_features + self.kernel_impl.text_pos_embed[:, :seq_len, :]
            ...
            v_q = self.vision_q_proj(vision_norm).view(batch_size, num_patches, self.kernel_impl.nhead, self.kernel_impl.head_dim).transpose(1, 2)
            v_k = self.vision_k_proj(vision_norm).view(batch_size, num_patches, self.kernel_impl.nhead, self.kernel_impl.head_dim).transpose(1, 2)
            v_v = self.vision_v_proj(vision_norm).view(batch_size, num_patches, self.kernel_impl.nhead, self.kernel_impl.head_dim).transpose(1, 2)

            # Project language features using Qwen3-VL-2B specific projections
            l_q = self.language_q_proj(lang_norm).view(batch_size, seq_len, self.kernel_impl.nhead, self.kernel_impl.head_dim).transpose(1, 2)
            l_k = self.language_k_proj(lang_norm).view(batch_size, seq_len, self.kernel_impl.nhead, self.kernel_impl.head_dim).transpose(1, 2)
            l_v = self.language_v_proj(lang_norm).view(batch_size, seq_len, self.kernel_impl.nhead, self.kernel_impl.head_dim).transpose(1, 2)
...
class Qwen3VL2BPositionEncodingKernel(nn.Module):
    ...
    def __init__(self,
        ...
        if config.use_quantized_kernels:
            ...
            # Use quantized kernel
            self.kernel_impl = QuantizedMultimodalPositionEncodingKernel(quantized_config)
        else:
            # Use standard kernel
            ...
            if "text" in self.kernel_impl.modalities:
                ...
                self.text_pos_embed = nn.Parameter(torch.zeros(1, config.max_position_embeddings, config.hidden_size))

            if "image" in self.kernel_impl.modalities:
                self.image_pos_embed = nn.Parameter(torch.randn(1, 1024, config.hidden_size) * 0.02)
...
def apply_qwen3_vl_cuda_optimizations_to_model(model: nn.Module,
                                              config: Qwen3VL2BConfig) -> nn.Module:
    """
    ...
    if config.use_quantized_kernels:
        logger.info(f"Using quantized kernels with scheme: {config.quantization_scheme}, bits: {config.quantization_bits}")
    ...
    for name, module in model.named_modules():
        ...
        if "attn" in name.lower() or "attention" in name.lower():
            if isinstance(module, nn.MultiheadAttention):
                ...
                qwen_attn = Qwen3VL2BAttentionWrapper(create_qwen3_vl_cross_attention_kernel(config, layer_idx))

                # Replace the attention module
                parent_module, child_name = _get_parent_module(model, name)
                setattr(parent_module, child_name, qwen_attn)

                logger.debug(f"Replaced attention module {name} with Qwen3-VL-2B optimized version (quantized: {config.use_quantized_kernels})")
        ...
        # Replace standard MLP layers with Qwen3-VL-2B optimized versions
            ...
            qwen_mlp = Qwen3VL2BMLPWrapper(create_qwen3_vl_mlp_kernel(config, layer_idx))

            # Replace the MLP module
            parent_module, child_name = _get_parent_module(model, name)
            setattr(parent_module, child_name, qwen_mlp)

            logger.debug(f"Replaced MLP module {name} with Qwen3-VL-2B optimized version (quantized: {config.use_quantized_kernels})")
        ...
        # Replace standard LayerNorm with Qwen3-VL-2B specific RMSNorm
            ...
            qwen_rms_norm = Qwen3VL2BRMSNormWrapper(create_qwen3_vl_rms_norm_kernel(config, layer_idx))

            # Replace the LayerNorm module
            parent_module, child_name = _get_parent_module(model, name)
            setattr(parent_module, child_name, qwen_rms_norm)

            logger.debug(f"Replaced LayerNorm module {name} with Qwen3-VL-2B RMSNorm version")
        ...
        if "vision" in name.lower() or "visual" in name.lower() or "patch" in name.lower():
            if isinstance(module, nn.Conv2d):
                ...
                if module.kernel_size[0] == config.vision_patch_size:
                    ...
                    vision_patch_embed = create_vision_patch_embedding_kernel(vision_config)
...
def get_qwen3_vl_cuda_optimization_report(model: nn.Module, config: Qwen3VL2BConfig) -> Dict:
    """
...
def create_qwen3_vl_vision_processing_kernel(config: Qwen3VL2BConfig) -> Qwen3VL2BVisionProcessingKernel:
    """
