...
class BenchmarkQwen3VL2BIntelligentCaching(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config.intelligent_multimodal_cache_size_gb = (
            0.5  # 500MB cache for testing
        )
    ...
    def benchmark_cache_hit_rate(self):
        ...
        success = plugin_without_caching.initialize(
            enable_intelligent_multimodal_caching=False, device="cpu"
        )
        self.assertTrue(success)

        # Prepare test data
        test_pairs = [
            (text, img) for text, img in zip(self.mock_texts, self.mock_images)
        ]
        ...
        uncached_consistency = (
            first_run_time_without_caching / second_run_time_without_caching
        )

        print(f"  First run with caching: {first_run_time_with_caching:.4f}s")
        print(
            f"  Second run with caching (benefiting from cache): {second_run_time_with_caching:.4f}s"
        )
    ...
    def benchmark_memory_efficiency(self):
        ...
        for i in range(10):  # Process 10 inputs
            ...
            img = Image.new(
                "RGB", (224, 224), color=(i * 20 % 255, (i * 30) % 255, (i * 40) % 255)
            )
            result = plugin_with_caching.infer({"text": text, "image": img})
        ...
        print(
            f"  Memory with caching: {caching_memory:.2f} MB (diff: {caching_memory - baseline_memory:.2f} MB)"
        )
        print(
            f"  Memory without caching: {no_caching_memory:.2f} MB (diff: {no_caching_memory - baseline_memory:.2f} MB)"
        )
    ...
    def benchmark_similarity_detection(self):
        ...
        base_text = (
            "This is a base text for similarity testing in the Qwen3-VL-2B model."
        )
        similar_texts = [
            base_text,
        ...
        for text in similar_texts:
            ...
            if result:
                print(
                    f"    Found similar text for: '{text[:30]}...' -> {result[0][:10]}..."
                )
        similarity_lookup_time = time.time() - start_time

        print(
            f"  Similarity lookup time for {len(similar_texts)} texts: {similarity_lookup_time:.4f}s"
        )
        print(
            f"  Average lookup time per text: {similarity_lookup_time/len(similar_texts)*1000:.2f}ms"
        )
    ...
    def benchmark_cache_scalability(self):
        ...
        for cache_size in cache_sizes_gb:
            ...
            print(
                f"  Cache size {cache_size}GB: {processing_time:.4f}s, hit_rate: {cache_stats.get('hit_rate', 0):.2%}, "
