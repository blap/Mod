...
class Qwen3VL2BCrossModalAlignmentOptimizer(nn.Module):
    ...
    def __init__(self, config: CrossModalAlignmentConfig, layer_idx: int = 0):
        ...
        self.alignment_up_proj = nn.Linear(config.hidden_size, config.intermediate_size, bias=False)
        self.alignment_gate_proj = nn.Linear(config.hidden_size, config.intermediate_size, bias=False)
        self.alignment_down_proj = nn.Linear(config.intermediate_size, config.hidden_size, bias=False)
    ...
    def forward(
        ...
        if vision_seq_len < max_len:
            vision_padded = F.pad(vision_norm, (0, 0, 0, max_len - vision_seq_len), value=0)
        else:
            vision_padded = vision_norm

        if lang_seq_len < max_len:
            language_padded = F.pad(language_norm, (0, 0, 0, max_len - lang_seq_len), value=0)
        else:
        ...
        vision_aligned = self.alignment_down_proj(vision_gate * vision_up)  # Element-wise multiplication for SwiGLU

        language_gate = F.silu(self.alignment_gate_proj(language_padded))
        language_up = self.alignment_up_proj(language_padded)
        language_aligned = self.alignment_down_proj(language_gate * language_up)  # Element-wise multiplication for SwiGLU
        ...
        if vision_padded.size(1) < max_len:
            vision_padded_for_gate = F.pad(vision_padded, (0, 0, 0, max_len - vision_padded.size(1)), value=0)
        else:
            vision_padded_for_gate = vision_padded

        if language_padded.size(1) < max_len:
            language_padded_for_gate = F.pad(language_padded, (0, 0, 0, max_len - language_padded.size(1)), value=0)
        else:
        ...
        vision_to_lang_avg = vision_padded_for_gate.mean(dim=1, keepdim=True)  # Average across sequence dimension
        vision_to_lang_combined = torch.cat([vision_to_lang_avg.expand_as(language_padded_for_gate), language_padded_for_gate], dim=-1)
        ...
        language_to_vision_avg = language_padded_for_gate.mean(dim=1, keepdim=True)  # Average across sequence dimension
        language_to_vision_combined = torch.cat([language_to_vision_avg.expand_as(vision_padded_for_gate), vision_padded_for_gate], dim=-1)
        ...
        vision_gate_part = language_vision_gate[:, :vision_seq_len, :self.config.hidden_size]
        language_gate_part = vision_language_gate[:, :lang_seq_len, :self.config.hidden_size]

        # Make sure the gate parts match the original sequence lengths
        if vision_gate_part.size(1) != vision_seq_len:
            vision_gate_part = F.pad(vision_gate_part, (0, 0, 0, vision_seq_len - vision_gate_part.size(1)), value=0)
        if language_gate_part.size(1) != lang_seq_len:
            language_gate_part = F.pad(language_gate_part, (0, 0, 0, lang_seq_len - language_gate_part.size(1)), value=0)

        # Apply gating to aligned features
        aligned_vision = vision_features + vision_aligned[:, :vision_seq_len, :] * vision_gate_part
        aligned_language = language_features + language_aligned[:, :lang_seq_len, :] * language_gate_part
    ...
    def _calculate_qwen_alignment_loss(
        ...
        if self.config.use_contrastive_alignment:
            ...
            vision_lang_sim = F.cosine_similarity(vision_repr, language_repr, dim=-1).mean()
            ...
            vision_neg_lang_sim = F.cosine_similarity(vision_repr, lang_shifted, dim=-1).mean()
        ...
        semantic_reg_loss = F.mse_loss(aligned_vision, original_vision) + F.mse_loss(aligned_language, original_language)

        # Combine similarity, contrastive, and regularization losses
        total_loss = (1 - vision_sim) + (1 - language_sim) + \
                     self.config.alignment_lambda * contrastive_loss + \
                     0.01 * semantic_reg_loss  # Small regularization term
...
class CrossModalAlignmentManager:
    ...
    def register_alignment_method(self, method_name: str, config: CrossModalAlignmentConfig):
        ...
        if method_name == "qwen3_vl_specific":
            ...
            self.alignment_methods[method_name] = Qwen3VL2BCrossModalAlignmentOptimizer(temp_config)

    def get_alignment_optimizer(self, method_name: str) -> Optional[Qwen3VL2BCrossModalAlignmentOptimizer]:
        """
    ...
    def evaluate_alignment_quality(
        ...
        vision_similarity = F.cosine_similarity(original_vision, aligned_vision, dim=-1).mean().item()
        language_similarity = F.cosine_similarity(original_language, aligned_language, dim=-1).mean().item()

        # Calculate cross-modal similarity
        vision_repr = aligned_vision.mean(dim=1)
        language_repr = aligned_language.mean(dim=1)
        cross_modal_similarity = F.cosine_similarity(vision_repr, language_repr, dim=-1).mean().item()

        # Calculate alignment improvement
        original_cross_modal_sim = F.cosine_similarity(
            original_vision.mean(dim=1),
            original_language.mean(dim=1),
            dim=-1
        ).mean().item()
        ...
        return {
            ...
            "quality_score": (vision_similarity + language_similarity + cross_modal_similarity) / 3.0
        }


def create_qwen3_vl_cross_modal_alignment(config: CrossModalAlignmentConfig) -> CrossModalAlignmentManager:
    """
...
def apply_cross_modal_alignment_to_model(model: nn.Module, config: CrossModalAlignmentConfig) -> nn.Module:
    """
    ...
    model.cross_modal_alignment_optimizer = alignment_manager.get_alignment_optimizer("qwen3_vl_specific")

    # Add alignment method to model
    def perform_cross_modal_alignment(self, vision_features, language_features, method="qwen3_vl_specific", attention_mask=None):
        """
...
def get_cross_modal_alignment_report(model: nn.Module, config: CrossModalAlignmentConfig) -> Dict:
    """
    ...
    alignment_enabled = getattr(config, 'enable_cross_modal_alignment', False) or hasattr(model, 'cross_modal_alignment_manager')
    ...
    report = {
        ...
        "alignment_config": {
            "alignment_temperature": getattr(config, 'cross_modal_alignment_temperature', getattr(config, 'rope_theta', 0.5)),
            "alignment_lambda": getattr(config, 'cross_modal_alignment_lambda', 0.1),
            "use_contrastive_alignment": getattr(config, 'use_cross_modal_contrastive_alignment', True),
            "contrastive_margin": getattr(config, 'cross_modal_contrastive_margin', 0.2),
            "enable_dynamic_alignment": getattr(config, 'enable_dynamic_cross_modal_alignment', True),
            "alignment_frequency": getattr(config, 'cross_modal_alignment_frequency', 10),
            "alignment_threshold": getattr(config, 'cross_modal_alignment_threshold', 0.8),
            "use_attention_alignment": getattr(config, 'use_cross_modal_attention_alignment', True),
            "use_learned_alignment": getattr(config, 'use_cross_modal_learned_alignment', True),
            "alignment_projection_dim": getattr(config, 'cross_modal_alignment_projection_dim', getattr(config, 'hidden_size', 2048)),
            "enable_similarity_alignment": getattr(config, 'enable_cross_modal_similarity_alignment', True),
            "similarity_method": getattr(config, 'cross_modal_similarity_method', 'cosine')
        }
    }

    if hasattr(model, 'cross_modal_alignment_manager'):
        if hasattr(model.cross_modal_alignment_manager, 'alignment_methods'):
            report["alignment_methods_registered"] = list(model.cross_modal_alignment_manager.alignment_methods.keys())
        else:
