        ...
        if freqs.size(1) < self.dim // 2:
            ...
            freqs = torch.cat(
                [
                    freqs,
                    torch.zeros(
                        freqs.size(0), pad_size, device=device, dtype=freqs.dtype
                    ),
                ],
                dim=1,
            )
...
def create_qwen3_vl_rotary_embedding(
    ...
    """
    return Qwen3VL2BRotaryEmbedding(
        dim, max_position_embeddings, base, device, precision
    )


__all__ = ["Qwen3VL2BRotaryEmbedding", "create_qwen3_vl_rotary_embedding"]
