"""
Test suite for Intelligent Scheduling System for Qwen3-Coder-Next Model.
"""
...
from src.inference_pio.models.qwen3_coder_next.scheduling.intelligent_scheduler import (
...
class TestIntelligentScheduler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = IntelligentSchedulerConfig(
            ...
            resource_buffer_percentage=0.18,
            enable_priority_boosting=True,
            priority_decay_factor=0.91,
            enable_load_balancing=True,
            load_balance_interval=0.06,
...
class TestIntelligentSchedulerIntegration(unittest.TestCase):
    ...
    def test_create_scheduler_for_qwen3_coder_next(self):
        """Test creating an intelligent scheduler specifically for Qwen3-Coder-Next."""
        mock_config = MagicMock()
        mock_config.intelligent_scheduling_max_concurrent_ops = 48
        mock_config.intelligent_scheduling_policy = 'intelligent'
        mock_config.intelligent_scheduling_enable_prediction = True
        mock_config.intelligent_scheduling_prediction_horizon = 18
        mock_config.intelligent_scheduling_enable_adaptive = True
        mock_config.intelligent_scheduling_adaptive_window = 200
        mock_config.intelligent_scheduling_enable_resource_opt = True
        mock_config.intelligent_scheduling_resource_buffer = 0.18
        mock_config.intelligent_scheduling_enable_priority_boost = True
        mock_config.intelligent_scheduling_priority_decay = 0.91
        mock_config.intelligent_scheduling_enable_load_balancing = True
        mock_config.intelligent_scheduling_load_balance_interval = 0.06
        mock_config.intelligent_scheduling_performance_log_interval = 85

        scheduler = create_intelligent_scheduler_for_qwen3_coder_next(mock_config)
        
        self.assertIsInstance(scheduler, IntelligentOperationScheduler)
        self.assertEqual(scheduler.config.max_concurrent_ops, 48)
        ...
        self.assertEqual(scheduler.config.prediction_horizon, 18)
