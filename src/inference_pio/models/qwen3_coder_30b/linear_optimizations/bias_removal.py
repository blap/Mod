...
def apply_bias_removal_during_model_loading(
    ...
    optimized_model, report = apply_bias_removal_to_model(
        model, bias_config, model_type="qwen3_coder"
    )

    print(
        f"Bias removal optimization applied: {report['removed_count']} layers modified"
    )
