"""
Qwen3-Coder-30B Prefix Caching Implementation

This module implements prefix caching for the Qwen3-Coder-30B model.
"""
...
class PrefixCacheManager:
    """
    Prefix cache manager for Qwen3-Coder-30B model.
    ...
    def _compress_tensor(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        if self.compression_method == "fp16":
            ...
            return torch.cat(
                [
                    quantized.flatten(),
                    torch.tensor([min_val, max_val], dtype=tensor.dtype),
                ]
            )
    ...
    def _decompress_tensor(self, compressed_tensor: torch.Tensor) -> torch.Tensor:
        ...
        if self.compression_method == "fp16":
            ...
            quantized = (
                compressed_tensor[:-2].view(compressed_tensor.shape[:-2] + (-1,)).char()
            )
            range_val = max_val - min_val
            if range_val == 0:
                range_val = 1.0
...
def create_prefix_cache_for_qwen3_coder(
    config: Qwen3Coder30BConfig,
) -> PrefixCacheManager:
    """
    Create a prefix cache manager specifically configured for Qwen3-Coder-30B.
...
__all__ = [
    ...
    "create_prefix_cache_for_qwen3_coder",
]
