...
# Import the specialized Multi-Query Attention for Qwen3-Coder-30B
from ..attention import MultiQueryAttention, MultiQueryAttentionConfig, create_mqa_layer
    ...
    def estimate_energy_savings(model, input_shape):
        ...
        if hasattr(self.config, 'intelligent_cache_enabled') and self.config.intelligent_cache_enabled:
            ...
            if not hasattr(self, 'intelligent_cache_manager'):
                self.intelligent_cache_manager = create_intelligent_cache_for_qwen3_coder(self.config)
