"""
Comprehensive Benchmark for Inference Speed Comparison - Qwen3-Coder-30B

This module benchmarks the inference speed comparison between the original
and modified versions of the Qwen3-Coder-30B model with all optimizations enabled.
"""
...
class Qwen3Coder30BInferenceSpeedComparisonBenchmark:
    """Benchmark class for comparing inference speed between original and modified Qwen3-Coder-30B models."""
    ...
    def __init__(self):
        ...
        self.results_dir = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', 'benchmark_results')
    ...
    def benchmark_model_inference_speed(self, plugin, model_label, input_length=50, num_iterations=10):
        ...
        std_time_per_inference = stdev(inference_times) if len(inference_times) > 1 else 0.0
    ...
    def run_comparison_benchmark(self):
        ...
        print("="*80)
        print("QWEN3-CODER-30B INFERENCE SPEED COMPARISON BENCHMARK WITH OPTIMIZATIONS")
        print("="*80)
        ...
        comparison_metrics = []
        for i, (orig_res, mod_res) in enumerate(zip(original_results, modified_results)):
            # Calculate percentage improvement (positive if modified is faster)
            if orig_res['tokens_per_second'] != float('inf') and mod_res['tokens_per_second'] != float('inf'):
                improvement = ((mod_res['tokens_per_second'] - orig_res['tokens_per_second']) /
                              orig_res['tokens_per_second']) * 100
            elif orig_res['tokens_per_second'] == float('inf'):
                improvement = float('inf') if mod_res['tokens_per_second'] != float('inf') else 0
            else:
        ...
        # Print summary
        print("\n" + "="*80)
        print("SUMMARY - QWEN3-CODER-30B INFERENCE SPEED COMPARISON WITH OPTIMIZATIONS")
        print("="*80)
        print(f"{'Input Length':<12} {'Orig TPS':<12} {'Mod TPS':<12} {'Improvement %':<15} {'Orig Time':<12} {'Mod Time':<12}")
        print("-"*80)
        for metric in comparison_metrics:
            improvement_str = f"{metric['improvement_percentage']:.2f}%" if isinstance(metric['improvement_percentage'], (int, float)) else str(metric['improvement_percentage'])
            print(f"{metric['input_length']:<12} {metric['original_tps']:<12.2f} {metric['modified_tps']:<12.2f} {improvement_str:<15} "
                  f"{metric['original_avg_time']:<12.4f} {metric['modified_avg_time']:<12.4f}")

        # Calculate overall improvement
        orig_avg_tps = mean([r['tokens_per_second'] for r in original_results if r['tokens_per_second'] != float('inf')])
        mod_avg_tps = mean([r['tokens_per_second'] for r in modified_results if r['tokens_per_second'] != float('inf')])
        ...
        memory_improvement = ((orig_avg_memory - mod_avg_memory) / orig_avg_memory) * 100 if orig_avg_memory > 0 else 0
        
        print(f"Memory Efficiency Improvement: {memory_improvement:.2f}% (lower is better)")
    ...
    def save_results(self):
        ...
        json_filename = os.path.join(
            self.results_dir,
            f"qwen3_coder_30b_inference_speed_comparison_optimized_{timestamp}.json"
        )
        ...
        csv_filename = os.path.join(
            self.results_dir,
            f"qwen3_coder_30b_inference_speed_comparison_optimized_{timestamp}.csv"
        )
        with open(csv_filename, 'w', newline='') as f:
            ...
            # Write header
            writer.writerow([
                'Model Name', 'Input Length', 'Original TPS', 'Modified TPS',
                'Improvement Percentage', 'Original Avg Time', 'Modified Avg Time',
                'Original Std Time', 'Modified Std Time', 'Memory Improvement %',
                'Timestamp'
            ])
            ...
            for metric in self.results['comparison']:
                writer.writerow([
                    ...
                    ((metric['original_memory_stats'].get('system_memory_percent', 0) - 
                      metric['modified_memory_stats'].get('system_memory_percent', 0)) / 
                     metric['original_memory_stats'].get('system_memory_percent', 1)) * 100 
                     if metric['original_memory_stats'].get('system_memory_percent', 1) > 0 else 0,
    ...
    def start_monitoring(self):
        ...
        def monitor_job():
            if self.monitoring_active:
                print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] System Status Monitor")
                cpu_percent = psutil.cpu_percent()
                memory_info = psutil.virtual_memory()
                ...
                if torch.cuda.is_available():
                    ...
                    for i, gpu in enumerate(gpus):
                        print(f"  GPU {i}: {gpu.load*100:.1f}% load, {gpu.memoryUsed}/{gpu.memoryTotal} MB")
...
def run_qwen3_coder_30b_benchmark():
    """Run the Qwen3-Coder-30B inference speed comparison benchmark."""
    benchmark = Qwen3Coder30BInferenceSpeedComparisonBenchmark()
    ...
    try:
        ...
        print(f"Error running Qwen3-Coder-30B benchmark: {e}")
