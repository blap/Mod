...
from src.inference_pio.models.qwen3_coder_30b.attention.paged_attention import (
...
def run_benchmark(num_requests=10, prompt_len=128, gen_len=128, batch_size=4):
    print(
        f"Running Benchmark: Qwen3-Coder-30B (Requests={num_requests}, Prompt={prompt_len}, Gen={gen_len}, Batch={batch_size})"
    )
    ...
    attn = (
        Qwen3CoderPagedAttention(config, layer_idx=0, kv_cache=kv_cache)
        .to(device)
        .to(dtype)
    )
