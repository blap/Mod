...
This plugin provides optimized execution strategies for Intel Comet Lake
processors, specifically targeting the i5-10210U (4 cores / 8 threads).
"""
...
class IntelCometLakePlugin(HardwareProcessorPluginInterface):
    """
    Optimized plugin for Intel Comet Lake CPUs (e.g., i5-10210U).
    Focuses on MKL-DNN utilization and optimal threading for 4-core mobile
    chips. Also supports Hybrid Offloading (NVIDIA dGPU + Intel CPU) and
    IPEX acceleration.
    """

    def __init__(self, profile: Optional[SystemProfile] = None):
        self._config = {}
        # Default to physical cores for stability on mobile chips
        self._num_threads = 4
    ...
    def initialize(self, config: Dict[str, Any]) -> bool:
        ...
        # For i5-10210U, sticking to 4 physical cores is often better for
        # sustained FP32 inference to manage thermals and cache thrashing.
        ...
        # Ensure we don't accidentally rely on stale config if profile
        # says otherwise
        if (
            self._profile
            and not self._profile.hybrid_capability
            and not hybrid
        ):
            pass  # Stay false

        logger.info(
            f"IntelCometLakePlugin initialized. Threads: {self._num_threads}. "
    ...
    def scaled_dot_product_attention(
        ...
        """
        Standard SDPA. Intel CPUs benefit significantly from the 'math' or
        'flash' backends if available in newer torch versions.
        """
    ...
    def get_layer_distribution(
        ...
        # MX330 has 2GB, very tight. We leave ~0.5GB buffer ideally.
        ...
        logger.info(
            f"Hybrid Plan: {layers_on_gpu} layers on GPU, "
            f"{layers_on_cpu} layers on CPU "
...
def create_intel_comet_lake_plugin(
