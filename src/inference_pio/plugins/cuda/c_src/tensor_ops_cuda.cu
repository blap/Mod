// ... (Previous contents of tensor_ops_cuda.cu) ...
...
__global__ void gelu_kernel(float* input, float* out, int size) {
    ...
    if (idx < size) {
        ...
        out[idx] = 0.5f * x * (1.0f + tanhf(sqrt_2_pi * (x + c * x * x * x)));
    }
}
...
__global__ void rms_norm_kernel(float* input, float* weight, float* out, int rows, int cols, float eps) {
    ...
    // Load weight to shared memory if small? Naive first.
    float sum_sq = 0.0f;
    int base = row_idx * cols;
    ...
    for (int j = 0; j < cols; j++) {
        out[base + j] = (input[base + j] * rms) * weight[j];
    }
}
...
__global__ void matmul_kernel_naive(float* A, float* B, float* C, int TotalM, int K, int N, int Batch, int M_per_Batch, int broadcast_B) {
    ...
    if (row < TotalM && col < N) {
        ...
        int batch_idx = row / M_per_Batch;
        int m_idx = row % M_per_Batch;

        int a_base = row * K;
        int b_base_start = broadcast_B ? 0 : (batch_idx * K * N);

        for (int k = 0; k < K; k++) {
            sum += A[a_base + k] * B[b_base_start + k * N + col];
        }
        C[row * N + col] = sum;
    }
}

// A: [Batch, M, K], B: [Batch, N, K] (Transposed view)
// C: [Batch, M, N]
__global__ void matmul_transposed_kernel(float* A, float* B, float* C, int TotalM, int K, int N, int Batch, int M_per_Batch, int broadcast_B) {
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x; // Col in C corresponds to N dimension
    ...
    if (row < TotalM && col < N) {
        ...
        int a_base = row * K;
        // B is [Batch, N, K] in memory
        // We want B^T [Batch, K, N]
        // Element (k, col) in B^T is element (col, k) in B
        // B layout: batch_idx * N * K + col * K + k
        int b_base_start = broadcast_B ? 0 : (batch_idx * N * K);

        for (int k = 0; k < K; k++) {
            sum += A[a_base + k] * B[b_base_start + col * K + k];
        }
        C[row * N + col] = sum;
    }
}
...
__global__ void softmax_kernel(float* input, float* out, int rows, int cols) {
    ...
    float sum_exp = 0.0f;
    for (int j = 0; j < cols; j++) {
        float val = expf(input[base + j] - max_val);
        out[base + j] = val;
        sum_exp += val;
    }
...
EXPORT Tensor* create_tensor(int* shape, int ndim, int device_id) {
    ...
    if (device_id >= 0) {
        ...
        CUDA_CHECK(cudaMalloc((void**)&t->data, t->size * sizeof(float)));
    } else {
...
EXPORT void tensor_fill(Tensor* t, float value) {
    if (t->device_id >= 0) {
        ...
        fill_kernel<<<blocks, threads>>>(t->data, value, t->size);
...
EXPORT void tensor_add(Tensor* a, Tensor* b, Tensor* out) {
    if (out->device_id >= 0) {
        ...
        add_kernel<<<blocks, threads>>>(a->data, b->data, out->data, out->size);
        CUDA_CHECK(cudaDeviceSynchronize());
    }
}
...
EXPORT void tensor_mul(Tensor* a, Tensor* b, Tensor* out) {
    if (out->device_id >= 0) {
        int threads = 256;
        int blocks = (out->size + threads - 1) / threads;
        mul_kernel<<<blocks, threads>>>(a->data, b->data, out->data, out->size);
        CUDA_CHECK(cudaDeviceSynchronize());
    }
}
...
EXPORT void tensor_gelu(Tensor* input, Tensor* out) {
    if (out->device_id >= 0) {
        int threads = 256;
        int blocks = (out->size + threads - 1) / threads;
        gelu_kernel<<<blocks, threads>>>(input->data, out->data, out->size);
        CUDA_CHECK(cudaDeviceSynchronize());
    }
}
...
EXPORT void tensor_rms_norm(Tensor* input, Tensor* weight, Tensor* out, float eps) {
    if (out->device_id >= 0) {
        ...
        int threads = 1;
        int blocks = rows;
        rms_norm_kernel<<<blocks, threads>>>(input->data, weight->data, out->data, rows, cols, eps);
        CUDA_CHECK(cudaDeviceSynchronize());
    }
}
...
EXPORT void tensor_linear(Tensor* input, Tensor* weight, Tensor* bias, Tensor* out) {
    if (out->device_id >= 0) {
        ...
        dim3 threads(16, 16);
        dim3 blocks((N + 15) / 16, (TotalRows + 15) / 16);
...
EXPORT void tensor_softmax(Tensor* input, Tensor* out) {
    if (out->device_id >= 0) {
        ...
        int threads = 1;
        int blocks = rows;
        softmax_kernel<<<blocks, threads>>>(input->data, out->data, rows, cols);
        CUDA_CHECK(cudaDeviceSynchronize());
    }
}
...
EXPORT void tensor_silu(Tensor* input, Tensor* out) {
    if (out->device_id >= 0) {
        int threads = 256;
        int blocks = (out->size + threads - 1) / threads;
        silu_kernel<<<blocks, threads>>>(input->data, out->data, out->size);
        CUDA_CHECK(cudaDeviceSynchronize());
    }
}
...
EXPORT void tensor_rope(Tensor* q, Tensor* k, Tensor* cos, Tensor* sin, Tensor* out_q, Tensor* out_k) {
    if (out_q->device_id >= 0) {
        ...
        int threads = 256;
        int blocks = (total_tokens * (dim/2) + threads - 1) / threads;
...
EXPORT void tensor_load_data(Tensor* t, float* buffer, int size) {
    if (t->device_id >= 0) {
        CUDA_CHECK(cudaMemcpy(t->data, buffer, size * sizeof(float), cudaMemcpyHostToDevice));
    } else {
...
EXPORT void tensor_get_data(Tensor* t, float* buffer, int size) {
    if (t->device_id >= 0) {
        CUDA_CHECK(cudaMemcpy(buffer, t->data, size * sizeof(float), cudaMemcpyDeviceToHost));
    } else {
