"""
Tensor Utilities Module for Inference-PIO System

This module provides consolidated utility functions for tensor operations
across the Inference-PIO system. It includes functions for rotary embeddings,
tensor manipulations, and other common tensor operations.
"""
...
def calculate_tensor_memory(tensor: torch.Tensor) -> int:
    """
...
def move_tensor_to_device(
    ...
    """
    Move tensor to specified device if not already on it.
...
def concatenate_tensors(tensors: list, dim: int = 0) -> torch.Tensor:
    """
...
def stack_tensors(tensors: list, dim: int = 0) -> torch.Tensor:
    """
    Stack a list of tensors along the specified dimension.
