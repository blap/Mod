"""
Simple test for the dynamic configuration system in Inference-PIO.
"""
...
def test_config_manager():
    ...
    # Register the config
    success = config_manager.register_config("test_config", config)
    ...
    # Update the config
    update_success = config_manager.update_config("test_config", {"temperature": 0.7})
...
def test_config_loader():
    ...
    try:
        save_success = config_manager.save_config("save_test_config", temp_path, "json")
        print(f"Config save success: {save_success}")
        
        if save_success:
            # Load from the file
            load_success = config_loader.load_config_from_file(temp_path, "loaded_config")
...
def test_config_profiles():
    ...
    mem_config = create_config_from_profile("qwen3_4b", "memory_efficient", temperature=0.7)
