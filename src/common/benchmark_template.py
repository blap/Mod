...
class StandardizedModelBenchmarkTemplate(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.assertTrue(
            self.plugin is not None, "Plugin must be initialized in subclass"
        )
    ...
    def run_standard_accuracy_benchmarks(self):
        ...
        benchmarks = [AccuracyBenchmark(self.plugin, self.model_name)]
            ...
            if result.category == "performance":
                if result.unit in ["tokens/sec", "MB", "seconds"]:
                    self.assertGreaterEqual(
                        result.value,
                        0,
                        f"Performance value should be non-negative: {result.name}",
                    )
    ...
    def test_accuracy_benchmarks(self):
        ...
        for result in results:
            ...
            if "accuracy" in result.name:
                self.assertGreaterEqual(
                    result.value, 0, "Accuracy should be non-negative"
                )
                if result.unit == "ratio":
                    self.assertLessEqual(
                        result.value, 1.0, "Accuracy ratio should be <= 1.0"
                    )
...
class GLM47FlashBenchmark(StandardizedModelBenchmarkTemplate):
    ...
    def test_specific_inference_speeds(self):
        ...
        short_result = InferenceSpeedBenchmark(
            self.plugin, self.model_name, input_length=20
        ).run()
        medium_result = InferenceSpeedBenchmark(
            self.plugin, self.model_name, input_length=50
        ).run()
        long_result = InferenceSpeedBenchmark(
            self.plugin, self.model_name, input_length=100
        ).run()
