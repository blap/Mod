...
class AdaptiveSparseAttention(nn.Module):
    ...
    def _analyze_input_characteristics(
        ...
        entropy = -torch.sum(
            F.softmax(hidden_states, dim=-1) * F.log_softmax(hidden_states, dim=-1),
            dim=-1,
        ).mean(
            dim=0
        )  # Entropy per position
        magnitude = torch.norm(hidden_states, p=2, dim=-1).mean(
            dim=0
        )  # L2 norm per position

        # Detect sequence patterns
        position_variance = torch.var(hidden_states, dim=0).sum(
            dim=-1
        )  # How much each position varies across batch
        activation_density = (
            (hidden_states.abs() > 0.1).float().mean()
    ...
    def forward(
        ...
        if position_ids is not None:
            try:
                from ..utils.rotary_embeddings import (
                    GenericRotaryEmbedding as RotaryEmbedding,
                )
        ...
        if self.adaptive_strategy == "input_dependent":
            ...
            selected_pattern, adjusted_sparsity = self._select_adaptive_pattern(
                input_features
            )
        elif self.adaptive_strategy == "dynamic":
            # Use learnable pattern selector based on hidden states
            pattern_logits = self.pattern_selector(
            ...
            sparsity_pred = self.sparsity_activation(
                self.sparsity_predictor(hidden_states.mean(dim=1))
            )
    ...
    def _apply_longformer_attention(
        self, query_states, key_states, value_states, attention_mask, sparsity_ratio
    ):
        """
        ...
        sparse_mask = self._create_longformer_sparse_mask(
            q_len, sparsity_ratio, device=query_states.device
        )
    ...
    def _create_longformer_sparse_mask(self, seq_len, sparsity_ratio, device):
        ...
        adaptive_window_size = max(
            8, min(adaptive_window_size, seq_len)
    ...
    def _apply_bigbird_attention(
        ...
        sparse_mask = self._create_bigbird_sparse_mask(
            q_len, sparsity_ratio, device=query_states.device
        )
    ...
    def _create_bigbird_sparse_mask(self, seq_len, sparsity_ratio, device):
        ...
        if num_random_connections > 0:
            ...
            for idx in random_indices:
                ...
                other_indices = torch.randperm(seq_len, device=device)[
                    : max(1, adaptive_window_size // 2)
                ]
    ...
    def _apply_block_sparse_attention(
        self, query_states, key_states, value_states, attention_mask, sparsity_ratio
    ):
        """
        ...
        sparse_mask = self._create_block_sparse_mask(
            q_len, num_blocks, sparsity_ratio, device=query_states.device
        )
    ...
    def _create_block_sparse_mask(self, seq_len, num_blocks, sparsity_ratio, device):
        ...
        for block_row in range(num_blocks):
            for block_col in range(num_blocks):
                ...
                if (
                    torch.rand(1, device=device).item() < sparsity_ratio
                    or block_row == block_col
                ):
    ...
    def _apply_random_sparse_attention(
        self, query_states, key_states, value_states, attention_mask, sparsity_ratio
    ):
        """
        ...
        sparse_mask = self._create_random_sparse_mask(
            q_len, sparsity_ratio, device=query_states.device
        )
    ...
    def _apply_strided_sparse_attention(
        self, query_states, key_states, value_states, attention_mask, sparsity_ratio
    ):
        """
        ...
        sparse_mask = self._create_strided_sparse_mask(
            q_len, sparsity_ratio, device=query_states.device
        )
    ...
    def _create_strided_sparse_mask(self, seq_len, sparsity_ratio, device):
        ...
        adaptive_stride = max(
            1, int(1 / max(sparsity_ratio, 0.05))
