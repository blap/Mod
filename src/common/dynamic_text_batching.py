...
class DynamicTextBatchManager:
    ...
    def __init__(
        ...
        self.current_batch_size = max(
            min_batch_size, min(initial_batch_size, max_batch_size)
        )
        ...
        logger.info(
            f"DynamicTextBatchManager initialized with batch_size range [{min_batch_size}, {max_batch_size}], "
    ...
    def analyze_text_batch_type(self, texts: Union[str, List[str]]) -> TextBatchType:
        ...
        code_matches = sum(
            len(re.findall(pattern, full_text)) for pattern in code_patterns
        )
    ...
    def estimate_memory_usage(
        self, texts: Union[str, List[str]], batch_size: int
    ) -> float:
        """
        ...
        estimated_memory_gb = (
            batch_size * max_seq_len * memory_per_token_gb * 1.5
        )  # 1.5x safety factor

        return estimated_memory_gb

    def collect_text_batch_metrics(
        self,
        ...
        recommended_batch_size = max(
            self.min_batch_size, min(length_adjusted_size, self.max_batch_size)
        )
    ...
    def collect_metrics(
        ...
        processing_time_ms: float,
        tokens_processed: int,
        input_data: Optional[Union[str, List[str], torch.Tensor]] = None,
    ) -> BatchMetrics:
        ...
        if processing_time_ms > 0:
            throughput = (
                tokens_processed / processing_time_ms
        ...
        if input_data is not None:
            try:
                complexity_metrics = self._complexity_analyzer.analyze_input_complexity(
                    input_data
                )
                complexity_score = complexity_metrics.complexity_score
        ...
        normalized_throughput = min(
            throughput / 1000.0, 1.0
        )  # Assume 1000 tokens/sec is excellent
        normalized_latency = max(
            0.0, 1.0 - (latency / 100.0)
        )  # Assume 100ms/token is poor
    ...
    def should_adjust_batch_size(
        self,
    ) -> Tuple[bool, Optional[BatchSizeAdjustmentReason]]:
        ...
        if (
            latest_metrics.memory_pressure_ratio > self.memory_threshold_ratio
            or latest_metrics.gpu_memory_pressure_ratio > self.memory_threshold_ratio
        ):
            return True, BatchSizeAdjustmentReason.MEMORY_PRESSURE

        # Check for performance degradation
        if (
            latest_metrics.performance_score < self.performance_target * 0.8
            and latest_metrics.performance_score
            < previous_metrics.performance_score * 0.95
        ):
            return True, BatchSizeAdjustmentReason.PERFORMANCE_DEGRADATION

        # Check if memory is available for larger batches
        if (
            latest_metrics.memory_pressure_ratio < self.memory_threshold_ratio * 0.7
            and latest_metrics.batch_size < self.max_batch_size
            and latest_metrics.performance_score > self.performance_target
        ):
        ...
        if (
            ...
            and latest_metrics.batch_size < self.max_batch_size
            and latest_metrics.performance_score
            > previous_metrics.performance_score * 1.05
        ):
    ...
    def calculate_new_batch_size(
        ...
        text_batch_info: Optional[TextBatchInfo] = None,
    ) -> int:
        ...
        if reason == BatchSizeAdjustmentReason.MEMORY_PRESSURE:
            ...
            if len(self._recent_throughputs) >= 2:
                recent_avg = sum(list(self._recent_throughputs)[-3:]) / min(
                    3, len(self._recent_throughputs)
                )
                prev_avg = sum(list(self._recent_throughputs)[-6:-3]) / min(
                    3, max(1, len(self._recent_throughputs) - 3)
                )
        ...
        if text_batch_info is not None:
            ...
            if text_batch_info.complexity_score > 0.7:
                ...
                new_size = max(
                    self.min_batch_size,
                    int(new_size * (1.0 - text_batch_info.complexity_score * 0.3)),
                )
                ...
                new_size = min(
                    ...
                    int(
                        new_size
                        * (1.0 + (0.3 - text_batch_info.complexity_score) * 0.5)
                    ),
                )
    ...
    def adjust_batch_size(
        self, text_batch_info: Optional[TextBatchInfo] = None
    ) -> Tuple[int, bool, Optional[BatchSizeAdjustmentReason]]:
        """
        ...
        with self._lock:
            ...
            if new_batch_size != self.current_batch_size:
                ...
                logger.info(
                    f"Batch size adjusted from {old_size} to {new_batch_size} "
    ...
    def get_optimal_batch_size(
        ...
        self.collect_metrics(
            self.current_batch_size, processing_time_ms, tokens_processed, input_data
        )
        ...
        if (
            ...
            and isinstance(input_data, (str, list))
            and all(
                isinstance(x, str)
                for x in ([input_data] if isinstance(input_data, str) else input_data)
            )
        ):
        ...
        if input_data is not None and text_batch_info is not None:
            ...
            # Ensure within bounds
            combined_size = max(
                self.min_batch_size, min(combined_size, self.max_batch_size)
            )
    ...
    def force_batch_size(self, batch_size: int) -> bool:
        ...
        with self._lock:
            if self.min_batch_size <= batch_size <= self.max_batch_size:
                ...
                logger.warning(
                    f"Attempted to set batch size to {batch_size}, "
    ...
    def get_status_report(self) -> Dict[str, Any]:
        ...
        with self._lock:
            if self.metrics_history:
                latest_metrics = self.metrics_history[-1]
                avg_processing_time = (
                    sum(self._recent_processing_times)
                    / len(self._recent_processing_times)
                    if self._recent_processing_times
                    else 0
                )
                avg_throughput = (
                    sum(self._recent_throughputs) / len(self._recent_throughputs)
            ...
            return {
                ...
                "cooldown_remaining": max(
                    0, self.cooldown_period - (time.time() - self.last_adjustment_time)
                ),
