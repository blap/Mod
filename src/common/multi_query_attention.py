...
class MultiQueryAttention(nn.Module):
    ...
    def __init__(
        ...
        )  # Fixed: should be self.num_key_value_heads
        ...
        self.k_proj = nn.Linear(
            self.hidden_size, self.num_key_value_heads * self.kv_head_dim, bias=bias
        )
    ...
    def forward(
        ...
        key_states = (
            ...
            .view(bsz, q_len, self.num_key_value_heads, self.kv_head_dim)
            .transpose(1, 2)
        )
        value_states = (
        ...
        key_states = key_states.repeat(
            1, 1, self.num_attention_heads // self.num_key_value_heads, 1
        )
...
class GroupedQueryAttention(nn.Module):
    ...
    def __init__(
        ...
        if num_key_value_heads >= num_attention_heads:
            raise ValueError(
                "GroupedQueryAttention requires num_key_value_heads < num_attention_heads"
            )
...
def create_mqa_gqa_attention(
    ...
    if attention_type.lower() == "mqa":
        ...
        n_kv_heads = num_key_value_heads or getattr(
            config, "num_key_value_heads", config.num_attention_heads // 4
        )
        ...
        raise ValueError(
            f"Invalid attention type: {attention_type}. Use 'mqa' or 'gqa'."
        )


__all__ = ["MultiQueryAttention", "GroupedQueryAttention", "create_mqa_gqa_attention"]
