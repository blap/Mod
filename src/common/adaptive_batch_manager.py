...
class AdaptiveBatchManager:
    ...
    def __init__(
        ...
        logger.info(
            f"AdaptiveBatchManager initialized with batch_size range [{min_batch_size}, {max_batch_size}], "
    ...
    def collect_metrics(
        ...
        input_data: Optional[Union[str, list, torch.Tensor]] = None,
    ) -> BatchMetrics:
    ...
    def get_optimal_batch_size(
        ...
        if input_data is not None and self.use_input_complexity:
            try:
                ...
                complexity_based_size = (
                    self._complexity_analyzer.get_adaptive_batch_size(
                        complexity_score=complexity_metrics.complexity_score,
                ...
                logger.debug(
                    f"Complexity-based batch size adjustment: "
    ...
    def get_status_report(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if self._recent_complexities:
                avg_complexity = sum(self._recent_complexities) / len(
                    self._recent_complexities
                )
            else:
                avg_complexity = 0
