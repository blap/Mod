...
class PerformanceRegressionTracker:
        ...
        if self.baseline_file.exists():
            try:
                ...
                for baseline_data in data:
                    key = (
                        f"{baseline_data['model_name']}_{baseline_data['metric_name']}"
                    )
                ...
                print(
                    f"Warning: Could not load baselines from {self.baseline_file}: {e}"
                )
    ...
    def detect_regression(
        ...
        if metric_name in ["inference_speed", "throughput", "tokens_per_second"]:
            ...
            if baseline.value != 0:
                percentage_change = (
                    (current_value - baseline.value) / abs(baseline.value)
                ) * 100
            else:
    ...
    def get_performance_trend(
        self, model_name: str, metric_name: str, window_size: int = 10
    ) -> List[Dict[str, Any]]:
        """
    ...
    def calculate_statistics(
        ...
        return {
            ...
            "stdev": (
                statistics.stdev(relevant_values) if len(relevant_values) > 1 else 0.0
            ),
    ...
    def export_regression_report(
        self, output_file: str = "regression_report.json"
    ) -> None:
        """
        ...
        unique_combinations = set(
            (record["model_name"], record["metric_name"]) for record in self.history
        )
...
class PerformanceRegressionTestSuite:
    ...
    def run_regression_tests(
        self, test_data: List[Dict[str, Any]], threshold_percentage: float = 5.0
    ) -> List[PerformanceRegressionResult]:
        """
    ...
    def generate_regression_summary(
        self, results: List[PerformanceRegressionResult]
    ) -> str:
        """
        ...
        if regressions_detected > 0:
            ...
            for result in results:
                if result.regression_detected:
                    direction = (
                        "DECREASE"
                        if result.current_value < result.baseline_value
                        else "INCREASE"
                    )
                    ...
                    summary += (
                        f"Change={result.regression_percentage:.2f}% ({direction})\n"
                    )
...
def create_default_baselines(
    tracker: PerformanceRegressionTracker, model_name: str
) -> None:
    """
