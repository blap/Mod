...
class TensorDecomposer:
    ...
    def __init__(
        ...
        logger.info(
            f"TensorDecomposer initialized with method: {decomposition_method}, "
            f"rank_ratio: {rank_ratio}"
        )

    def decompose_tensor(
        self, tensor: torch.Tensor, tensor_id: str = "default"
    ) -> Tuple[Union[Dict[str, torch.Tensor], torch.Tensor], Dict[str, Any]]:
        """
        ...
        try:
            ...
            compression_ratio = (
                decomposed_size / original_size if original_size > 0 else 0
            )
            decomposition_metadata["actual_compression_ratio"] = compression_ratio
            saved_bytes = max(
                0, (original_size - decomposed_size) * tensor.element_size()
            )  # Ensure non-negative
            decomposition_metadata["compression_saved_bytes"] = saved_bytes
            decomposition_metadata["decomposition_failed"] = (
                False  # Explicitly mark as successful
            )
            ...
            logger.debug(
                f"Decomposed tensor {tensor_id}: {original_size} -> {decomposed_size} "
    ...
    def recompose_tensor(
        ...
        try:
            ...
            if method == "cp_decomposition":
                recomposed_tensor = self._recompose_cp(decomposed_data, metadata)
            elif method == "tucker_decomposition":
                recomposed_tensor = self._recompose_tucker(decomposed_data, metadata)
            elif method == "tensor_train":
                recomposed_tensor = self._recompose_tensor_train(
                    decomposed_data, metadata
                )
            elif method == "matrix_svd":
                recomposed_tensor = self._recompose_svd(decomposed_data, metadata)
            else:
            ...
            logger.debug(
                f"Recomposed tensor {tensor_id}, shape: {recomposed_tensor.shape}"
            )
            ...
            if (
                isinstance(decomposed_data, dict)
                and "original_tensor" in decomposed_data
            ):
    ...
    def _cp_decomposition(
        self, tensor: np.ndarray, tensor_id: str, target_ranks: List[int]
        ...
        if len(tensor.shape) == 2:
            ...
            metadata = {"rank_used": rank, "decomposition_type": "matrix_svd"}
        ...
        rank = min(
            (
                target_ranks[0]
                if target_ranks
                else max(1, int(effective_rank * self.rank_ratio))
            ),
            effective_rank,
        )
    ...
    def _recompose_cp(
        self, decomposed_data: Dict[str, torch.Tensor], metadata: Dict[str, Any]
    ) -> torch.Tensor:
        """
            ...
            try:
                ...
                if "original_shape" in decomposed_data:
                    original_shape = tuple(
                        int(x) for x in decomposed_data["original_shape"]
                    )
    ...
    def _tucker_decomposition(
        ...
        if n_modes == 2:
            ...
            metadata = {"ranks_used": [rank], "decomposition_type": "matrix_svd"}
            ...
            rank = min(
                (
                    target_ranks[0]
                    if target_ranks
                    else max(1, int(avg_dim * self.rank_ratio))
                ),
                min(original_shape),
            )
            ...
            metadata = {"ranks_used": [rank], "decomposition_type": "high_order_svd"}

            return decomposed_data, metadata

    def _recompose_tucker(
        self, decomposed_data: Dict[str, torch.Tensor], metadata: Dict[str, Any]
    ) -> torch.Tensor:
        """
    ...
    def _tensor_train_decomposition(
        self, tensor: np.ndarray, tensor_id: str, target_ranks: List[int]
    ) -> Tuple[Dict[str, torch.Tensor], Dict[str, Any]]:
        """
        ...
        if n_dims > 2:
            ...
            # Calculate rank
            rank = min(
                target_ranks[0] if target_ranks else min(left_dim, right_dim) // 2,
    ...
    def _matrix_svd_decomposition(
        self, tensor: np.ndarray, tensor_id: str, target_rank: int
    ) -> Tuple[Dict[str, torch.Tensor], Dict[str, Any]]:
        """
    ...
    def _n_mode_product(
        self, tensor: np.ndarray, matrix: np.ndarray, mode: int
    ) -> np.ndarray:
        """
        ...
        other_dims_size = int(
            np.prod(
                [original_shape[i] for i in range(len(original_shape)) if i != mode]
            )
        )
...
class AdaptiveTensorDecomposer(TensorDecomposer):
    ...
    def adjust_rank_ratio(self, accuracy_estimate: Optional[float] = None) -> float:
        ...
        logger.debug(
            f"Memory usage: {avg_memory:.3f}, Accuracy estimate: {accuracy_estimate}, "
...
def decompose_model_weights(
    ...
    for name, param in model.named_parameters():
        ...
        ):  # Only decompose trainable or multi-dimensional params
...
def recompose_model_weights(
    decomposed_model: nn.Module, decomposition_metadata: Dict[str, Any]
) -> nn.Module:
    """
