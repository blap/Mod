"""
Intelligent Tensor Pagination System for Unimodal Text Data

This module implements an intelligent pagination system optimized specifically for unimodal text data
with different memory access patterns and requirements compared to multimodal data.
"""
...
class TextDataType(Enum):
    """Enumeration of different text data types for unimodal pagination."""

    TEXT_EMBEDDINGS = "text_embeddings"
    TEXT_ACTIVATIONS = "text_activations"
    TEXT_KV_CACHE = "text_kv_cache"
    TEXT_ATTENTION_WEIGHTS = "text_attention_weights"
    TEXT_MLP_WEIGHTS = "text_mlp_weights"
    TEXT_LAYERNORM_WEIGHTS = "text_layernorm_weights"
...
@dataclass
class TextTensorPage:
    """Represents a memory page for text tensor pagination."""

    id: str
    tensor: Optional[Tensor] = None
    data_type: Optional[TextDataType] = None
    ...
    position_in_sequence: Optional[int] = None  # Position in the text sequence
    attention_head: Optional[int] = None  # For attention-related tensors
    is_past_key_value: bool = False  # For KV cache tensors
...
class TextAccessPatternAnalyzer:
    """Analyze access patterns to predict future page accesses for text data."""
    ...
    def __init__(self):
        ...
        self.sequence_positions = {}  # Maps page_id to sequence position for text data
        self.layer_positions = {}  # Maps page_id to layer index
    ...
    def record_access(
        ...
        with self.lock:
            self.access_history.append((timestamp, page_id, sequence_position))
            ...
            if sequence_position is not None:
                self.sequence_positions[page_id] = sequence_position
            if layer_index is not None:
                self.layer_positions[page_id] = layer_index
            ...
            if page_id in self.access_intervals:
                ...
                if prev_access is not None:
                    ...
                    if (
                        len(self.access_intervals[page_id]) > 10
    ...
    def get_access_score(self, page_id: str, current_time: float) -> float:
        ...
        with self.lock:
            ...
            recency_score = max(
                0.0, (current_time - last_access) / 10.0
            ...
            time_score = max(
                0.0, (predicted_next - current_time) / 5.0
            )  # Normalize by 5 seconds

            # Sequence position affects score (earlier positions in sequence may be less needed)
            seq_pos = self.sequence_positions.get(page_id, 0)
            seq_score = max(0.0, seq_pos / 1000.0)  # Normalize by 1000 positions

            # Layer position affects score (deeper layers may be more important)
            layer_pos = self.layer_positions.get(page_id, 0)
            layer_score = min(layer_pos / 100.0, 1.0)  # Normalize by 100 layers
    ...
    def get_access_frequency(self, page_id: str, current_time: float) -> float:
        ...
        with self.lock:
            ...
            recent_accesses = sum(
                1
                for t, pid, pos in self.access_history
...
class UnimodalTensorPaginationSystem:
    """Intelligent tensor pagination system optimized for unimodal text data."""
    ...
    def __init__(
        ...
        self.swap_directory = (
            Path(swap_directory) if swap_directory else Path("./text_tensor_swap")
        )
        ...
        # Track memory pages
        self.pages: Dict[str, TextTensorPage] = {}
        ...
        self.access_analyzer = TextAccessPatternAnalyzer()
        ...
        logger.info(
            f"Initialized UnimodalTensorPaginationSystem with swap_directory={self.swap_directory}, "
    ...
    def allocate_page(
        ...
        page = TextTensorPage(
            ...
            is_past_key_value=is_past_key_value,
        )
        ...
        self.access_analyzer.record_access(
            page_id, current_time, access_pattern, position_in_sequence, layer_index
        )
        ...
        logger.debug(
            f"Allocated page {page_id} ({size_bytes} bytes) in RAM, data_type={data_type.value}"
        )
        return True
    ...
    def swap_page_to_disk(self, page_id: str) -> bool:
        ...
        # Create swap file
        swap_file = self.swap_directory / f"text_page_{page_id}.pkl"
    ...
    def _handle_memory_pressure(self):
        ...
        if self.eviction_policy == "lru":
            ...
            pages_to_consider.sort(
                key=lambda x: self._calculate_intelligent_score(x[0], time.time())
            )
        else:
            # Default to LRU
            pages_to_consider.sort(key=lambda x: self.access_times[x[0]])
        ...
        for i, (page_id, page) in enumerate(pages_to_consider):
            ...
            if self.swap_page_to_disk(page_id):
                logger.debug(
                    f"Evicted page {page_id} due to memory pressure using {self.eviction_policy} strategy"
                )
            else:
    ...
    def _calculate_intelligent_score(self, page_id: str, current_time: float) -> float:
        ...
        # Data type factor (some types are more critical than others for text models)
        type_factor = {
            TextDataType.TEXT_KV_CACHE: 0.8,  # KV cache is important but can be evicted
            TextDataType.TEXT_ACTIVATIONS: 0.6,  # Activations can be recomputed
            TextDataType.TEXT_EMBEDDINGS: 0.7,  # Text embeddings somewhat important
            TextDataType.TEXT_ATTENTION_WEIGHTS: 0.9,  # Attention weights are critical
            TextDataType.TEXT_MLP_WEIGHTS: 0.9,  # MLP weights are critical
            TextDataType.TEXT_LAYERNORM_WEIGHTS: 0.9,  # LayerNorm weights are critical
            TextDataType.TEXT_INPUT_TOKENS: 0.5,  # Input tokens can be reloaded
            TextDataType.TEXT_OUTPUT_TOKENS: 0.4,  # Output tokens can be stored elsewhere
            TextDataType.TEXT_HIDDEN_STATES: 0.6,  # Hidden states are important
        ...
        # Position in sequence (earlier positions may be less needed)
        if page.position_in_sequence is not None:
            position_factor = min(
                page.position_in_sequence / 1000.0, 1.0
            )  # Normalize by 1000
        else:
            position_factor = 0.5  # Default if not specified

        # Is past key-value (KV cache tensors)
        kv_factor = (
            0.3 if page.is_past_key_value else 0.7
        )  # Past KV can be evicted more easily

        # Combine factors (lower score = more likely to be evicted)
        score = (
            access_score * 0.2
            + (5 - priority_factor)
            * 0.2  # Invert priority (lower priority = higher score)
            + size_factor * 0.2
            + (1 - type_factor) * 0.15
            + temporal_factor * 0.1
            + position_factor * 0.1
...
class UnimodalTensorPager:
    """Specialized pager for unimodal text data with different optimization strategies."""

    def __init__(self, pagination_system: UnimodalTensorPaginationSystem):
    ...
    def page_tensor(
        ...
        page_id = f"text_tensor_{data_type.value}_{tensor_id}_{id(tensor)}"
    ...
    def _get_data_type_frequency(self, data_type: TextDataType) -> float:
        """Get expected access frequency for a data type."""
        frequency_map = {
            TextDataType.TEXT_KV_CACHE: 0.9,  # Very high frequency
            TextDataType.TEXT_ACTIVATIONS: 0.7,  # High frequency
            TextDataType.TEXT_EMBEDDINGS: 0.6,  # Medium-high frequency
            TextDataType.TEXT_ATTENTION_WEIGHTS: 0.95,  # Very high frequency
            TextDataType.TEXT_MLP_WEIGHTS: 0.95,  # Very high frequency
            TextDataType.TEXT_LAYERNORM_WEIGHTS: 0.9,  # High frequency
            TextDataType.TEXT_INPUT_TOKENS: 0.4,  # Medium-low frequency
            TextDataType.TEXT_OUTPUT_TOKENS: 0.3,  # Low frequency
            TextDataType.TEXT_HIDDEN_STATES: 0.6,  # Medium frequency
        }
        return frequency_map.get(data_type, 0.5)


def create_unimodal_pagination_system(
    swap_directory: Optional[Union[str, Path]] = None,
    page_size_mb: int = 16,
    eviction_policy: str = "intelligent",
    max_memory_ratio: float = 0.8,
) -> Tuple[UnimodalTensorPaginationSystem, UnimodalTensorPager]:
    """Create a unimodal pagination system with pager."""
    pagination_system = UnimodalTensorPaginationSystem(
    ...
    pager = UnimodalTensorPager(pagination_system)
