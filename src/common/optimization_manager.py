...
class OptimizationRegistry:
    ...
    def create_instance(
        self, name: str, config: OptimizationConfig
...
class ModularOptimizationManager:
    ...
    def __init__(self):
        ...
        self.applied_to_models: Dict[str, List[str]] = (
            {}
        )  # model_id -> list of applied opt names
        self.optimization_configs: Dict[str, OptimizationConfig] = {}

    def register_optimization(
        self, name: str, optimization_class: Type[OptimizationInterface]
    ):
    ...
    def apply_optimizations(
        self, model: nn.Module, optimization_names: List[str] = None
    ) -> nn.Module:
        """
        ...
        if not self._validate_dependencies(sorted_names):
            logger.error(
                "Dependency validation failed, aborting optimization application"
            )
            return model
        ...
        for opt_name in sorted_names:
            ...
            if optimization is None:
                logger.error(
                    f"Failed to create optimization instance for '{opt_name}', skipping"
                )
                continue

            if not optimization.is_applicable(model):
                logger.info(
                    f"Optimization '{opt_name}' not applicable to this model, skipping"
                )
                continue
    ...
    def remove_optimizations(
        ...
        optimization_names = [
            name for name in reversed(applied_to_model) if name in optimization_names
        ]
        ...
        for opt_name in optimization_names:
            if opt_name in self.active_optimizations:
                ...
                try:
                    ...
                    if (
                        model_id in self.applied_to_models
                        and opt_name in self.applied_to_models[model_id]
                    ):
    ...
    def _sort_optimizations_by_priority(
        self, optimization_names: List[str]
    ) -> List[str]:
            ...
            if config:
                for dep in config.dependencies:
                    if dep not in optimization_names:
                        logger.error(
                            f"Optimization '{name}' depends on '{dep}' which is not in the list"
                        )
                        return False
        return True
