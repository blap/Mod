...
class InputComplexityAnalyzer:
    ...
    def analyze_input_complexity(
        self, input_data: Union[str, List[str], torch.Tensor, List[torch.Tensor]]
    ) -> ComplexityMetrics:
        """
        ...
        if isinstance(input_data, str):
            ...
            isinstance(item, torch.Tensor) for item in input_data
        ):
            # Average complexity across multiple tensor inputs
            complexities = [
                self._analyze_tensor_complexity(tensor) for tensor in input_data
            ]
            return self._aggregate_complexities(
                [self._tensor_to_complexity_metrics(c) for c in complexities]
            )
        else:
    ...
    def _analyze_text_complexity(self, text: str) -> ComplexityMetrics:
        ...
        vocabulary_richness = (
            len(unique_tokens) / token_count if token_count > 0 else 0.0
        )

        # Syntactic complexity: based on sentence structure
        sentences = [s.strip() for s in re.split(r"[.!?]+", text) if s.strip()]
        avg_sentence_length = (
            np.mean([len(self.token_pattern.findall(s)) for s in sentences])
            if sentences
            else 0
        )
        clause_separators = len(re.findall(r",|;|:", text))
        syntactic_complexity = min(
            1.0, (avg_sentence_length * 0.1 + clause_separators * 0.05) / 10.0
        )
    ...
    def _analyze_tensor_complexity(self, tensor: torch.Tensor) -> Dict[str, Any]:
        ...
        vocabulary_richness = (
            unique_count / sequence_length if sequence_length > 0 else 0.0
        )

        # Numerical complexity (how much variation there is)
        numerical_complexity = min(
            1.0, (std_val / (abs(mean_val) + 1e-8)) if mean_val != 0 else std_val
        )
    ...
    def _tensor_to_complexity_metrics(
        self, tensor_analysis: Dict[str, Any]
    ) -> ComplexityMetrics:
    ...
    def _aggregate_complexities(
        ...
        # Calculate averages
        avg_seq_len = int(
            sum(c.sequence_length for c in complexities) / len(complexities)
        )
        avg_vocab_richness = sum(c.vocabulary_richness for c in complexities) / len(
            complexities
        )
        avg_syntactic_complexity = sum(
            c.syntactic_complexity for c in complexities
        ) / len(complexities)
        avg_semantic_density = sum(c.semantic_density for c in complexities) / len(
            complexities
        )
        avg_numerical_ratio = sum(
            c.numerical_content_ratio for c in complexities
        ) / len(complexities)
        avg_special_ratio = sum(c.special_character_ratio for c in complexities) / len(
            complexities
        )
        avg_complexity_score = sum(c.complexity_score for c in complexities) / len(
            complexities
        )
    ...
    def get_adaptive_batch_size(
        ...
        if complexity_score <= complexity_threshold_low:
            ...
            normalized_complexity = (
                complexity_score - complexity_threshold_low
            ) / complexity_range
            ...
            calculated_batch_size = int(
                base_max_batch - (normalized_complexity * batch_range)
            )
