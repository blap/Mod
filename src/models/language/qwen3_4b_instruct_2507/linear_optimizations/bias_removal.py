...
def apply_bias_removal_to_model(
    ...
    for name, module in model.named_modules():
        if config.remove_bias_in_attention and isinstance(
            module, (nn.MultiheadAttention, nn.Linear)
        ):
            # Check if this is part of attention mechanism
            if any(
                part in name.lower()
                for part in ["attn", "attention", "self_attn", "q_", "k_", "v_", "proj"]
            ):
        ...
        elif config.remove_bias_in_embeddings and isinstance(
            module, (nn.Embedding, nn.EmbeddingBag)
        ):
            # Remove bias from embeddings if configured
            if hasattr(module, "bias") and module.bias is not None:
                module.bias = None
                removed_components["embedding_biases"] += 1
        elif config.remove_bias_after_norm and isinstance(
...
__all__ = ["BiasRemovalConfig", "apply_bias_removal_to_model"]
