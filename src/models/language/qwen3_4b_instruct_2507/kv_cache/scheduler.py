"""
Qwen3-4B-Instruct-2507 Continuous Batching Scheduler

This module implements iteration-level scheduling (Orca-style) for continuous batching.
It manages the allocation of KV cache blocks and the scheduling of requests.
"""
...
class ContinuousBatchingScheduler:
    """
    Scheduler for Continuous Batching (Iteration Level Scheduling).
    """
    ...
    def add_request(
        ...
        logger.info(
            f"Added request {request_id} to waiting queue. Queue length: {len(self.waiting_queue)}"
        )

    def _allocate_blocks_for_request(
        self, request: Request, num_tokens_to_alloc: int
    ) -> bool:
        """Allocate blocks for a request. Returns True if successful."""
        num_blocks_needed = (
            num_tokens_to_alloc + self.block_size - 1
        ) // self.block_size
    ...
    def schedule(self) -> Tuple[List[Request], List[List[int]], List[int], List[int]]:
        """
        Select requests to run for the next iteration.

        Returns:
            running_requests: List of Request objects
            block_tables: List of block tables (one per request)
            seq_lens: List of current sequence lengths (total)
            input_token_ids: Flattened list of input tokens (prompt for new, last token for running)
        """
        # 1. Check if we can add new requests from waiting queue
        while self.waiting_queue and len(self.running_queue) < self.max_batch_size:
            candidate_request = self.waiting_queue[0]

            # For new request, we need to allocate blocks for the PROMPT
            ...
            if self._allocate_blocks_for_request(candidate_request, num_prompt_tokens):
                ...
                # Not enough memory for new request, stop adding
                break

        # 2. Allocate space for the NEXT token for all running requests
        # (This is a simplification; in reality, we only need to allocate if the current block is full)
        requests_to_preempt = []

        for request in self.running_queue:
            # Check if current last block is full
            # Total length so far is prompt + generated
            current_len = request.total_len

            # If current length is a multiple of block size, we need a new block for the NEXT token
        ...
        # Handle preemption (simplified: just remove and fail for now, or swap to CPU in full implementation)
        for req in requests_to_preempt:
            logger.warning(
                f"Preempting request {req.request_id} due to memory pressure"
            )
            self.running_queue.remove(req)
            self.kv_cache.free_blocks(req.block_table)
            # In a real system, put back to waiting queue or swap
    ...
    def update_request_status(self, request: Request, new_token_id: int):
        """Update request with newly generated token."""
    ...
    def free_finished_requests(self):
        ...
        for i, req in enumerate(self.running_queue):
            if req.finished:
                ...
                self.kv_cache.free_blocks(req.block_table)
                logger.info(
                    f"Request {req.request_id} finished. Freed {len(req.block_table)} blocks."
                )

        # Remove from running queue (in reverse order to keep indices valid)
