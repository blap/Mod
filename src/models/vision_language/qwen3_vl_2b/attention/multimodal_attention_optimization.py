...
class Qwen3VL2BMultimodalAttentionOptimizer(BaseQwen3VL2BMultimodalAttentionOptimizer):
    ...
    def forward(
        ...
        if vision_seq_len > 0 and lang_seq_len > 0:
            ...
            if (
                base_output.size(1) >= min_len
            ):  # Assuming base_output has shape (batch, seq_len, hidden)
                relevant_output = base_output[
                    :, :min_len, :
                ]  # Take first min_len tokens
            else:
            ...
            if relevant_output.size(1) > 0:
                ...
                avg_output = torch.mean(
                    relevant_output, dim=1, keepdim=True
                )  # Shape: (batch, 1, hidden_size)

                # Create combined representation for gating
                combined_for_gating = torch.cat(
                    [avg_output, avg_output], dim=-1
                )  # Shape: (batch, 1, 2*hidden_size)

                # Apply gates to control information flow between modalities
                vision_language_gate = torch.sigmoid(
                    self.vision_language_gate(combined_for_gating.squeeze(1))
                ).unsqueeze(1)
                language_vision_gate = torch.sigmoid(
                    self.language_vision_gate(combined_for_gating.squeeze(1))
                ).unsqueeze(1)
...
class Qwen3VL2BAttentionManager(BaseQwen3VL2BAttentionManager):
    ...
    def apply_optimizations_to_model(self, model: nn.Module) -> nn.Module:
        ...
        """
        logger.info(
            "Applying Qwen3-VL-2B specific multimodal attention optimizations..."
        )
                ...
                try:
                    ...
                    if hasattr(layer, "self_attn"):
                        ...
                        if optimized_attn is not None:
                            ...
                            replaced_count += 1
                            logger.debug(
                                f"Replaced attention in layer {idx} with Qwen3-VL-2B optimized multimodal attention"
                            )
        ...
        logger.info(
            f"Qwen3-VL-2B multimodal attention optimizations applied: {replaced_count}/{total_layers} attention layers replaced"
        )

        return model


def create_qwen3_vl_multimodal_attention_optimizer(
...
def apply_qwen3_vl_multimodal_attention_optimizations_to_model(
...
def get_qwen3_vl_multimodal_attention_optimization_report(
    model: nn.Module, config: Qwen3VL2BConfig
) -> Dict[str, Any]:
    """
