...
from src.models.qwen3_vl_2b.attention.paged_attention import Qwen3VL2BPagedAttention
from src.models.qwen3_vl_2b.config import Qwen3VL2BConfig
from src.models.qwen3_vl_2b.kv_cache.paged_kv_cache import PagedKVCache
from src.models.qwen3_vl_2b.kv_cache.scheduler import ContinuousBatchingScheduler


def run_benchmark(num_requests=10, prompt_len=128, gen_len=128, batch_size=4):
    print(
        f"Running Benchmark: Qwen3-VL-2B (Requests={num_requests}, Prompt={prompt_len}, Gen={gen_len}, Batch={batch_size})"
    )
    ...
    attn = (
        Qwen3VL2BPagedAttention(config, layer_idx=0, kv_cache=kv_cache)
        .to(device)
        .to(dtype)
    )
    ...
    while scheduler.waiting_queue or scheduler.running_queue:
        ...
        with torch.no_grad():
            output, _, _ = attn(
                hidden_states, block_tables=block_tables, seq_lens=seq_lens
            )
