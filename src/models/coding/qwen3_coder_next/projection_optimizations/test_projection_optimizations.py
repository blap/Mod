"""
Test suite for Qwen3-Coder-Next Projection Optimizations Implementation

This module tests the projection layer optimizations for the Qwen3-Coder-Next model.
"""

import unittest
import torch
import torch.nn as nn
from src.models.coding.qwen3_coder_next.projection_optimizations import (
    Qwen3CoderNextProjectionLayer,
    Qwen3CoderNextMultiProjectionLayer,
    create_qwen3_coder_next_projection_layer,
    create_qwen3_coder_next_multi_projection_layer,
    apply_qwen3_coder_next_projection_optimizations
)
from src.models.coding.qwen3_coder_next.projection_optimizations.config import get_qwen3_coder_next_projection_config


class BaseUnitTest(unittest.TestCase):
    """Base test class for Qwen3-Coder-Next projection optimizations."""
    
    def setUp(self):
        """Set up test fixtures before each test method."""
        self.batch_size = 2
        self.seq_len = 10
        self.input_dim = 512
        self.output_dim = 256
        self.test_input = torch.randn(self.batch_size, self.seq_len, self.input_dim)
        self.syntax_ids = torch.randint(0, 512, (self.batch_size, self.seq_len))


class TestQwen3CoderNextProjectionLayer(BaseUnitTest):
    """Test cases for Qwen3-Coder-Next projection layer implementation."""
    
    def test_projection_layer_initialization(self):
        """Test that the projection layer initializes correctly."""
        layer = Qwen3CoderNextProjectionLayer(
            input_dim=self.input_dim,
            output_dim=self.output_dim
        )
        
        self.assertIsInstance(layer, Qwen3CoderNextProjectionLayer)
        self.assertIsInstance(layer.input_projection, nn.Linear)
        self.assertIsInstance(layer.output_projection, nn.Linear)
        self.assertEqual(layer.input_projection.in_features, self.input_dim)
        self.assertEqual(layer.output_projection.out_features, self.output_dim)
    
    def test_projection_layer_forward_pass(self):
        """Test the forward pass of the projection layer."""
        layer = Qwen3CoderNextProjectionLayer(
            input_dim=self.input_dim,
            output_dim=self.output_dim
        )
        
        output = layer(self.test_input)
        
        self.assertEqual(output.shape, (self.batch_size, self.seq_len, self.output_dim))
        self.assertTrue(torch.isfinite(output).all())
    
    def test_projection_layer_with_syntax_aware_projections(self):
        """Test the projection layer with syntax-aware projections."""
        layer = Qwen3CoderNextProjectionLayer(
            input_dim=self.input_dim,
            output_dim=self.output_dim,
            use_syntax_aware_projections=True
        )
        
        self.assertIsNotNone(layer.syntax_embedding)
        self.assertIsInstance(layer.syntax_embedding, nn.Embedding)
        self.assertIsInstance(layer.syntax_projection, nn.Linear)
        
        output = layer(self.test_input, self.syntax_ids)
        
        self.assertEqual(output.shape, (self.batch_size, self.seq_len, self.output_dim))
        self.assertTrue(torch.isfinite(output).all())
    
    def test_projection_layer_with_low_rank(self):
        """Test the projection layer with low-rank optimization."""
        layer = Qwen3CoderNextProjectionLayer(
            input_dim=self.input_dim,
            output_dim=self.output_dim,
            use_low_rank=True,
            low_rank_dim=64
        )
        
        # Check that intermediate projection exists
        self.assertTrue(hasattr(layer, 'intermediate_projection'))
        self.assertIsInstance(layer.intermediate_projection, nn.Linear)
        
        output = layer(self.test_input)
        
        self.assertEqual(output.shape, (self.batch_size, self.seq_len, self.output_dim))
        self.assertTrue(torch.isfinite(output).all())
    
    def test_projection_layer_with_group_norm(self):
        """Test the projection layer with group normalization."""
        layer = Qwen3CoderNextProjectionLayer(
            input_dim=self.input_dim,
            output_dim=self.output_dim,
            use_group_norm=True,
            group_norm_num_groups=16
        )
        
        self.assertIsNotNone(layer.group_norm)
        self.assertIsInstance(layer.group_norm, nn.GroupNorm)
        
        output = layer(self.test_input)
        
        self.assertEqual(output.shape, (self.batch_size, self.seq_len, self.output_dim))
        self.assertTrue(torch.isfinite(output).all())
    
    def test_projection_layer_with_code_specific_optimizations(self):
        """Test the projection layer with code-specific optimizations."""
        layer = Qwen3CoderNextProjectionLayer(
            input_dim=self.input_dim,
            output_dim=self.output_dim,
            use_code_specific_optimizations=True
        )
        
        self.assertIsNotNone(layer.code_context_enhancement)
        self.assertIsInstance(layer.code_context_enhancement, nn.Linear)
        self.assertIsInstance(layer.code_pattern_extractor, nn.Conv1d)
        
        output = layer(self.test_input)
        
        self.assertEqual(output.shape, (self.batch_size, self.seq_len, self.output_dim))
        self.assertTrue(torch.isfinite(output).all())
    
    def test_projection_layer_with_dynamic_routing(self):
        """Test the projection layer with dynamic routing (MoE)."""
        layer = Qwen3CoderNextProjectionLayer(
            input_dim=self.input_dim,
            output_dim=self.output_dim,
            use_dynamic_routing=True,
            num_experts=2
        )
        
        self.assertIsNotNone(layer.moe_router)
        self.assertIsInstance(layer.moe_router, nn.Linear)
        self.assertIsInstance(layer.experts, nn.ModuleList)
        self.assertEqual(len(layer.experts), 2)
        
        output = layer(self.test_input)
        
        self.assertEqual(output.shape, (self.batch_size, self.seq_len, self.output_dim))
        self.assertTrue(torch.isfinite(output).all())
    
    def test_projection_layer_different_activations(self):
        """Test the projection layer with different activation functions."""
        activations = ["silu", "gelu", "relu"]
        
        for activation in activations:
            layer = Qwen3CoderNextProjectionLayer(
                input_dim=self.input_dim,
                output_dim=self.output_dim,
                activation=activation
            )
            
            output = layer(self.test_input)
            
            self.assertEqual(output.shape, (self.batch_size, self.seq_len, self.output_dim))
            self.assertTrue(torch.isfinite(output).all())


class TestQwen3CoderNextMultiProjectionLayer(BaseUnitTest):
    """Test cases for Qwen3-Coder-Next multi-projection layer implementation."""
    
    def test_multi_projection_layer_initialization(self):
        """Test that the multi-projection layer initializes correctly."""
        layer = Qwen3CoderNextMultiProjectionLayer(
            input_dim=self.input_dim,
            output_dim=self.output_dim,
            num_layers=2
        )
        
        self.assertIsInstance(layer, Qwen3CoderNextMultiProjectionLayer)
        self.assertEqual(len(layer.projection_layers), 2)
        self.assertIsInstance(layer.final_projection, nn.Linear)
        
        for proj_layer in layer.projection_layers:
            self.assertIsInstance(proj_layer, Qwen3CoderNextProjectionLayer)
    
    def test_multi_projection_layer_forward_pass(self):
        """Test the forward pass of the multi-projection layer."""
        layer = Qwen3CoderNextMultiProjectionLayer(
            input_dim=self.input_dim,
            output_dim=self.output_dim,
            num_layers=2
        )
        
        output = layer(self.test_input, self.syntax_ids)
        
        self.assertEqual(output.shape, (self.batch_size, self.seq_len, self.output_dim))
        self.assertTrue(torch.isfinite(output).all())


class TestQwen3CoderNextProjectionFactoryFunctions(BaseUnitTest):
    """Test cases for Qwen3-Coder-Next projection layer factory functions."""
    
    def test_create_qwen3_coder_next_projection_layer(self):
        """Test creating a Qwen3-Coder-Next projection layer."""
        config = get_qwen3_coder_next_projection_config(
            input_dim=self.input_dim,
            output_dim=self.output_dim
        )
        
        layer = create_qwen3_coder_next_projection_layer(config)
        
        self.assertIsInstance(layer, Qwen3CoderNextProjectionLayer)
        self.assertEqual(layer.input_dim, self.input_dim)
        self.assertEqual(layer.output_dim, self.output_dim)
    
    def test_create_qwen3_coder_next_multi_projection_layer(self):
        """Test creating a Qwen3-Coder-Next multi-projection layer."""
        config = get_qwen3_coder_next_projection_config(
            input_dim=self.input_dim,
            output_dim=self.output_dim,
            num_projection_layers=3
        )
        
        layer = create_qwen3_coder_next_multi_projection_layer(config)
        
        self.assertIsInstance(layer, Qwen3CoderNextMultiProjectionLayer)
        self.assertEqual(layer.num_layers, 3)


class TestApplyQwen3CoderNextProjectionOptimizations(BaseUnitTest):
    """Test cases for applying Qwen3-Coder-Next projection optimizations to a model."""
    
    def test_apply_qwen3_coder_next_projection_optimizations(self):
        """Test applying projection optimizations to a model."""
        # Create a simple model with projection layers
        class SimpleModel(nn.Module):
            def __init__(self):
                super().__init__()
                self.embedding_projection = nn.Linear(self.input_dim, self.output_dim)
                self.output_projection = nn.Linear(self.output_dim, self.input_dim)
                self.other_layer = nn.Linear(self.input_dim, self.input_dim)
        
        model = SimpleModel()
        config = get_qwen3_coder_next_projection_config(
            input_dim=self.input_dim,
            output_dim=self.output_dim
        )
        
        # Apply projection optimizations
        optimized_model = apply_qwen3_coder_next_projection_optimizations(model, config)
        
        # Check that projection layers were replaced
        self.assertIsInstance(optimized_model.embedding_projection, Qwen3CoderNextProjectionLayer)
        self.assertIsInstance(optimized_model.output_projection, Qwen3CoderNextProjectionLayer)
        self.assertIsInstance(optimized_model.other_layer, nn.Linear)


if __name__ == "__main__":
    unittest.main()