"""
Qwen3-Coder-30B Tensor Parallel Implementation

This module implements tensor parallelism for the Qwen3-Coder-30B model.
"""
...
class ColumnParallelLinear(nn.Module):
    ...
    def __init__(
        ...
        if bias:
            self.bias = nn.Parameter(torch.empty(self.partitioned_output_size))
        else:
...
def convert_linear_to_tensor_parallel(
    model: nn.Module, config: TensorParallelConfig
) -> nn.Module:
    """
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            ...
            is_output_projection = any(
                proj in name for proj in ["o_proj", "down_proj", "lm_head"]
            )
            ...
            with torch.no_grad():
                if is_output_projection:
                    ...
                    partitioned_size = (
                        module.out_features // config.tensor_parallel_size
                    )
                    start_idx = config.local_rank * partitioned_size
                    end_idx = (config.local_rank + 1) * partitioned_size
...
def validate_tensor_parallelism_compatibility(
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            ...
            if (
                module.in_features % tensor_parallel_size != 0
                and module.out_features % tensor_parallel_size != 0
            ):
                return (
                    False,
                    f"Linear layer '{name}' has dimensions ({module.in_features}, {module.out_features}) not divisible by tensor_parallel_size {tensor_parallel_size}",
                )
            elif module.out_features % tensor_parallel_size != 0:
                # For column parallel, output must be divisible
                if not any(
                    proj in name for proj in ["o_proj", "down_proj", "lm_head"]
                ):  # Not an output projection
                    return (
                        False,
                        f"Column-parallel linear layer '{name}' output dimension {module.out_features} not divisible by tensor_parallel_size {tensor_parallel_size}",
                    )
            elif module.in_features % tensor_parallel_size != 0:
                # For row parallel, input must be divisible
                if any(
                    proj in name for proj in ["o_proj", "down_proj", "lm_head"]
                ):  # Is an output projection
                    return (
                        False,
                        f"Row-parallel linear layer '{name}' input dimension {module.in_features} not divisible by tensor_parallel_size {tensor_parallel_size}",
                    )
    ...
    if tensor_parallel_size > torch.cuda.device_count():
        return (
            False,
            f"tensor_parallel_size {tensor_parallel_size} exceeds available GPU count {torch.cuda.device_count()}",
        )
...
def initialize_tensor_parallelism(config: TensorParallelConfig) -> bool:
    ...
    try:
        ...
        if config.tensor_parallel_size > torch.cuda.device_count():
            print(
                f"Warning: Requested tensor parallel size {config.tensor_parallel_size} "
    ...
    try:
        ...
        if not is_compatible:
            return (
                model,
                False,
                f"Model not compatible with tensor parallelism: {reason}",
            )
