#!/usr/bin/env python
"""
Updated Real Model Tests - Execute tests with actual models and real data
instead of using mocks. This script verifies that all functionality
works with real models and datasets, with proper H drive detection and model loading.
"""
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                from inference_pio.models.qwen3_0_6b.config import Qwen3_0_6B_Config
                from inference_pio.core.model_loader import ModelLoader
            except ImportError as e2:
                print(f"[FAIL] Alternative import also failed: {str(e2)}")
                return False
        ...
        # Test config creation with H drive detection
        ...
        # Check if H drive is available
        h_drive = ModelLoader.get_h_drive_base()
        if h_drive:
            print(f"[INFO] H drive detected: {h_drive}")
        else:
            print("[INFO] H drive not detected, will attempt to download or use local cache")

        # Initialize with a smaller model or local model if available
        # For this test, we'll try to initialize with a minimal config
        try:
            # Attempt to resolve model path using the ModelLoader
            resolved_path = ModelLoader.resolve_model_path("qwen3-0_6b", "Qwen/Qwen3-0.6B")
            config.model_path = resolved_path
            print(f"[INFO] Resolved model path: {resolved_path}")

            # Initialize with the resolved config
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                from inference_pio.models.qwen3_coder_next.config import Qwen3CoderNextConfig
                from inference_pio.core.model_loader import ModelLoader
            except ImportError as e2:
                print(f"[FAIL] Alternative import also failed: {str(e2)}")
                return False
        ...
        try:
            # Attempt to resolve model path using the ModelLoader
            resolved_path = ModelLoader.resolve_model_path("qwen3_coder_next", "Qwen/Qwen3-Coder-Next")
            config.model_path = resolved_path
            print(f"[INFO] Resolved model path: {resolved_path}")
...
def test_model_interoperability():
    ...
    try:
        ...
        try:
            ...
            print(f"[FAIL] Failed to import model plugins for interoperability test: {str(e)}")
            # Try alternative import paths
            import sys
            import os
            sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'src'))
    ...
    try:
        ...
        try:
            ...
            try:
                from inference_pio.models.qwen3_0_6b.plugin import create_qwen3_0_6b_plugin
                from inference_pio.models.qwen3_coder_next.plugin import create_qwen3_coder_next_plugin
                from inference_pio.common.interfaces.improved_base_plugin_interface import TextModelPluginInterface
            except ImportError as e2:
                print(f"[FAIL] Alternative import also failed: {str(e2)}")
                return False
