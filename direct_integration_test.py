"""
Direct Integration Test for Refined Test and Benchmark Systems

This script verifies that all components of the refined test and benchmark systems
work together properly and maintain backward compatibility, without importing problematic modules.
"""
...
# Import only the specific modules we need to test directly
import importlib.util

# Import test utilities directly from the file
...
common_utils_spec = importlib.util.spec_from_file_location(
    "test_utilities", 
    "C:/Users/Admin/Documents/GitHub/Mod/src/inference_pio/common/test_utilities.py"
)
common_utils = importlib.util.module_from_spec(common_utils_spec)
common_utils_spec.loader.exec_module(common_utils)


def test_common_utilities_integration():
    """Test that common utilities work correctly with all systems."""
    # Test cache key generation
    key1 = common_utils.generate_cache_key("test_path", "test_args")
    key2 = common_utils.generate_cache_key("test_path", "test_args")
    assert key1 == key2, "Cache keys should be consistent for same inputs"
    
    # Test cache validity
    result = common_utils.is_cache_valid(time.strftime('%Y-%m-%dT%H:%M:%S'), 1.0)  # 1 hour max age
    assert result, "Fresh timestamp should be valid"
    
    # Test statistics calculation
    stats = common_utils.calculate_statistics([1, 2, 3, 4, 5])
    assert stats['mean'] == 3.0, "Mean should be 3.0"
    assert stats['count'] == 5, "Count should be 5"
    
    # Test duration formatting
    formatted = common_utils.format_duration(3661)  # 1 hour, 1 minute, 1 second
...
if __name__ == "__main__":
    print("Running direct integration tests for refined test and benchmark systems...")
