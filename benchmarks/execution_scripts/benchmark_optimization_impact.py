"""
Custom Benchmark Executor for Optimization Impact - All Models

This script runs optimization impact benchmarks for all models without using
...
def run_optimization_impact_benchmark_for_model(model_name: str):
    """
    Run optimization impact benchmark for a specific model.
    """
    print(f"\n{'='*60}")
    print(f"Running Optimization Impact Benchmark for {model_name}")
    print(f"{'='*60}")
...
def run_all_optimization_impact_benchmarks():
    """
    Run optimization impact benchmarks for all models.
    """
    models = [
        "glm_4_7",
    ...
    print("Starting Optimization Impact Benchmarks for All Models...")
    
    all_results = {}
    for model in models:
        result = run_optimization_impact_benchmark_for_model(model)
        all_results[model] = result
    
    # Print summary
    print("\n" + "="*60)
    print("OPTIMIZATION IMPACT BENCHMARK SUMMARY")
    print("="*60)
...
if __name__ == "__main__":
    results = run_all_optimization_impact_benchmarks()
    
    # Save results to file
    import json
    with open("optimization_impact_benchmark_results.json", "w") as f:
        json.dump(results, f, indent=2, default=str)
    
    print(f"\nResults saved to optimization_impact_benchmark_results.json")