"""
Final Comprehensive Benchmark Execution Script

This script runs comprehensive benchmarks comparing optimized and unoptimized versions 
of all 4 models, measuring real performance metrics and saving results to files.
"""
...
class ModelBenchmarkRunner:
    ...
    def run_model_comparison(self, model_name: str) -> Dict[str, Any]:
        ...
        # Determine the correct model path based on the model name
        model_paths = {
            "glm_4_7": "H:/GLM-4.7",
        ...
        model_path = model_paths.get(model_name, f"H:/{model_name}")

        unopt_init_success = unoptimized_plugin.initialize(device="cpu", model_path=model_path)  # Using CPU for consistency
...
def main():
    ...
    # Save JSON results
    with open("final_comprehensive_optimization_impact_results.json", "w") as f:
        json.dump(results, f, indent=2, default=str)

    # Save markdown report
    with open("final_comprehensive_optimization_impact_report.md", "w") as f:
        f.write(report)

    # Also save a summary CSV
    import csv
    with open("final_optimization_summary.csv", "w", newline='') as csvfile:
    ...
    logger.info(f"\nDetailed results saved to 'final_comprehensive_optimization_impact_results.json'")
    logger.info(f"Markdown report saved to 'final_comprehensive_optimization_impact_report.md'")
    logger.info(f"Summary CSV saved to 'final_optimization_summary.csv'")
