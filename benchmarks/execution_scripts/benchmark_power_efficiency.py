"""
Custom Benchmark Executor for Power Efficiency - All Models

This script runs power efficiency benchmarks for all models without using
...
def run_power_efficiency_benchmark_for_model(model_name: str):
    """
    Run power efficiency benchmark for a specific model.
    """
    print(f"\n{'='*60}")
    print(f"Running Power Efficiency Benchmark for {model_name}")
    print(f"{'='*60}")
    
    # Import the benchmark module
    module_path = f"inference_pio.models.{model_name}.benchmarks.performance.benchmark_power_efficiency"
    try:
    ...
    for attr_name in dir(benchmark_module):
        ...
        if (hasattr(attr, '__bases__') and 
            ...
            attr_name.endswith('PowerEfficiency')):
    ...
    # Run specific power efficiency tests
    results = {
        "model": model_name,
    ...
    test_methods = [
        'test_power_efficiency_workload',
        'test_cpu_utilization_during_inference'
    ]
...
def run_all_power_efficiency_benchmarks():
    """
    Run power efficiency benchmarks for all models.
    """
    models = [
        "glm_4_7",
    ...
    print("Starting Power Efficiency Benchmarks for All Models...")
    
    all_results = {}
    for model in models:
        result = run_power_efficiency_benchmark_for_model(model)
        all_results[model] = result
    
    # Print summary
    print("\n" + "="*60)
    print("POWER EFFICIENCY BENCHMARK SUMMARY")
    print("="*60)
...
if __name__ == "__main__":
    results = run_all_power_efficiency_benchmarks()
    
    # Save results to file
    import json
    with open("power_efficiency_benchmark_results.json", "w") as f:
        json.dump(results, f, indent=2, default=str)
    
    print(f"\nResults saved to power_efficiency_benchmark_results.json")