...
class ModelBenchmarkRunner:
    ...
    def run_benchmarks_for_model(
        self, model_info: Dict[str, str], benchmark_suite: str = "full"
    ) -> List[Any]:
        """
        ...
        try:
            ...
            # Initialize the plugin with real model (using CPU for consistent benchmarks)
            success = plugin.initialize(device="cpu", use_mock_model=False)
            if not success:
    ...
    def save_aggregated_results(
        ...
        filename_prefix: str = "aggregated_benchmarks",
    ) -> Dict[str, str]:
        """
        ...
        with open(csv_filename, "w", newline="") as f:
            ...
            writer.writerow(
                [
                    ...
                    sum(
                        len(results_list)
                        for results_list in results.get("results", {}).values()
                    ),
